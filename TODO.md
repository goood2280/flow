...
- (Claude) TEG 위치 조회에 `설비값 검사` 탭 추가 — 설비 레시피 원문(#wafer-map/<SITES>/#teg-map) 파싱(core/teg_check.py, POST /api/teg-map/inspect), 전체 Pattern 을 작은 WF MAP 카드로 동시 표시(클릭 확대·패턴별 맵 재지정), #teg-map 좌표를 flat 변환(v_R=반시계 90° 회전 원복 + PCHK/모듈 오프셋) 후 Teg_location raw ebeam 정답지와 🟢/🔴/⚪ 대조
