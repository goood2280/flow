...
> 관련 문서: `ARCHITECTURE.md`(시스템) · `DOMAIN.md`(공정 지식) · `../domain_sources_2026_04_26/FabCanvas_domain.txt`(전체 원본).
...
### Admin (`/admin`)
- **언제**: 유저 승인/탭 권한, data roots override, 알림/활동/백업/모니터처럼 앱 전체 운영에 필요한 설정.
...
- [ ] 신규 parquet 스키마 차이는 해당 데이터 페이지의 설정/룰북에서 역할 매핑 저장
...
| TableMap/SplitTable 매칭 미스 | `matching_step.csv` 3-tier(exact/alias/substring) 불충분 | 해당 페이지의 설정/룰북에서 alias/substring 추가 저장 |
