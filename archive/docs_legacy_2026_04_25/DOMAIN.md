...
> 전체 도메인 원본은 `../domain_sources_2026_04_26/FabCanvas_domain.txt`. 본 문서는 에이전트가 spot-reference 용으로 쓰는 **요약 슬라이스**.
...
- **페이지별 매핑 흐름**: S3/parquet 등록 → 해당 데이터 페이지에서 컬럼 스캔/타입 확인 → 역할 매핑 저장 후 재사용.
...
더 자세한 맥락(소프트랜딩 전략, 로드맵, UX 원칙 전문)은 `../domain_sources_2026_04_26/FabCanvas_domain.txt` [5][10][11][12] 참조.
