...
/* ═══ WIP × Split 현황 (latest cache 기반 기본 화면) ═══ */
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr)", gap: 12, paddingBottom: 14 }}>
      <div style={{ display: "flex", gap: 10, alignItems: "end", flexWrap: "wrap", padding: "10px 12px", border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-secondary)" }}>
        ...
        <label style={{ display: "grid", gap: 4, fontSize: 12, color: "var(--text-secondary)" }}>
          STEP BIN 간격 (step_id 숫자 6자리 기준)
          <select style={selStyle} value={binSize} onChange={(e) => { const b = Number(e.target.value) || 100; setBinSize(b); fetchData(product, b, splitCol); }}>
            {WIP_BIN_CHOICES.map((b) => <option key={b} value={b}>{b}</option>)}
          </select>
        </label>
        <label style={{ display: "grid", gap: 4, fontSize: 12, color: "var(--text-secondary)" }}>
...
export default function My_Dashboard({ user }) {
  ...
  // v9.2: 대시보드 기본 화면 = WIP × Split 현황 (latest cache). 기존 차트 보드는 탭으로 유지.
  const [dashboardView, setDashboardView] = useState("wip");
  ...
  <div style={{ padding: "16px 18px", background: "var(--bg-primary)", color: "var(--text-primary)", maxWidth: "none", margin: 0, height: "100%", minHeight: 0, overflow: "auto", boxSizing: "border-box" }}>
    ...
    <div style={{ marginBottom: 12 }}>
      <TabStrip
        ...
        items={[{ k: "wip", l: "WIP 현황" }, { k: "charts", l: "차트 보드" }]}
      />
    </div>

    {dashboardView === "wip" && <WipSplitPanel />}
