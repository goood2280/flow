...
AI Hub 화면의 `워크플로우 지도` 패널은 태그별 focus filter와 노드 detail을 제공한다. 운영자는 n8n처럼 반복 prompt template → policy gate → unit/function step 흐름을 보고, Obsidian처럼 도구가 어떤 지식/스키마에 연결되는지 확인한다. 노드 detail의 입력/출력 엣지는 연결 노드로 바로 이동하고, workflow 노드 detail의 `Runbook` 버튼은 같은 workflow row만 Runbook에서 펼쳐 운영 issue와 다음 조치를 이어서 볼 수 있게 한다.
