...
- `core/ai_hub_ops_snapshot.py` 는 readiness, workflow runbook, deep-eval, Agent Wiki health, 운영 timeline을 읽어 AI Hub 첫 화면의 일일 운영 스냅샷을 만든다. 새 상태를 만들지 않고 상위 개선 항목, Runbook 조치 큐, 최근 이벤트, Obsidian/n8n export 링크만 묶어 반환한다. 화면의 summary card를 누르면 해당 운영 패널이 열리고, 상위 개선 항목은 `운영 준비도` 백로그 focus로, Runbook 조치 큐 항목은 `Workflow Runbook` issue filter로, 최근 이벤트는 `운영 타임라인` category filter로 이어진다.
