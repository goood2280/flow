...
GET  /api/ai-hub/ops-snapshot               운영 스냅샷: readiness/deep-eval/wiki-health/timeline 상위 요약 + 개선/이벤트 큐
...
- `core/ai_hub_ops_snapshot.py` 는 readiness, deep-eval, Agent Wiki health, 운영 timeline을 읽어 AI Hub 첫 화면의 일일 운영 스냅샷을 만든다. 새 상태를 만들지 않고 상위 개선 항목, 최근 이벤트, Obsidian/n8n export 링크만 묶어 반환한다.
