...
- 화면의 row `상세`는 workflow step, bind/fixed slot, evidence node, missing/disabled tool, next action route를 펼쳐 보여주고, row `지도`는 `workflow:<key>` 노드를 워크플로우 지도에서 바로 focus한다. 상세의 step은 `workflow_step:<key>:<n>` 지도 노드로, evidence tag는 해당 evidence 지도 노드로, `도구`/missing/disabled tool 버튼은 AI Hub 도구 카탈로그 상세 또는 검색으로 이어진다.
- 각 row는 issue key에 맞춘 `next_actions[]`를 내려준다. 예: `missing_tools`는 ToolRegistry/workflow step 수정, `not_checked`는 Dry-run 재검증, `no_evidence`는 Wiki/schema 근거 연결로 안내한다. 화면의 next action은 자동 실행하지 않고, route/key에 따라 도구 카탈로그, 워크플로우 지도, Wiki 상태, DeepEval, 또는 해당 Runbook row로 focus해 운영자가 조치 위치를 바로 열게 한다.
...
- AI Hub 화면의 `운영 타임라인` 패널은 기존 `activity.jsonl`과 Knowledge Vault `wiki_log`를 읽어 workflow dry-run/execute, Agent Wiki source/page 변경, semantic proposal/lexicon 변경, deep-eval, tool toggle, skill 승인/거부 이력을 시간순으로 보여준다. 이벤트 제목은 workflow를 Runbook, tool을 도구 카탈로그, wiki를 Agent Wiki 상태, validation을 Agent 검증 리포트, skill을 스킬 패널로 focus한다.
