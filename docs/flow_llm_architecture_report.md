...
| Dataframe/DB | Polars, DuckDB, PyArrow, Parquet | 대용량 parquet preview, read-only filter/search, multi-DB join, aggregation |
...
- 큰 단일 파일/DB 검색과 Flow-i 차트용 다중 DB 병합은 DuckDB read-only query를 우선 시도하고, 실패하거나 미설치이면 기존 Polars lazy 경로로 fallback한다.
