...
| 계층 | 위치 | 용도 | 덮어쓰기 정책 |
|---|---|---|---|
| 코드 기본 seed | `backend/core/semiconductor_rca_seed_knowledge.json` | 보편적인 반도체 RCA/상관관계 카드, causal edge, seed case | Git/setup.py로 갱신 |
| 설치 seed | `data/flow-data/semiconductor/seed_knowledge/semiconductor_rca_seed_knowledge.json` | 새 설치에서 바로 쓰는 범용 기본 지식 | 없을 때만 생성, 있으면 보존 |
| RAG 추가 지식 | `data/flow-data/semiconductor/custom_knowledge.jsonl` | GPT 심층리서치, 사내 문서, 공정/Inline/TEG 표, 사내 item 해석 | append-only |
| 유저/엔지니어 prior | `data/flow-data/semiconductor/engineer_knowledge.jsonl` | 사용자별 업무 성향, 담당 module, 자주 보는 판단 기준 | append-only |
| reformatter | `data/flow-data/reformatter/<product>.json` | real item alias/정규화 rule | admin 또는 FileBrowser 위임 apply만 |
| 제품/TEG YAML | `data/flow-data/product_config/products.yaml` | wafer layout, TEG 좌표, 제품 설정 | admin 또는 FileBrowser 위임 apply만 |

`FLOW_DATA_ROOT`가 설정되어 있으면 `data/flow-data` 대신 그 경로 아래에 저장된다.

## 가장 쉬운 입력 방법

기본 seed에는 특정 사내 제품명/비공개 item 정의를 넣지 않는다. setup.py에 같이 들어가는 지식은 DIBL/SS/Vth/Ion/Rsd/contact/open/leakage/GAA/데이터 grain처럼 보편적인 판단 기준과 상관관계까지만 담는다. 사내에서는 Agent 탭에서 제품별 구조, raw item 이름, item_desc, TEG 좌표, step_id/func_step 기준표를 추가해 디테일을 가져간다.

Home Flow-i 또는 Agent 탭에서 아래처럼 시작한다.
...
이 입력은 `custom_knowledge.jsonl`에 저장된다. 짧은 Flow-i update는 Admin이 입력하면 public 지식으로, 일반 user가 입력하면 private 지식으로 저장된다. 문서 타입 지식 등록은 Admin 전용이며 private 범위를 쓰지 않고 항상 public 공용 RAG로 저장한다.
...
Agent 탭의 `표 지식 Preview → 확정 반영`에 공정 plan, Inline item 표, TEG 좌표표, 데이터 클리닝 기준표를 넣는다.

```text
step_id,item_id,item_desc,unit
AA200000,DIBL_SHORT,short Lg DIBL monitor,mV/V
CA100000,PC-CA_OPEN,PC to CA open monitor,count
```

서버는 먼저 preview만 만든다. `step_id`, `func_step`, `item_id`, `item_desc`, 좌표 열 이름이 조금 달라도 후보 컬럼을 잡고, `func_step`/canonical item/TEG 좌표/cleaning action을 제안한다. 사용자가 확인 후 `확인 후 RAG 반영`을 눌러야 `custom_knowledge.jsonl`에 저장된다. 이 단계는 원본 DB/파일을 수정하지 않는다.

Reformatter나 TEG YAML처럼 실제 설정 파일을 바꾸는 apply는 admin 또는 FileBrowser 위임 사용자만 가능하다. 위임받지 않은 일반 유저는 preview와 짧은 Flow-i update의 private RAG 저장까지만 할 수 있다. 긴 문서 지식은 admin이 검토해 public으로 등록한 뒤 모든 유저가 에이전트 검색에서 활용한다.
