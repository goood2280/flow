...
- SplitTable UI의 root lot 조회는 `/api/splittable/view?cache_first=1`로 먼저 들어간다. 이 요청은 middleware heavy guard를 우회하지만, payload/root/product cache hit 또는 lookup cache 준비 응답만 허용하고 원본 ML_TABLE raw scan fallback은 수행하지 않는다. lookup cache miss/stale이면 build를 background로 queue하고 사용자는 준비 중 응답을 받는다.
...
- ML_TABLE root lot 후보와 lot view는 `backend/core/ml_table_lookup.py`의 root-lot lookup cache를 먼저 사용한다. 후보 조회는 fresh cache의 `root_lot_id=<id>` partition 이름을 index로 읽고, cache hit view는 원본 `ML_TABLE_*.parquet` 전체 scan 대신 해당 partition에서 필요한 KNOB/MASK/CUSTOM 컬럼을 읽는다. 큰 parquet에서 cache miss/stale이면 build를 백그라운드로 큐잉하고 현재 후보/view 요청은 `lookup_cache.queued=true`와 준비 메시지를 빠르게 반환한다. 작은 source는 기존 원본 fallback 검색을 유지한다. cache build worker는 shared cache가 이미 fresh이면 재생성하지 않는다.
