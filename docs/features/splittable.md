...
- SplitTable UI의 root lot 조회는 `/api/splittable/view`로 들어간다. payload/root/product/lookup cache가 있으면 먼저 사용하고, lookup cache가 miss/stale이면 build를 background로 queue한 뒤 같은 요청에서 원본 ML_TABLE에 root/wafer/prefix 필터를 적용해 raw fallback 결과를 반환한다. 이 요청은 raw fallback을 수행할 수 있으므로 middleware heavy guard를 우회하지 않는다.
...
- ML_TABLE root lot 후보와 lot view는 `backend/core/ml_table_lookup.py`의 root-lot lookup cache를 먼저 사용한다. 후보 조회는 fresh cache의 `root_lot_id=<id>` partition 이름을 index로 읽고, cache hit view는 원본 `ML_TABLE_*.parquet` 전체 scan 대신 해당 partition에서 필요한 KNOB/MASK/CUSTOM 컬럼을 읽는다. cache miss/stale이면 build를 백그라운드로 큐잉해 `lookup_cache.queued=true`를 함께 반환하되, 현재 후보/view 요청은 준비 메시지로 종료하지 않고 원본 `ML_TABLE_*.parquet`에 요청 필터를 적용해 raw fallback 결과를 반환한다. cache build worker는 shared cache가 이미 fresh이면 재생성하지 않는다.
- ML_TABLE lookup cache build는 단일 background queue에서 순차 처리한다. 각 source build 전 runtime memory guard가 high이면 대기하고, 이후 전체 source DataFrame을 `collect()`하지 않고 `LazyFrame.sink_parquet(PartitionBy(...))`로 `root_lot_id=<id>` partition을 바로 쓴다. 대기 간격은 `FLOW_ML_TABLE_LOOKUP_CACHE_MEMORY_WAIT_SECONDS`, partition file row cap은 `FLOW_ML_TABLE_LOOKUP_CACHE_MAX_ROWS_PER_FILE`로 조정한다.
- `/api/splittable/long-wide-preview`는 FAB/INLINE/ET long source pivot을 요청 중에 직접 수행하지 않는다. cache가 fresh이면 `data/flow-data/splittable/long_pivot_cache/*.parquet`에서 preview row만 읽고, cache miss/stale이면 pivot build를 background worker에 queue한 뒤 `pivot_cache.queued=true`를 반환한다.
- root-lot lookup cache 위에는 root lot별 RAM cache를 둔다. 기본 예산은 3GB(`FLOW_SPLITTABLE_ROOT_LOT_RAM_CACHE_MAX_GB`)이고, root-lot RAM cache CPU 예산은 최대 2 core(`FLOW_SPLITTABLE_ROOT_LOT_RAM_CACHE_CPU_CORES`, 상한 2.0)로 둔다. 스케줄러는 30분마다 톱니바퀴의 root lot cache 설정을 읽어 기본 `AZ` prefix root lot을 최대 설정 개수까지 예열한다. prefix/최근 검색 root lot 캐싱 개수 설정 상한은 50,000이다. 캐시 후보는 현재 설정 prefix에 맞는 `prefix` 그룹과 그 외 `other` 그룹으로 나누고, prefix 그룹을 먼저 RAM에 올린 뒤 `cache/lot_progress_latest_lot_by_root_wafer.parquet`의 최신 root와 최근 조회/검색한 other root lot을 유지한다. 상태 응답은 그룹별 target/hit 수와 root별 `cache_group`을 노출한다. lookup partition이 아직 없거나 원본이 stale이면 background build를 enqueue하고 현재 요청은 기존 fallback을 유지한다. 프로세스 CPU/메모리 guard가 초과 상태이면 남은 root 예열을 건너뛰고 상태 응답에 `last_resource_guard_reason`을 남긴다.
