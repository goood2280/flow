...
- ML_TABLE root lot 후보와 lot view는 `backend/core/ml_table_lookup.py`의 root-lot lookup cache를 먼저 사용한다. 후보 조회는 cache build 때 함께 저장한 `_candidate_index.json`의 `root_lot_ids`를 우선 읽고, 이 index에는 자주 쓰는 `KNOB_` 열 목록도 prefix별로 분류해 둔다. index가 없는 오래된 fresh cache만 `root_lot_id=<id>` partition 이름을 fallback index로 읽는다. cache hit view는 원본 `ML_TABLE_*.parquet` 전체 scan 대신 해당 partition에서 필요한 KNOB/MASK/CUSTOM 컬럼을 읽는다. cache miss/stale이면 build를 백그라운드로 큐잉해 `lookup_cache.queued=true`를 함께 반환하되, 현재 후보/view 요청은 준비 메시지로 종료하지 않고 원본 `ML_TABLE_*.parquet`에 요청 필터를 적용해 raw fallback 결과를 반환한다. cache build worker는 shared cache가 이미 fresh이면 재생성하지 않는다.
