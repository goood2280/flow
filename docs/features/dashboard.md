...
- Home Flow-i의 ET 단일 metric trend는 `tkout_time` X축 scatter, `lot_wf` 기준 median 집계가 기본이다. 직전 trend chart session이 있으면 KNOB 컬러링/값 제외 후속 요청은 같은 product/metric/lot scope를 재사용한다.
