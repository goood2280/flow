...
- 실패 시 UI/API에 보여줄 진단
- 실행할 검증 명령
- 현재 브랜치가 로컬 `main`인지, 작업트리가 이미 dirty라면 어떤 변경이 있는지

이 답이 없으면 UI부터 붙이지 않는다.

## 작업트리 정리

- Flow 작업은 로컬 `main` 기준으로 진행한다.
- 각 작업은 수정, 검증, 문서 반영을 마친 뒤 하나의 정리된 커밋으로 `main`에 남긴다.
- 작업이 끝났는데 `git status --short`가 계속 dirty인 상태를 유지하지 않는다.
- 다음 작업을 시작하기 전에는 이전 작업이 `main`에 정리되어 있고 작업트리가 깨끗한지 확인한다.
- 이미 있던 변경을 되돌리거나 덮지 말고, 현재 로컬 변경을 포함해 사용자가 볼 수 있는 단위로 정리한다.
- GitHub `main`에 푸시할 때는 변경 범위가 작거나 직전에 검증을 했더라도 푸시 전 `_build_setup.py`로 `setup.py`를 다시 만들고, `python setup.py version` 출력을 확인한 뒤 그 diff까지 커밋한다.
