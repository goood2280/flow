...
function Code({ children }) {
  ...
  return <pre className="flow-fixed" style={{ background:"#171717", borderRadius:8, padding:"14px 18px", border:"1px solid #333", overflow:"auto", fontFamily:mono, fontSize:14, lineHeight:1.8, color:"#e5e5e5", whiteSpace:"pre", margin:"12px 0" }}>{children}</pre>;
}
...
└── data/                    # 로컬 fallback root (Git 제외)
    ...
    └── flow-data/           # local 사용자/운영 상태`}</Code>
        ...
        <H2 id="tabs">탭 구성</H2>
        <p>탭 메타, 네비게이션 그룹, 레이아웃, lazy page 연결은 <code>frontend/src/app/pageManifest.jsx</code>에서 한 번만 관리합니다. <code>npm run design:check</code>가 페이지 누락과 백엔드 <code>core/auth.py TAB_SUBTABS</code> 불일치를 차단합니다.</p>
        <Code>{`main   : home(홈)
...
* tablemap 페이지는 PAGE_MAP에는 등록되어 있으나 네비 탭에는 없음.`}</Code>
        ...
        <RouterRow file="lot_requests.py" prefix="/api/lot-requests" desc="랏 배정·Hot grade 요청/PI 답변/처리 이력" />
        ...
        <H2 id="worklog">업무 이력/리치 본문</H2>
        <p><code>lotrequest</code>는 Tracker와 분리된 제품별 PI 업무 보드입니다. 요청 유형은 랏 배정·Hot grade·기타, 상태는 등록·처리완료·반려입니다. 기존 처리중 데이터는 이력을 보존한 채 현재 등록 상태로 호환 표시합니다. 요청·답변·상태 전환마다 작성자와 시각을 저장하고 감사 로그를 별도로 남깁니다.</p>
        <p>UI는 인폼로그형 이슈 피드 구조입니다. 제품 탭을 두지 않고 제품·이슈·등록자·요청 팀·유형·상태·우선순위·등록일·답변 수를 컬럼 한 줄로 표시합니다. 제품·요청 팀·요청 유형·등록자·상태·내 요청·검색 조건은 조합 가능하며 <code>/stats</code>도 상태 이외의 현재 필터를 받아 건수를 재집계합니다. 이슈를 선택하면 해당 행 바로 아래에 요청 내용·처리/답변·이력 탭을 펼치며 상세에서는 목록 메타데이터를 반복하지 않습니다.</p>
        <Code>{`storage (<FLOW_DATA_ROOT>/lot_requests/):
...
permission boundary:
  ...
  * admin도 타인의 본문 소유권을 우회하지 않음`}</Code>

        <div style={{fontSize:14,fontWeight:700,color:"var(--accent)",marginTop:20,marginBottom:8,fontFamily:mono}}>랏 요청 API</div>
        <ApiRow method="GET" path="/api/lot-requests" desc="필터 가능한 요청 목록" />
        <ApiRow method="GET" path="/api/lot-requests/{id}" desc="답변·상태/편집 이력이 포함된 상세" />
        <ApiRow method="POST" path="/api/lot-requests" desc="요청 등록" />
        <ApiRow method="POST" path="/api/lot-requests/{id}/status" desc="PI 처리 상태 변경" />
        <ApiRow method="POST" path="/api/lot-requests/{id}/responses" desc="PI 답변 등록" />
        <ApiRow method="POST" path="/api/lot-requests/upload" desc="본문 이미지 업로드(최대 8MB)" />
        <ApiRow method="POST" path="/api/lot-requests/{id}/mail-preview" desc="요청·답변·히스토리 HTML 메일 미리보기" />
        <ApiRow method="POST" path="/api/lot-requests/{id}/send-mail" desc="선택 사용자·공용 그룹·추가 이메일로 발송 후 증적 기록" />

        <p><strong style={{color:"var(--text-primary)"}}>리치 본문 규약:</strong> 프런트엔드는 <code>components/RichBoardEditor.jsx</code>를 공통으로 사용합니다. 이미지 붙여넣기는 clipboard items/files와 HTML data image를 함께 감지하고, 현재 커서 또는 편집기 끝에 인증 이미지 URL을 삽입합니다. Windows 계열 MIME은 안전한 이미지 확장자로 정규화하며 TSV 붙여넣기는 표로 바꿉니다. 저장 직전 표시용 토큰은 제거합니다. 백엔드는 <code>core/rich_text.py</code>로 태그·속성·스타일을 정제하며 <code>/api/lot-requests/files/...</code>와 <code>/api/informs/files/...</code> 내부 이미지만 허용합니다.</p>
        <p>Inform 신규 Note는 <code>text_format: "html"</code>로 저장합니다. 기존 레코드는 기본값 <code>plain</code>으로 계속 렌더링하고, 메일 HTML 생성 시 내부 이미지를 인증된 파일에서 읽어 data URI로 변환합니다. 리치 본문 필드를 새로 추가할 때는 브라우저 정제만 신뢰하지 말고 반드시 서버 정제와 내용 존재 검사를 함께 적용합니다.</p>
        <p><strong style={{color:"var(--text-primary)"}}>메일과 증적:</strong> 랏 요청 메일은 Inform의 승인 사용자·공용 그룹 카탈로그와 Admin 메일 API 설정을 재사용합니다. HTML 본문에는 요청, 대상 랏, 답변과 통합 <code>activity_history</code>가 포함됩니다. 성공한 발송만 <code>mail_history</code>, <code>activity_history</code>, <code>audit.jsonl</code> 세 위치에 기록하며 실제 수신 주소·제목·발송자·시각·dry-run 상태를 남깁니다.</p>
        <p><strong style={{color:"var(--text-primary)"}}>페이지 설정·처리 권한:</strong> <code>GET/POST /api/lot-requests/config</code>은 <code>&lt;FLOW_DATA_ROOT&gt;/lot_requests/config.json</code>의 요청 유형과 요청한 팀 목록을 관리합니다. UI에서는 입력칸의 추가 버튼 또는 Enter 키로 값을 하나씩 등록하고 칩의 삭제 버튼으로 개별 제거합니다. 진행·완료·반려 상태 전환, 처리 답변 등록·수정·삭제, 메일 미리보기·발송은 <code>admin_settings.json</code>의 <code>page_admins.lotrequest</code>에 명시적으로 위임된 사용자만 가능합니다. 전역 관리자 권한만으로는 처리할 수 없습니다. 모든 변경과 발송은 이력·감사 로그에 남습니다.</p>
...
measure_time   # 측정 시각 또는 source update time`}</Code>
        ...
        <p><strong style={{color:"var(--text-primary)"}}>INLINE:</strong> 원천 DB는 <code>subitem_id</code>로 측정 위치를 구분하고 <code>shot_x/shot_y</code>는 없습니다. 기본 집계는 <code>lot_wf</code> 기준 평균이며, <code>inline_matching.csv</code>의 <code>matching_table</code>이 TEG 위치조회의 Inline map TABLE을 가리킬 때 매핑된 subitem만 ET shot 좌표로 변환합니다. AVG/MED/STD/MIN/MAX/Q1/Q3 요약 행은 평균과 shot 매칭에서 제외합니다.</p>
...
- atomic cache build: temp file -> replace`}</Code>
        ...
        <H2 id="ux">UX 시스템</H2>
        <p>Flow는 작업용 도구이므로 페이지마다 같은 밀도, 글씨, 색, 표 스타일을 사용합니다. 신규 화면은 <code>frontend/src/components/ui</code>에서만 UI 프리미티브를 가져오고, <code>styles/tokens.css</code>에 없는 시각 값을 직접 만들지 않습니다. <code>UXKit.jsx</code>는 기존 화면을 위한 호환 계층입니다.</p>
        <Code>{`UX rules:
...
- npm run design:check와 npm run build를 모두 통과`}</Code>
        <p><strong style={{color:"var(--text-primary)"}}>게시판형 입력:</strong> 표·이미지가 필요한 업무 본문은 별도 첨부 버튼보다 <code>RichBoardEditor</code>를 사용합니다. 이미지/Excel 표 Ctrl+V, 키보드 포커스, 빈 HTML 판정, 저장 후 재표시, 메일 렌더링까지 한 흐름으로 검증합니다.</p>
...
def get_items():
    return {"items": [...]}`}</Code>
        ...
        <p><strong style={{color:"var(--text-primary)"}}>2단계.</strong> 프론트 페이지 생성. API 호출은 <code>fetch()</code> 직접 호출 대신 <code>src/lib/api.js</code> helper를 사용하고, 화면은 공통 Shell/Header/상태 컴포넌트로 시작합니다.</p>
  ...
  return (
    <PageShell layout="analysis">
      ...
      <div className="flow-page__content">
        ...
        <p><strong style={{color:"var(--text-primary)"}}>3단계.</strong> 탭 등록 — <code>pageManifest.jsx</code> 한 곳에 메타와 lazy import를 추가합니다. 신규 항목은 <code>designSystem: true</code>가 필수입니다.</p>
        <Code>{`// frontend/src/app/pageManifest.jsx
{
  key: "myfeature",
  label: "내 기능",
  icon: "🧩",
  ...
  load: () => import("../pages/My_MyFeature"),
        ...
        <Code>{`cd frontend && npm run check
python scripts/smoke_test.py      # 서버 떠 있는 상태
python -m pytest tests            # 백엔드 단위 테스트`}</Code>

        <p style={{marginTop:12}}>manifest에 등록하면 네비게이션·홈·권한 편집 화면에 자동 반영됩니다. 소탭을 추가하면 <code>design:check</code>가 백엔드 <code>core/auth.py TAB_SUBTABS</code> 갱신 여부까지 확인합니다.</p>
...
python setup.py restore [latest]  # ~/.flow_backups 스냅샷 복원`}</Code>
        ...
        <p>CSS 변수 기반 다크/라이트 테마. 토큰 값은 <code>styles/tokens.css</code>가 소유하고, <code>app/useFlowShell.js</code>는 <code>data-theme</code> 상태만 전환합니다.</p>
        <Code>{`/* 사용 가능한 CSS 변수 */
...
*/30 * * * * curl -X POST http://localhost:8080/api/monitor/heartbeat`}</Code>
        ...
        <p style={{marginTop:24,padding:"12px 16px",borderRadius:8,background:"var(--bg-card,#2a2a2a)",border:"1px solid var(--border,#333)",fontSize:14}}>
          ...
          <strong style={{color:"var(--accent)"}}>초기 admin:</strong> FLOW_ADMIN_PW 지정 시 hol 생성 &nbsp;|&nbsp;
