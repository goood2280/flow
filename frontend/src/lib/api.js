...
// JSON body를 보내고 생성된 PPTX/ZIP 같은 binary 응답을 바로 저장한다.
export function postDownload(url, body, filename) {
  const _isApi = typeof url === "string" && url.startsWith("/api/");
  if (_isApi) _touchActivity();
  const opts = {
    method: "POST",
  ...
  return fetch(url, _withAuthHeaders(opts)).then(async (r) => {
    ...
    if (!r.ok) {
      ...
      await _throwApiError(url, opts, r.status, payload, "Download failed");
    }
    const blob = await r.blob();
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
