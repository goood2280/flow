...
function InlineETOverviewPanel({ result }) {
  if (!result) return null;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "var(--accent)", fontFamily: "monospace" }}>🧪 Inline_ET + KNOB</div>
          <div style={{ fontSize: 10, color: "var(--text-secondary)", fontFamily: "monospace" }}>{result.product} · target={result.target_et} · rows={result.rows}</div>
        </div>
        <div style={{ fontSize: 10, color: "var(--text-secondary)", lineHeight: 1.6 }}>{result.note}</div>
      </div>
      <div style={{ background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: 16 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "var(--accent)", fontFamily: "monospace", marginBottom: 8 }}>ET 타겟 기준 Top Inline</div>
        <table style={{ width: "100%", fontSize: 10, borderCollapse: "collapse", fontFamily: "monospace" }}>
          <thead><tr style={{ color: "var(--text-secondary)", borderBottom: "1px solid var(--border)" }}>
            <th style={{ textAlign: "left", padding: "4px 6px" }}>Inline feature</th>
            <th style={{ textAlign: "right", padding: "4px 6px" }}>|corr|</th>
            <th style={{ textAlign: "center", padding: "4px 6px" }}>dir</th>
          </tr></thead>
          <tbody>
            {(result.top_inline || []).map((r, i) => (
              <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <td style={{ padding: "4px 6px", color: "var(--text-primary)" }}>{r.feature}</td>
                <td style={{ padding: "4px 6px", textAlign: "right", color: Math.abs(r.importance) > 0.4 ? "#10b981" : Math.abs(r.importance) > 0.25 ? "#f59e0b" : "var(--text-secondary)", fontWeight: 700 }}>{r.importance.toFixed(3)}</td>
                <td style={{ padding: "4px 6px", textAlign: "center", color: r.direction === "+" ? "#10b981" : "#ef4444", fontWeight: 700 }}>{r.direction === "+" ? "↑" : "↓"}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
        {(result.knob_cards || []).map((card, i) => (
          <div key={card.knob} style={{ background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: COLORS[i % COLORS.length], fontFamily: "monospace" }}>{card.knob}</div>
              <div style={{ fontSize: 9, color: "var(--text-secondary)", fontFamily: "monospace" }}>earliest {card.earliest_step_id || "-"}</div>
            </div>
            {(card.modules || []).length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 8 }}>
                {card.modules.map((mod) => (
                  <span key={mod} style={{ fontSize: 9, fontFamily: "monospace", padding: "2px 6px", borderRadius: 999, background: "rgba(16,185,129,0.08)", color: "#059669", border: "1px solid rgba(16,185,129,0.18)" }}>{mod}</span>
                ))}
              </div>
            )}
            {(card.groups || []).map((g, gi) => (
              <div key={gi} style={{ marginBottom: 6, padding: "6px 8px", borderRadius: 6, background: COLORS[gi % COLORS.length] + "16", border: "1px solid " + COLORS[gi % COLORS.length] }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, fontSize: 10, fontFamily: "monospace" }}>
                  <span style={{ color: "var(--text-primary)", fontWeight: 700 }}>{g.value}</span>
                  <span style={{ color: "var(--text-secondary)" }}>n={g.count}</span>
                </div>
                <div style={{ fontSize: 10, color: "#10b981", fontFamily: "monospace", marginTop: 2 }}>ET μ = {g.target_mean != null ? g.target_mean.toFixed(3) : "-"}</div>
                {Object.keys(g.inline_means || {}).length > 0 && (
                  <div style={{ marginTop: 4, display: "flex", flexDirection: "column", gap: 2 }}>
                    {Object.entries(g.inline_means || {}).map(([k, v]) => (
                      <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 9, fontFamily: "monospace", color: "var(--text-secondary)" }}>
...
export default function My_ML({ user }) {
  ...
  const [tab, setTab] = useState("train"); // train | window | transfer | pareto | flow | wfmap | inlcorr | inlineet | ppid | guide
  ...
  const [inlineEtResult, setInlineEtResult] = useState(null);
  ...
  const train = () => {
    ...
    setLoading(true); setResult(null); setTransferResult(null); setPwResult(null); setTrResult(null); setPrResult(null); setFlowResult(null); setWfResult(null); setInlResult(null); setInlineEtResult(null); setPpidResult(null);
    const body = {
    ...
    if (tab === "window") {
      ...
      sf(API + "/inline_corr_search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source_type: source.source_type || "flat", root: source.root || "", product: source.product || "", file: source.file || "", target, max_pairs: maxPairs, top_k: 25, filter_expr: filter }) })
        ...
        .catch(e => { alert(e.message); setLoading(false); });
    } else if (tab === "inlineet") {
      sf(API + "/inline_et_overview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source_type: source.source_type || "flat", root: source.root || "", product: source.product || "", file: source.file || "", target_et: target, filter_expr: filter }) })
        .then(d => { setInlineEtResult(d); setLoading(false); })
        .catch(e => { alert(e.message); setLoading(false); });
    } else if (tab === "ppid") {
  ...
  return (
    <div style={{ padding: "24px 32px", background: "var(--bg-primary)", minHeight: "calc(100vh - 48px)", color: "var(--text-primary)" }}>
      <div style={{ marginBottom: 16, padding: "12px 14px", borderRadius: 8, border: "1px solid rgba(249,115,22,0.35)", background: "rgba(249,115,22,0.08)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: "rgba(249,115,22,0.15)", color: "var(--accent)", fontFamily: "monospace", letterSpacing: "0.06em" }}>BETA</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: "var(--accent)", fontFamily: "monospace" }}>ML 분석 탭 공개</span>
            </div>
            <div style={{ fontSize: 12, lineHeight: 1.6, color: "var(--text-secondary)" }}>
              Inline_ET 요약과 공정 윈도우 분석을 우선 공개합니다. 운영 피드백을 받아 모델 연결과 설명력을 단계적으로 확장합니다.
            </div>
          </div>
          <button onClick={() => setTab("inlineet")} style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid var(--accent)", background: tab === "inlineet" ? "var(--accent)" : "transparent", color: tab === "inlineet" ? "#fff" : "var(--accent)", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: "monospace" }}>
            ▶ Inline_ET 바로가기
          </button>
        </div>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 800, padding: "2px 8px", borderRadius: 999, background: "rgba(249,115,22,0.15)", color: "var(--accent)", fontFamily: "monospace", letterSpacing: "0.06em" }}>BETA</span>
          </div>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {[["train", "학습"], ["window", "공정 윈도우"], ["flow", "모델 흐름"],
              ["transfer", "전이"], ["pareto", "파레토"],
              ["wfmap", "WF Map"], ["inlcorr", "INLINE 상관"], ["inlineet", "Inline_ET"], ["ppid", "PPID 분할"],
              ["guide", "가이드"]].map(([k, l]) => (
        ...
        <div style={{ fontSize: 11, color: "var(--text-secondary)" }}>
          {tab === "window" ? "L0→L1→L2→L3 인과 분석"
            ...
            : tab === "inlineet" ? "ET target + Inline + KNOB 그룹 요약"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, minWidth: 0 }}>
          {!result && !pwResult && !trResult && !prResult && !flowResult && !wfResult && !inlResult && !inlineEtResult && !ppidResult && !loading && (
          ...
          {tab === "inlineet" && inlineEtResult && <InlineETOverviewPanel result={inlineEtResult} />}
