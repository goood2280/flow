...
// 시간 창은 차트 코드가 가진 값이다 — 여기서는 라벨로만 쓴다.
...
export default function My_TemplateReport({user}){
  ...
  const resetRun=()=>{setDeck(null);setRuns({});setTables({});setImages([]);};
  ...
  const runReport=async()=>{
    ...
    try{
      ...
      const prepared=await postJson(`${API}/run`,{template_id:templateId,bindings,repeat_values:repeatValues});
      ...
      await Promise.all([
        ...(nextDeck.charts||[]).map(async chart=>{
          try{const result=await postJson("/api/filebrowser/chart-builder/run",chart.request);nextRuns[chart.key]={...chart,definition:chart.request,result};}
          catch(error){nextRuns[chart.key]={...chart,definition:chart.request,error:error.message||String(error)};}
        }),
        ...splitBlocks.map(async block=>{
          try{
      ...
      const captured=[];
      for(const chart of nextDeck.charts||[]){
  ...
  const download=async kind=>{
    ...
    const repeatValues=text(repeatText).split(/[,\n]+/).map(item=>item.trim()).filter(Boolean);
    const payload={
      template_id:template.id,bindings,repeat_values:repeatValues,images,
        ...
        <div style={{...card,padding:14,marginBottom:14}}>
          ...
          {!editing&&<div style={{display:"grid",gap:10,marginTop:12}}>
            ...
            <div style={{display:"flex",gap:9,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <span style={{fontSize:11,color:"var(--text-secondary)"}}>Template에 저장된 그대로 한 번 실행하고 PPTX로 내려받습니다.{deck?` · ${deck.pages.length}장 생성됨`:""}</span>
            </div>
            {/* 기간은 이 화면이 정하지 않는다 — 차트생성 코드의 RECENT_DAYS 를 그대로 보여만 준다. */}
            {!!chartSlots.length&&<div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center"}}>
              <span style={{fontSize:11,color:"var(--text-secondary)"}}>조회 기간은 차트생성 코드에 있습니다 ·</span>
