...
function newSource(index){return{id:`q${index}`,root:"",product:"",sql:"",select_cols:"",apply_reformatter:false,reformatter_items:"",runtime_recent_days:"",runtime_date_column:""};}
// 시간 창은 저장되는 코드가 가진다 — Template Report 는 이 값을 그대로 다시 실행할 뿐이다.
...
MAX_ROWS = 5000`,
  },
  {
    ...
    interpretation:"Template Report 실행 시 PRODUCT와 LOT만 바꿉니다. Split 조건과 ET 값을 JOIN하고, 저장된 코드의 최근 60일 조회 및 최근 7일 강조 규칙까지 그대로 재실행됩니다. 보고서에서 기간이나 색을 다시 입력할 필요가 없습니다.",
    code:`[split]
TABLE = SPLITTABLE
PRODUCT = {{PRODUCT}}
...
function QueryCard({source,index,roots,onChange,onRemove,onClone}){
  ...
  return <div style={card}>
    ...
    {/* 시간 창 — 이 값이 저장 코드(RECENT_DAYS)에 실려 Template Report 실행에도 그대로 쓰인다. */}
    <div style={{display:"grid",gridTemplateColumns:"minmax(120px,1fr) minmax(160px,1.4fr)",gap:9,marginTop:9}}>
      ...
      <div style={{gridColumn:"1 / -1",fontSize:11,color:"var(--text-secondary)"}}>
        {recentDaysValue(source)
          ?`${text(source.runtime_date_column).trim()||DEFAULT_DATE_COLUMN} 기준 최근 ${recentDaysValue(source)}일만 조회합니다. 저장 코드에 RECENT_DAYS로 실려 Template Report에서도 같은 구간이 나옵니다.`
          :"기간 제한 없이 조회합니다. 보고서에서 매번 같은 구간을 보려면 일수를 적어 주세요."}
      </div>
    </div>
...
export default function My_ChartBuilder({user}){
  ...
  useEffect(()=>{sf("/api/filebrowser/roots?fast=true").then(d=>setRoots(d.roots||[])).catch(e=>toast.error(e.message));},[]);
  ...
  const requestSource=source=>{
    ...
    return{...resolved,runtime_recent_days:days,runtime_date_column:days?(text(resolved.runtime_date_column).trim()||DEFAULT_DATE_COLUMN):""};
  };
  ...
  const run=async(config=null,options={})=>{
    const activeSources=(config?.sources||sources).map(requestSource);
  ...
  const formToCode=()=>{
    const code=definitionFromForm(sources,sources.length>1?joins:[],maxRows,currentChartConfig());
  ...
  return <div style={{padding:"20px 24px 60px",maxWidth:1500,margin:"0 auto",color:"var(--text-primary)"}}>
    ...
    <details style={{...card,marginBottom:16,padding:0,overflow:"hidden"}}>
      ...
      <div style={{padding:"0 14px 13px",borderTop:"1px solid var(--border)",fontSize:13,lineHeight:1.75,color:"var(--text-secondary)"}}>
        ...
        <div><b style={{color:"var(--text-primary)"}}>기간</b> — Query마다 <code>RECENT_DAYS = 7</code>(선택 <code>DATE_COLUMN = tkout_time</code>)을 적으면 그 열 기준 최근 7일만 조회합니다. <b style={{color:"var(--text-primary)"}}>이 값은 저장 코드에 함께 실려</b> Template Report가 실행할 때도 같은 구간이 나옵니다 — 보고서 화면에는 기간 입력이 없습니다.</div>
