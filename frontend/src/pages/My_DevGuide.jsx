  ...
  return (
    <div style={{ display:"flex", minHeight:"calc(100vh - 48px)", background:"var(--bg-primary,#1a1a1a)", color:"var(--text-primary,#e5e5e5)", fontFamily:"'Pretendard',sans-serif" }}>
      ...
      <div style={{ flex:1, padding:"28px 36px", maxWidth:860, overflow:"auto", lineHeight:1.8, fontSize:13, color:"var(--text-secondary,#a3a3a3)" }}>
        ...
        <H2 id="arch">아키텍처</H2>
        <p>FabCanvas.ai = <strong style={{color:"var(--text-primary)"}}>FastAPI</strong> (백엔드) + <strong style={{color:"var(--text-primary)"}}>React + Vite</strong> (프론트엔드) + <strong style={{color:"var(--text-primary)"}}>Polars/Parquet</strong> (데이터)</p>
...
[Data]
  ...
  /config/work/sharedworkspace/holweb-data/ → Users, logs, sessions (read-write)`}</Code>

        <p><strong style={{color:"var(--text-primary)"}}>동적 라우터 로딩:</strong> backend/routers/ 에 .py 파일을 넣으면 자동으로 등록됩니다. app.py 수정이 필요 없습니다.</p>
        <Code>{`# backend/app.py (핵심 로직)
for f in sorted(ROUTERS_DIR.glob("*.py")):
    mod = importlib.import_module(f"routers.{f.stem}")
    if hasattr(mod, "router"):
        app.include_router(mod.router)  # 자동 등록`}</Code>

        <H2 id="files">파일 구조</H2>
        <Code>{`holweb/
├── setup.py                 # 전체 프로젝트 생성 (1회)
├── update_v*.py             # 기능 업데이트 스크립트
...
└── /config/work/sharedworkspace/
    ...
    └── holweb-data/         # 앱 데이터 (읽기쓰기)
        ...
        <p>Swagger UI: <code style={{background:"#333",padding:"2px 8px",borderRadius:4,fontSize:11,fontFamily:mono}}>http://server:8080/docs</code></p>
...
PATHS.db_root      # /config/work/sharedworkspace/DB
PATHS.data_root    # /config/work/sharedworkspace/holweb-data
PATHS.users_csv    # holweb-data/users.csv
PATHS.activity_log # holweb-data/logs/activity.jsonl
PATHS.log_dir      # holweb-data/logs/

# 환경변수로 오버라이드 가능
HOL_DB_ROOT=/other/path uvicorn app:app`}</Code>
