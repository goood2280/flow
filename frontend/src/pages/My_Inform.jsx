...
function reInformFormFromParent(root) {
  ...
  return {
    ...
    split_check_display: displayMode === "split_check",
  };
}
...
export default function My_Inform({ user }) {
    ...
    if (isReInform) {
      ...
      return postJson(API, payload).then((res) => {
        ...
        if (rootForDetail?.id) {
          ...
          loadDetailForRoot(rootForDetail);
  ...
  useEffect(() => {
    if (!creating || wizardMode !== "create" || wizardStep !== 3) return;
      ...
      <div style={{ flex: "0 0 auto", padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid var(--border)", background: "var(--bg-secondary)", minWidth: 0 }}>
        ...
        {root && (
          <span style={{ marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 6, flexWrap: "wrap", justifyContent: "flex-end", minWidth: 0 }}>
            {root.product && <Chip style={{ background: "var(--brand-50)", borderColor: "var(--brand-line)", color: "var(--brand)", fontWeight: 800 }}>{stripMlPrefix(root.product)}</Chip>}
            <LotPill root={root} />
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start", minWidth: 0 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <LotPill root={root} />
              {root.product && <Chip style={{ background: "var(--brand-50)", borderColor: "var(--brand-line)", color: "var(--brand)" }}>{stripMlPrefix(root.product)}</Chip>}
...
function InformWizard({
  ...
  useEffect(() => {
    ...
    return () => { alive = false; };
  }, [attachMode, selectedSetIds, setRows, form.product, form.lot_id, form.split_check_display, lotOptions, isReInform]);
  const validate = () => {
    if (step === 0) {
            ...
            <section style={{ border: "1px solid var(--border)", borderRadius: 10, background: "var(--bg-card)", padding: 10, display: "grid", gap: 8 }}>
                ...
                <div style={{ maxHeight: 320, overflow: "auto", border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-primary)" }}>
                  ...
                  {visibleFabOptions.map(o => {
                    ...
                    return (
                      <label key={o.value} style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 34, padding: "6px 9px", borderBottom: "1px solid var(--border)", background: on ? "var(--accent-glow)" : "transparent", cursor: isReInform ? "default" : "pointer", fontFamily: "monospace", fontWeight: 800 }}>
                        <input type="checkbox" checked={on} disabled={isReInform} onChange={() => toggleFab(o.value)} />
                        <span>{o.value}</span>
                      </label>
                    );
                  })}
              ...
              <div style={{ border: "1px solid var(--border)", borderRadius: 10, padding: 12, background: "var(--bg-card)", display: "grid", gap: 8, maxHeight: 560, overflow: "auto" }}>
                ...
                <div style={{ maxHeight: 340, overflow: "auto", display: "grid", gridTemplateColumns: "1fr", gap: 2, border: "1px solid var(--border)", borderRadius: 8, background: "var(--bg-primary)" }}>
                  {filteredCols.map(c => {
                    ...
                    return (
                      <label key={c} style={{ display: "flex", gap: 7, alignItems: "center", minHeight: 32, padding: "5px 8px", borderBottom: "1px solid var(--border)", background: on ? "var(--accent-glow)" : "transparent", cursor: "pointer", fontFamily: "monospace", minWidth: 0 }}>
                ...
                <table style={{ borderCollapse: "collapse", border: "1px solid #d1d5db", width: "100%", maxWidth: 560 }}>
                  <tbody>
                    {[
                      ...
                      <tr key={k}>
                        <td style={{ padding: "4px 10px", color: "var(--muted)", background: "var(--bg-tertiary)", width: 90 }}>{k}</td>
