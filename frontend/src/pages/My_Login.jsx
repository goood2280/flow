...
function MatrixRain() {
  ...
  useEffect(() => {
    ...
    let w, h, cols, drops;
    const pool = "OPENSHORTLKGISOFAILPASSBINWAFERLOTDIEYIELDDEFECTSPECCPFTEDSATESPCPROBEMAPVTHRDSONBVDSSFABCANVASETCHCVDPVDCMPLINE01>_".split("");
    const resize = () => {
...
//   typing   — `>FLOW` 한 글자씩 찍힘 (dot pixel, Claude Code 느낌)
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        setMsg("Registered! Waiting for admin approval."); setMode("login"); setP("");
      } else if (mode === "reset") {
        if (!u) { setMsg("Enter username first"); setLoading(false); return; }
        const r = await fetch("/api/auth/reset-request", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: u }) });
        const d = await r.json();
        setMsg(r.ok ? "Reset request sent to admin." : (d.detail || "Error"));
      }
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#050508", overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ position: "relative", zIndex: 4, display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 10, color: "#555", fontFamily: "'JetBrains Mono',monospace", marginBottom: 5, letterSpacing: 1.5, fontWeight: 600 }}>USERNAME</div>
          <input value={u} onChange={e => setU(e.target.value)} style={inputStyle} onFocus={onF} onBlur={onB} onKeyDown={e => e.key === "Enter" && submit()} autoComplete="username" />
