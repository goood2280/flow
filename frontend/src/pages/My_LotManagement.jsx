...
import My_SplitTable from "./My_SplitTable";
...
const inputStyle = {width:"100%",border:0,outline:0,background:"transparent",color:"#111827",fontSize:14,padding:"7px 8px",boxSizing:"border-box"};
...
function LotIdEditor({ value, candidates, onChange, onCommit, onPaste }) {
  ...
  return <div style={{position:"relative",minHeight:34,flex:1,minWidth:0}}>
    <input value={value || ""} onChange={e => {onChange(e.target.value);setOpen(true);}} onFocus={() => setOpen(true)} onBlur={() => {onCommit(value);setTimeout(() => setOpen(false), 120);}} onPaste={onPaste} autoComplete="off" style={{...inputStyle,minHeight:34}}/>
    {open&&<div style={{position:"absolute",top:"100%",left:0,right:0,zIndex:50,maxHeight:190,overflowY:"auto",border:"1px solid #9ca3af",borderRadius:"0 0 6px 6px",background:"#ffffff",boxShadow:"0 5px 14px rgba(0,0,0,.18)"}}>
      {filtered.length?filtered.map(lot => <div key={lot} onMouseDown={e => {e.preventDefault();onChange(lot);onCommit(lot);setOpen(false);}} title={lot} style={{height:31,boxSizing:"border-box",padding:"6px 9px",borderBottom:"1px solid #e5e7eb",fontSize:13,fontFamily:"monospace",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",cursor:"pointer",color:"#111827"}} onMouseEnter={e => e.currentTarget.style.background="#eff6ff"} onMouseLeave={e => e.currentTarget.style.background="#ffffff"}>{lot}</div>):<div style={{padding:"8px 9px",fontSize:12,color:"#6b7280"}}>일치하는 LOT_ID가 없습니다.</div>}
    </div>}
  </div>;
}
...
export default function My_LotManagement({ user }) {
  ...
  return <div className="flow-connected-page" style={{height:"calc(100vh - 52px)",display:"flex",background:"var(--bg-primary)",color:"var(--text-primary)"}}>
    <aside style={{width:230,borderRight:"1px solid var(--border)",background:"var(--bg-secondary)",display:"flex",flexDirection:"column",minWidth:0}}>
      <div style={{overflow:"auto",padding:"10px 8px 12px"}}>{products.map(item => <button key={item.name} onClick={() => setProduct(item.name)} style={{width:"100%",textAlign:"left",padding:"7px 9px",marginBottom:2,border:0,borderRadius:5,cursor:"pointer",background:product===item.name?"var(--accent-glow)":"transparent",color:product===item.name?"var(--accent)":"var(--text-secondary)",fontSize:13,fontWeight:product===item.name?800:500}}>{String(item.name || "").replace(/^ML_TABLE_/, "")}</button>)}</div>
    </aside>
    <main style={{flex:1,overflow:"auto",minWidth:0}}>
      <div style={{padding:"14px 18px",borderBottom:"1px solid var(--border)",background:"var(--bg-secondary)",display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",position:"sticky",top:0,zIndex:10}}>
        <strong style={{fontFamily:"monospace",color:"var(--accent)"}}>{String(product).replace(/^ML_TABLE_/, "")}</strong>
        <input type="search" value={purposeSearch} onChange={e => setPurposeSearch(e.target.value)} placeholder="purpose 검색 (예: CS)" style={{...buttonStyle,minWidth:210,fontWeight:500}} aria-label="purpose 검색" title="purpose에 입력한 문자가 포함된 LOT만 표시" autoComplete="off"/>
        <span style={{fontSize:12,color:"var(--text-secondary)",whiteSpace:"nowrap"}}>{visibleRows.length}건</span>
        <select value={selectedCustom} onChange={e => setSelectedCustom(e.target.value)} style={{...buttonStyle,minWidth:180}} title="LOT View에 적용할 SplitTable CUSTOM SET">
          <option value="">CUSTOM SET 선택</option>{customs.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
        </select>
        <button style={buttonStyle} onClick={openCustomEditor}>CUSTOM SET 편집</button>
        <span style={{marginLeft:"auto",fontSize:12,color:"var(--text-secondary)"}}>{work?.version ? `v${work.version} · ${work.updated_by || "-"}` : "아직 저장되지 않음"}</span>
        <button style={buttonStyle} onClick={loadVersions}>버전 기록</button>
        {editing ? <>
          <button style={{...buttonStyle,background:"var(--ok)",color:"white",borderColor:"var(--ok)"}} disabled={saving} onClick={save}>{saving ? "저장 중…" : "저장"}</button>
          <button style={buttonStyle} onClick={() => {setDraft(clone(table));setEditing(false);}}>취소</button>
        </> : <button style={{...buttonStyle,background:"var(--accent)",color:"white",borderColor:"var(--accent)"}} onClick={() => {setDraft(clone(table));setEditing(true);}}>편집</button>}
      </div>
      {loading || !work ? <Loading text="랏 관리 표 로딩..."/> : <div style={{padding:18}}>
        {viewLot&&<section ref={splitViewRef} style={{marginBottom:22,scrollMarginTop:62,border:"1px solid var(--border)",borderRadius:10,overflow:"hidden",background:"var(--bg-card)"}}><div style={{padding:"10px 14px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid var(--border)"}}><strong style={{color:"var(--accent)"}}>LOT {viewLot} SplitTable</strong><span style={{fontSize:12,color:"var(--text-secondary)"}}>{selectedCustom ? `CUSTOM: ${selectedCustom}` : "기본 KNOB"}</span><button onClick={() => setViewLot("")} style={{...buttonStyle,marginLeft:"auto"}}>닫기</button></div><My_SplitTable key={`${product}:${viewLot}:${selectedCustom}`} user={user} initialProduct={product} initialFabLotId={viewLot} initialCustomName={selectedCustom} embedded/></section>}
        <div style={{overflow:"auto",border:"1px solid var(--border)",borderRadius:8,background:"white"}}>
          <table style={{borderCollapse:"separate",borderSpacing:0,minWidth:"100%",tableLayout:"fixed",color:"#111827"}}>
            <thead><tr><th style={{width:42,background:"#f3f4f6",borderRight:"1px solid #d1d5db",borderBottom:"1px solid #d1d5db"}}>#</th>{work.columns.map(column => <Fragment key={column.id}><th onDoubleClick={() => editing && renameColumn(column)} style={{minWidth:column.id==="comment"?260:170,padding:"8px",background:"#f3f4f6",borderRight:"1px solid #d1d5db",borderBottom:"1px solid #d1d5db",fontSize:13,textAlign:"left",cursor:editing&&!DEFAULT_COLUMNS.some(c=>c.id===column.id)?"pointer":"default"}}>{column.label}{editing&&!DEFAULT_COLUMNS.some(c=>c.id===column.id)&&<button onClick={() => deleteColumn(column)} style={{float:"right",border:0,background:"transparent",color:"#ef4444",cursor:"pointer"}}>×</button>}</th>{column.id==="lot_id"&&<th title="SplitTable 보기" style={{width:60,background:"#f3f4f6",borderRight:"1px solid #d1d5db",borderBottom:"1px solid #d1d5db"}}>View</th>}</Fragment>)}{editing&&<th style={{width:42,background:"#f3f4f6",borderBottom:"1px solid #d1d5db"}}/>}</tr></thead>
            <tbody>{visibleRows.length ? visibleRows.map((row, ri) => <tr key={row.id}>
              <td style={{textAlign:"center",fontSize:12,color:"#6b7280",borderRight:"1px solid #d1d5db",borderBottom:"1px solid #d1d5db"}}>{ri+1}</td>
              {work.columns.map(column => {
                const key = `${row.id}:${column.id}`;
                const cellColor = work.colors[key] || "#ffffff";
                ...
                return <Fragment key={column.id}>
                  <td onContextMenu={editing&&colorable ? event => openCellColorPicker(event,row.id,column.id) : undefined} title={editing&&colorable ? "우클릭: 배경색 팔레트 열기" : undefined} style={{padding:0,background:cellColor,borderRight:"1px solid #d1d5db",borderBottom:"1px solid #d1d5db",verticalAlign:"top"}}>
                    {editing ? (computed
                      ? <div style={{padding:"8px",minHeight:34,boxSizing:"border-box",fontSize:14,fontFamily:"monospace",color:"#4b5563",background:"rgba(243,244,246,.42)"}}>{value}</div>
                      : <div style={{display:"flex",alignItems:"center",minHeight:34}}>{column.id==="lot_id"
                        ...
                        : <textarea value={value} onChange={e => updateCell(row.id,column.id,e.target.value)} onPaste={e => pasteBlock(e,row.id,column.id)} rows={1} style={{...inputStyle,resize:"vertical",minHeight:34,flex:1,minWidth:0}}/>}</div>)
                      : <div style={{padding:"8px",whiteSpace:"pre-wrap",wordBreak:"break-word",minHeight:34,fontSize:14}}>{value}</div>}
                  </td>
                  {column.id==="lot_id"&&<td style={{textAlign:"center",borderRight:"1px solid #d1d5db",borderBottom:"1px solid #d1d5db"}}><button disabled={!String(row.values?.lot_id || "").trim()} onClick={() => setViewLot(String(row.values?.lot_id || "").trim())} title="SplitTable 보기" aria-label="SplitTable 보기" style={{...buttonStyle,padding:"4px 9px",fontSize:13,lineHeight:1.1,opacity:String(row.values?.lot_id || "").trim()?1:.4}}>보기</button></td>}
                </Fragment>;
              })}
              {editing&&<td style={{textAlign:"center",borderBottom:"1px solid #d1d5db"}}><button onClick={() => deleteRow(row.id)} style={{border:0,background:"transparent",color:"#ef4444",cursor:"pointer",fontSize:17}}>×</button></td>}
            </tr>) : <tr><td colSpan={tableColumnCount} style={{padding:28,textAlign:"center",fontSize:13,color:"#6b7280",borderBottom:"1px solid #d1d5db"}}>검색한 purpose에 해당하는 LOT ID가 없습니다.</td></tr>}</tbody>
          </table>
        </div>
        {editing&&<div style={{display:"flex",gap:8,marginTop:10}}><button style={buttonStyle} onClick={addRow}>＋ 행 추가</button><button style={buttonStyle} onClick={addColumn}>＋ 열 추가</button><span style={{fontSize:12,color:"var(--text-secondary)",alignSelf:"center"}}>purpose 또는 lot_id 셀을 우클릭해 배경색을 선택하세요.</span></div>}
      </div>}
    </main>
