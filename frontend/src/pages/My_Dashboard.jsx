...
if(typeof document!=="undefined"&&!document.getElementById("dash-styles")){
  ...
  s.textContent=`.chart-card .chart-actions{opacity:0;transition:opacity 0.15s}.chart-card:hover .chart-actions{opacity:1}`;
...
function ChartCanvas({ cfg, points, computedAt }) {
  ...
  if (!points?.length) return <div style={{ padding: 40, textAlign: "center", color: "var(--text-secondary)", fontSize: 12 }}>데이터 없음</div>;
  const type = cfg.chart_type || "scatter";
  ...
  const Tip = tip ? <div style={{ position: "absolute", left: tip.x + 12, top: tip.y - 10, background: "#111", border: "1px solid var(--border)", borderRadius: 6, padding: "6px 10px", fontSize: 11, color: "#e5e5e5", pointerEvents: "none", zIndex: 10, maxWidth: 280, whiteSpace: "pre-wrap", lineHeight: 1.5, boxShadow: "0 4px 12px rgba(0,0,0,0.5)" }}>
    {tip.lines.map((l, i) => <div key={i}>{l}</div>)}
  </div> : null;

  const Header = <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", margin: "-12px -14px 8px", borderRadius: "8px 8px 0 0", background: "rgba(249,115,22,0.06)", borderBottom: "1px solid rgba(249,115,22,0.12)" }}>
    {title && <div style={{ fontSize: 12, fontWeight: 700, fontFamily: "monospace", color: "var(--accent)" }}>{title}</div>}
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      {cfg._oos > 0 && <span style={{ fontSize: 9, fontWeight: 700, color: "#fff", background: "#ef4444", borderRadius: 4, padding: "1px 5px" }}>OOS: {cfg._oos}</span>}
  ...
  if (type === "pie") {
    ...
    const slices = points.map((p, i) => {
      ...
      return { ...p, frac, a0, a1, lx: cx + R * 0.65 * Math.sin(mid), ly: cy - R * 0.65 * Math.cos(mid),
        ...
        color: PASTEL[i % PASTEL.length], i };
    });
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        <svg width={280} height={280} viewBox="0 0 280 280" ref={svgRef}
          ...
          {slices.map(s => (<g key={s.i}>
            ...
            {s.frac > 0.04 && <text x={s.lx} y={s.ly} textAnchor="middle" dominantBaseline="middle" fill="#fff" fontSize={s.frac > 0.1 ? 12 : 10} fontWeight={700} style={{ textShadow: "0 1px 3px rgba(0,0,0,0.6)", pointerEvents: "none" }}>{(s.frac * 100).toFixed(1)}%</text>}
          </g>))}
        </svg>
    ...
    return (<div style={{ overflow: "hidden", background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: "14px 16px", position: "relative" }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" ref={svgRef} onMouseLeave={() => setTip(null)} style={{ display: "block" }}>
        ...
        {points.map((p, i) => {
          ...
          return (<g key={i}>
            <rect x={x} y={pad.t + ch - barH} width={bw} height={Math.max(1, barH)} fill={COLORS[i % COLORS.length]} rx={2} opacity={0.85}
              ...
              style={{ cursor: "pointer" }} />
            {barH > 16 && <text x={x + bw / 2} y={pad.t + ch - barH + 13} textAnchor="middle" fill="#fff" fontSize={9} fontWeight={600} style={{ pointerEvents: "none" }}>{p.y}</text>}
  ...
  if (type === "donut") {
    ...
    const slices = points.map((p, i) => {
      const frac = p.y / total; const a0 = acc * 2 * Math.PI; acc += frac; const a1 = acc * 2 * Math.PI;
      const mid = (a0 + a1) / 2;
      return { ...p, frac, color: PASTEL[i % PASTEL.length], i,
    ...
    return (<div style={{ overflow: "hidden", background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: "12px 14px", position: "relative" }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" ref={svgRef} onMouseLeave={() => setTip(null)} style={{ display: "block" }}>
        ...
        {points.map((p, i) => {
          ...
          return (<g key={i}>
            <rect x={x} y={pad.t + ch - barH} width={bw} height={Math.max(1, barH)} fill={COLORS[i % COLORS.length]} rx={2} opacity={0.8}
            ...
            <circle cx={x + bw / 2} cy={lineY} r={3} fill="#ef4444" />
            {i > 0 && <line x1={pad.l + (i - 1) * (cw / points.length) + (cw / points.length) / 2} y1={pad.t + ch * (1 - (cum - p.y) / total)} x2={x + bw / 2} y2={lineY} stroke="#ef4444" strokeWidth={1.5} />}
        ...
        {/* Right axis: cum% */}
        {[0, 50, 80, 100].map(pct => <text key={pct} x={W - pad.r + 4} y={pad.t + ch * (1 - pct / 100) + 3} fill="#ef4444" fontSize={8}>{pct}%</text>)}
      </svg>
    </div>);
  }
    ...
    return (<div style={{ overflow: "hidden", background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: "12px 14px", position: "relative" }}>
      ...
      <svg width={W} height={H} ref={svgRef} onMouseLeave={() => setTip(null)} style={{ display: "block" }}>
        ...
        {points.map((p, i) => {
          ...
          return (<g key={i}
            ...
            style={{ cursor: "pointer" }}>
            <line x1={cx} y1={toY(p.min)} x2={cx} y2={toY(p.q1)} stroke={COLORS[i % COLORS.length]} strokeWidth={1} />
            <line x1={cx} y1={toY(p.q3)} x2={cx} y2={toY(p.max)} stroke={COLORS[i % COLORS.length]} strokeWidth={1} />
            <line x1={cx - bw / 3} y1={toY(p.min)} x2={cx + bw / 3} y2={toY(p.min)} stroke={COLORS[i % COLORS.length]} strokeWidth={1.5} />
            <line x1={cx - bw / 3} y1={toY(p.max)} x2={cx + bw / 3} y2={toY(p.max)} stroke={COLORS[i % COLORS.length]} strokeWidth={1.5} />
            <rect x={cx - bw / 2} y={toY(p.q3)} width={bw} height={Math.max(1, toY(p.q1) - toY(p.q3))} fill={COLORS[i % COLORS.length] + "44"} stroke={COLORS[i % COLORS.length]} strokeWidth={1.5} rx={2} />
            <line x1={cx - bw / 2} y1={toY(p.median)} x2={cx + bw / 2} y2={toY(p.median)} stroke="#fff" strokeWidth={2.5} />
            <circle cx={cx} cy={toY(p.mean)} r={3} fill="#fff" stroke={COLORS[i % COLORS.length]} strokeWidth={1} />
            <text x={cx} y={H - 4} textAnchor="middle" fill={COLORS[i % COLORS.length]} fontSize={10} fontWeight={600}>{(p.x || "").slice(0, 10)}</text>
          </g>);
        })}
      ...
      <table style={{ borderCollapse: "collapse", tableLayout: "fixed", width: W, border: "1px solid var(--border)" }}>
        ...
        <thead><tr>
          ...
          {points.map((p, i) => <th key={i} style={{ ...tS, background: "var(--bg-tertiary)", fontWeight: 700, color: COLORS[i % COLORS.length], fontSize: 10 }}>{p.x}</th>)}
  ...
  if (type === "treemap") {
    const total = points.reduce((s, p) => s + p.y, 0) || 1;
    const H = 220;
    const sorted = [...points].sort((a, b) => b.y - a.y).map((p, i) => ({ ...p, pct: p.y / total * 100, color: PASTEL[i % PASTEL.length], i }));
      ...
      <div ref={svgRef} onMouseLeave={() => setTip(null)} style={{ display: "flex", height: H, borderRadius: 6, overflow: "hidden", gap: 2 }}>
        {sorted.map((r, i) => (
          <div key={i} style={{ flex: `${r.pct} 0 0%`, minWidth: 2, height: "100%", background: r.color, borderRadius: 4, opacity: 0.88, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", position: "relative" }}
            ...
            {r.pct > 8 && <div style={{ color: "#fff", fontSize: Math.min(13, Math.max(9, r.pct / 3)), fontWeight: 700, textShadow: "0 1px 3px rgba(0,0,0,0.6)", pointerEvents: "none", textAlign: "center", padding: "0 4px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "100%" }}>{(r.label || "").slice(0, 12)}</div>}
  ...
  /* ── Wafer Map (circular grid heatmap) v6: clipPath + enhanced ── */
  if (type === "wafer_map") {
    ...
    let colorFn;
    if (isNumeric) {
      const numVals = vals.map(Number); const vMin = Math.min(...numVals), vMax = Math.max(...numVals), vR = vMax - vMin || 1;
      ...
      colorFn = (v) => COLORS[uniq.indexOf(v) % COLORS.length];
    }
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 6, fontSize: 9, color: "var(--text-secondary)" }}>
        {isNumeric ? <>
          ...
          {[...new Set(vals)].map((v, i) => <span key={i} style={{ display: "flex", alignItems: "center", gap: 3 }}>
            <span style={{ width: 8, height: 8, borderRadius: 2, background: COLORS[i % COLORS.length] }} />{String(v)}
          </span>)}
        </div>}
      </div>
    </div>);
  ...
  if (type === "combo" && points.length > 0) {
    ...
    return (<div style={{ overflow: "hidden", background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: "12px 14px", position: "relative" }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" ref={svgRef} onMouseLeave={() => setTip(null)} style={{ display: "block" }}>
        ...
        {/* Bars */}
        {points.map((p, i) => p.bar != null && <rect key={"b" + i} x={toX(i) - bw / 2} y={toY(p.bar)} width={bw} height={Math.max(1, pad.t + ch - toY(p.bar))} fill={COLORS[0]} rx={2} opacity={0.7}
          ...
          style={{ cursor: "pointer" }} />)}
        {/* Line overlay */}
        {hasLine && <polyline points={points.filter(p => p.line != null).map((p, i) => `${toX(points.indexOf(p))},${toY(p.line)}`).join(" ")} fill="none" stroke="#ef4444" strokeWidth={2} />}
        {hasLine && points.filter(p => p.line != null).map((p, i) => <circle key={"l" + i} cx={toX(points.indexOf(p))} cy={toY(p.line)} r={3} fill="#ef4444" stroke="#fff" strokeWidth={1} />)}
        {/* X labels */}
        ...
        {/* Legend */}
        <rect x={W - pad.r - 80} y={pad.t} width={8} height={8} fill={COLORS[0]} rx={2} />
        <text x={W - pad.r - 68} y={pad.t + 8} fill="var(--text-secondary)" fontSize={9}>{cfg.y_expr || "Bar"}</text>
        {hasLine && <><rect x={W - pad.r - 80} y={pad.t + 14} width={8} height={8} fill="#ef4444" rx={2} />
  ...
  if (fitEnabled && points.length > 2) {
    ...
    // Solve (X^T X) β = X^T y via Gauss elimination
  ...
  return (<div style={{ overflow: "hidden", background: "var(--bg-card)", borderRadius: 8, border: "1px solid var(--border)", padding: "14px 16px", position: "relative" }}>
    ...
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" ref={svgRef} onMouseLeave={() => setTip(null)} style={{ display: "block" }}>
      ...
      {cfg.usl != null && <g>
        <line x1={pad.l} y1={toY(cfg.usl)} x2={W - pad.r} y2={toY(cfg.usl)} stroke="#ef4444" strokeWidth={2} strokeDasharray="6,3" opacity={0.8} />
        <rect x={W - pad.r - 62} y={toY(cfg.usl) - 14} width={58} height={15} rx={3} fill="rgba(239,68,68,0.15)" />
        <text x={W - pad.r - 6} y={toY(cfg.usl) - 3} textAnchor="end" fill="#ef4444" fontSize={10} fontWeight={600}>USL {fmt(cfg.usl)}</text>
      </g>}
      {cfg.lsl != null && <g>
        <line x1={pad.l} y1={toY(cfg.lsl)} x2={W - pad.r} y2={toY(cfg.lsl)} stroke="#ef4444" strokeWidth={2} strokeDasharray="6,3" opacity={0.8} />
        <rect x={W - pad.r - 62} y={toY(cfg.lsl) + 2} width={58} height={15} rx={3} fill="rgba(239,68,68,0.15)" />
        <text x={W - pad.r - 6} y={toY(cfg.lsl) + 13} textAnchor="end" fill="#ef4444" fontSize={10} fontWeight={600}>LSL {fmt(cfg.lsl)}</text>
      </g>}
      {cfg.target != null && <g>
        <line x1={pad.l} y1={toY(cfg.target)} x2={W - pad.r} y2={toY(cfg.target)} stroke="#10b981" strokeWidth={2} opacity={0.9} />
        <rect x={W - pad.r - 72} y={toY(cfg.target) - 14} width={68} height={15} rx={3} fill="rgba(16,185,129,0.15)" />
        <text x={W - pad.r - 6} y={toY(cfg.target) - 3} textAnchor="end" fill="#10b981" fontSize={10} fontWeight={600}>Target {fmt(cfg.target)}</text>
      </g>}
      {/* v7: Multi spec_lines[] */}
      {(cfg.spec_lines || []).map((sl, i) => {
        const v = Number(sl.value); if (isNaN(v)) return null;
        const k = (sl.kind || "custom").toLowerCase();
        const color = sl.color || (k === "usl" || k === "lsl" ? "#ef4444" : k === "target" ? "#10b981" : "#8b5cf6");
      ...
      {cfg._spc && <g>
        <line x1={pad.l} y1={toY(cfg._spc.ucl)} x2={W - pad.r} y2={toY(cfg._spc.ucl)} stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="4,4" opacity={0.8} />
        <text x={pad.l + 4} y={toY(cfg._spc.ucl) - 3} fill="#f59e0b" fontSize={9} fontWeight={600}>UCL {fmt(cfg._spc.ucl)}</text>
        <line x1={pad.l} y1={toY(cfg._spc.lcl)} x2={W - pad.r} y2={toY(cfg._spc.lcl)} stroke="#f59e0b" strokeWidth={1.5} strokeDasharray="4,4" opacity={0.8} />
        <text x={pad.l + 4} y={toY(cfg._spc.lcl) + 12} fill="#f59e0b" fontSize={9} fontWeight={600}>LCL {fmt(cfg._spc.lcl)}</text>
        <line x1={pad.l} y1={toY(cfg._spc.cl)} x2={W - pad.r} y2={toY(cfg._spc.cl)} stroke="#3b82f6" strokeWidth={1.5} opacity={0.8} />
        <text x={pad.l + 4} y={toY(cfg._spc.cl) - 3} fill="#3b82f6" fontSize={9} fontWeight={600}>CL {fmt(cfg._spc.cl)}</text>
      </g>}
      ...
      {type === "bar" ? points.map((p, i) => {
        ...
        hasColor ? groupNames.map((gn, gi) => {
          const gp = colorGroups[gn]; const ci = COLORS[gi % COLORS.length];
          ...
          <polyline points={points.map((p, i) => `${toX(i)},${toY(p.y)}`).join(" ")} fill="none" stroke={COLORS[0]} strokeWidth={1.5} />
          {points.map((p, i) => <circle key={i} cx={toX(i)} cy={toY(p.y)} r={ptSize} fill={COLORS[0]}
        ...
        <polyline points={points.map((p, i) => `${toX(i)},${toY(p.y)}`).join(" ")} fill="none" stroke={COLORS[0]} strokeWidth={2} />
        {points.map((p, i) => <circle key={i} cx={toX(i)} cy={toY(p.y)} r={2} fill={COLORS[0]}
          onMouseMove={e => { const r = svgRef.current.getBoundingClientRect(); setTip({ x: e.clientX - r.left, y: e.clientY - r.top, lines: [`X: ${p.x}`, `Y: ${fmt(p.y)}`] }); }}
          style={{ cursor: "pointer" }} />)}
      </g>
      : /* scatter */
        ...
        {points.map((p, i) => {
          ...
          const fill = isOOS ? "#ef4444" : COLORS[ci % COLORS.length];
          const op = isDimmed ? 0.15 : (isMarked ? 1.0 : (isOOS ? 0.95 : ptOpacity));
          const stroke = isMarked ? "#fbbf24" : (isOOS ? "#fff" : "rgba(0,0,0,0.3)");
      ...
      {hasColor && groupNames.map((gn, gi) => (
        <g key={gn} transform={`translate(${W - pad.r + 6},${pad.t + gi * 15})`}>
          <rect width={8} height={8} rx={2} fill={COLORS[gi % COLORS.length]} />
...
function ChartEditor({ cfg, onSave, onClose, isAdmin }) {
  ...
  return (<div style={{ background: "var(--bg-secondary)", borderRadius: 10, border: "1px solid var(--border)", padding: 20, maxWidth: 660, marginBottom: 16 }}>
      ...
      {(form.joins || []).map((j, i) => {
        ...
        return (<div key={i} style={{ marginBottom: 8, padding: 8, background: "var(--bg-secondary)", borderRadius: 5, border: "1px solid var(--border)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            ...
            <span onClick={() => u("joins", (form.joins || []).filter((_, k) => k !== i))} style={{ cursor: "pointer", color: "#ef4444", fontSize: 11 }}>✕ 제거</span>
          </div>
    ...
    {!isPieOrBin && !isWaferMap && <div style={{ display: "flex", gap: 8, marginBottom: 8, padding: "8px 10px", background: "rgba(239,68,68,0.04)", borderRadius: 6, border: "1px solid rgba(239,68,68,0.1)" }}>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 10, color: "#ef4444", marginBottom: 2, fontWeight: 600 }}>USL</div>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 10, color: "#10b981", marginBottom: 2, fontWeight: 600 }}>Target</div>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 10, color: "#ef4444", marginBottom: 2, fontWeight: 600 }}>LSL</div>
      ...
      <div style={{ flex: "0 0 auto", display: "flex", alignItems: "flex-end", paddingBottom: 2 }}>
        <label style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, color: "#f59e0b", whiteSpace: "nowrap" }}>
          <input type="checkbox" checked={form.enable_spc || false} onChange={e => u("enable_spc", e.target.checked)} style={{ accentColor: "#f59e0b" }} />SPC
        </label>
      </div>
    </div>}
    ...
    {!isPieOrBin && !isWaferMap && <div style={{ marginBottom: 8, padding: "8px 10px", background: "rgba(139,92,246,0.04)", borderRadius: 6, border: "1px solid rgba(139,92,246,0.15)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 10, color: "#8b5cf6", fontWeight: 700 }}>추가 Spec Line ({(form.spec_lines || []).length})</span>
        <span onClick={() => u("spec_lines", [...(form.spec_lines || []), { name: "", value: 0, kind: "custom", color: "#8b5cf6", style: "dashed" }])}
              style={{ cursor: "pointer", color: "#8b5cf6", fontSize: 11, fontWeight: 600 }}>+ 추가</span>
      </div>
      {(form.spec_lines || []).map((sl, i) => (
        <div key={i} style={{ display: "flex", gap: 6, marginBottom: 4 }}>
          ...
          <input type="color" value={sl.color || "#8b5cf6"} onChange={e => { const arr = [...form.spec_lines]; arr[i] = { ...sl, color: e.target.value }; u("spec_lines", arr); }} style={{ width: 32, height: 24, border: "none", background: "transparent", cursor: "pointer" }} />
          ...
          <span onClick={() => u("spec_lines", form.spec_lines.filter((_, j) => j !== i))} style={{ cursor: "pointer", color: "#ef4444", fontSize: 14, padding: "2px 6px" }}>✕</span>
        </div>
      ))}
    </div>}
    ...
    <div style={{ marginBottom: 12 }}>
      <button onClick={runPreview} disabled={prevLoading} style={{ padding: "6px 16px", borderRadius: 5, border: "1px solid #3b82f6", background: "#3b82f611", color: "#3b82f6", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
      ...
      {preview?.error && <div style={{ marginTop: 6, fontSize: 11, color: "#ef4444" }}>{preview.error}</div>}
    </div>
    <div style={{ display: "flex", gap: 8 }}>
      <button onClick={doSave} style={{ flex: 1, padding: 8, borderRadius: 6, border: "none", background: "var(--accent)", color: "#fff", fontWeight: 600, cursor: "pointer" }}>저장</button>
...
export default function My_Dashboard({ user }) {
  ...
  useEffect(() => {
    sf("/api/admin/settings").then(s => { if (s && typeof s.dashboard_refresh_minutes === "number") setRefreshMin(s.dashboard_refresh_minutes); }).catch(() => {});
  }, []);
      ...
      {visibleCharts.map(c => { const snap = snapshots[c.id]; const isAdminChart = c.visible_to === "admin";
        ...
        <div style={{ fontSize: 10, color: "var(--text-secondary)", padding: "4px 12px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ display: "flex", gap: 4, alignItems: "center" }}>
            ...
            {isAdmin && (isAdminChart
              ? <span style={{ fontSize: 8, fontWeight: 700, color: "#a855f7", background: "#a855f711", padding: "1px 5px", borderRadius: 3, border: "1px solid #a855f733" }}>관리자</span>
        ...
        <div className="chart-actions" style={{ position: "absolute", top: 8, right: 8, display: "flex", gap: 4, alignItems: "center" }}>
          ...
          {canEdit && <>
            ...
            <span onClick={(e) => { e.stopPropagation(); sf(API + "/charts/copy?chart_id=" + c.id, { method: "POST" }).then(load); }} style={{ cursor: "pointer", fontSize: 11, color: "#fff", padding: "3px 8px", background: "#8b5cf6", borderRadius: 4, fontWeight: 600, boxShadow: "0 1px 4px rgba(0,0,0,0.3)" }}>복사</span>
            {isAdmin && <span onClick={(e) => { e.stopPropagation(); deleteChart(c.id); }} style={{ cursor: "pointer", fontSize: 11, color: "#fff", padding: "3px 8px", background: "#ef4444", borderRadius: 4, fontWeight: 600, boxShadow: "0 1px 4px rgba(0,0,0,0.3)" }}>삭제</span>}
          </>}
        </div>
      </div>); })}
    </div>
