...
// maxZoom — 화면마다 필요한 배율이 다르다. TEG Mapfile 체크의 shot 확대는 TEG 하나가
// shot 대비 아주 작아 기본 12배로는 이름이 안 읽힌다 (사용자 요청으로 더 크게 연다).
export default function ZoomPanSvg({ size = 380, maxZoom = ZOOM_MAX, style = {}, children }) {
  const ZMAX = Math.max(ZOOM_MIN, maxZoom);
  ...
  const onPointerMove = useCallback((e) => {
    ...
    if (pointersRef.current.size === 2 && pinchRef.current) {
      ...
      if (pinchRef.current.dist0 > 0) {
        setZoom(Math.max(ZOOM_MIN, Math.min(ZMAX, pinchRef.current.zoom0 * (d / pinchRef.current.dist0))));
      }
