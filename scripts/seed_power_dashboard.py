"""scripts/seed_power_dashboard.py — 파워유저용 데모 대시보드 차트 12개 시드.
...
def mk(title, *, chart_type, x_col="", y_expr="", color_col="", agg_col="", agg_method="",
       ...
       selection_key="LOT_WF", extra=None):
    c = {
...
# 12 연결된 차트 — 위에서 아래로 스토리라인.
CHARTS = [
    # [종합] 상단 — 큰 요약. width 3 (L).
    mk("① 제품별 로트 수 (전체 분포)", chart_type="pie",
       x_col="product", agg_col="wafer_id", agg_method="count",
       group="종합", width=2, height=1),
    mk("② root_lot_id × wafer_id × Vth_N (Wafer Map 대체 — Heatmap)",
       chart_type="heatmap", x_col="root_lot_id", y_expr="wafer_id",
       agg_col="VM_PREDICTED_VTH_N", agg_method="mean",
       group="종합", width=2, height=1, sort_x=True),

    # [KNOB 영향] — 레시피(PPID) 별로 결과 지표 차이.
    ...
    mk("④ KNOB_ETCH_PPID 별 Predicted Vth_N (색: KNOB_GATE_PPID)",
       chart_type="scatter", x_col="KNOB_ETCH_PPID", y_expr="VM_PREDICTED_VTH_N",
       color_col="KNOB_GATE_PPID",
       group="KNOB 영향", width=2, height=1),
    mk("⑤ KNOB_CVD_PPID 사용 빈도 Pareto",
       chart_type="pareto", x_col="KNOB_CVD_PPID",
       agg_col="wafer_id", agg_method="count",
       group="KNOB 영향", width=2, height=1, sort_x=True),

    # [INLINE 분포] — INLINE 측정값들의 분포 + 관계.
    mk("⑥ INLINE_CD_GATE_MEAN 히스토그램 (30 bins)",
       chart_type="binning", x_col="INLINE_CD_GATE_MEAN",
       group="INLINE 분포", width=2, height=1,
       extra={"bin_count": 30}),
    mk("⑦ INLINE_OVL_X vs INLINE_OVL_Y (오버레이 산점도, 색: product)",
       chart_type="scatter", x_col="INLINE_OVL_X", y_expr="INLINE_OVL_Y",
       color_col="product",
       group="INLINE 분포", width=2, height=1),

    # [VM vs 실측] — VM 예측과 INLINE 실측 비교.
    mk("⑧ VM_PREDICTED_VTH_N vs INLINE_CD_GATE_MEAN (색: FAB_EQP_GATE)",
    ...
    mk("⑨ VM_CHAMBER_DRIFT_GATE 분포 (색: FAB_EQP_GATE)",
       chart_type="box", x_col="FAB_EQP_GATE", y_expr="VM_CHAMBER_DRIFT_GATE",
       group="VM vs 실측", width=1, height=1),

    # [FAB 장비] — 장비별 영향.
    mk("⑩ FAB_EQP_ETCH 별 wafer 수 (Treemap)",
       chart_type="treemap", x_col="FAB_EQP_ETCH",
       agg_col="wafer_id", agg_method="count",
       group="FAB 장비", width=2, height=1),
    mk("⑪ FAB_EQP_GATE × FAB_EQP_ETCH 크로스 집계 (Vth_N 평균)",
