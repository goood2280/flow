...
Writes to the DB root currently shown by File Browser. It does not create a
separate hidden DB root.

Run: python3 scripts/seed_v73_matching.py
"""
import csv, json, sys
...
MATCH = PATHS.db_root / "matching"
PCDIR = PATHS.data_root / "product_config"
...
item_map_rows = []
inline_items = [
    ("CD_GATE",  "CD",  1, "INLINE_METRO"),
    ("CD_SPACE", "CD",  2, "INLINE_METRO"),
    ("THK_OX",   "THK", 1, "INLINE_METRO"),
    ("OVL_X",    "OVL", 1, "INLINE_METRO"),
]
for prod in ["PRODUCT_A", "PRODUCT_B", "PRODA", "PRODB"]:
    for canon_item, family, map_idx, process_id in inline_items:
        raw_item = canon_item
...
write_csv(MATCH / "inline_item_map.csv", item_map_rows,
          ["product", "process_id", "item_id", "canonical_item", "map_id"])
...
map_positions = {
    ...
    "MAP_CD_2":   [(-2, -1), (-1, 1), (0, 0), (1, -1), (2, 1), (-1, -2), (1, 2)],
    "MAP_THK_1":  [(0, -2), (0, -1), (0, 0), (0, 1), (0, 2)],
    "MAP_OVL_1":  [(-2, -2), (-2, 0), (-2, 2), (0, -2), (0, 0), (0, 2), (2, -2), (2, 0), (2, 2)],
}
for map_id, positions in map_positions.items():
    for i, (x, y) in enumerate(positions, 1):
        if map_id == "MAP_THK_1":
            sid = ["C", "N", "E", "S", "W"][i - 1]
        elif map_id == "MAP_OVL_1":
            sid = f"P{i:02d}"
        else:
            sid = f"S{i:02d}"
        subitem_rows.append({"map_id": map_id, "subitem_id": sid, "shot_x": x, "shot_y": y})
...
for prod, proc_id, owner in [
    ...
    cfg.update({
        ...
        "canonical_inline_items": ["CD_GATE", "CD_SPACE", "THK_OX", "OVL_X"],
