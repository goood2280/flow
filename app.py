...
`No module named 'routers'` 에러가 발생할 수 있다. 이 shim 은:
  ...
  1. 이 파일이 위치한 디렉토리(=FabCanvas.ai/) 와 그 하위 backend/ 를 sys.path 앞에
     추가한다 → 어디서 호출하든 `routers.*` import 가 안전하다.
  2. backend.app 의 `app` 객체를 그대로 re-export 한다.

이렇게 하면:
  - cd FabCanvas.ai && uvicorn app:app  ✓
  - python -m uvicorn app:app           ✓ (FabCanvas.ai 가 cwd)
...
try:
    ...
    spec = importlib.util.spec_from_file_location("_fabcanvas_backend_app", _BACKEND / "app.py")
    if spec is None or spec.loader is None:
        raise ImportError("Cannot locate backend/app.py spec")
    _mod = importlib.util.module_from_spec(spec)
    sys.modules["_fabcanvas_backend_app"] = _mod
