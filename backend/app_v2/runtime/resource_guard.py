...
DEFAULT_SELF_GATED_PATHS = ("/api/splittable/view", "/api/filebrowser/view")
...
DEFAULT_LIGHT_PATHS = (
    # Small authenticated write; it must not wait behind unrelated scans in
    # the shared heavy-request lane (whose queue timeout is 120 seconds).
    "/api/tracker/create",
    ...
    # Must bypass the SQL lane so leaving a page can remove queued work and
    # interrupt a running DuckDB scan immediately.
    "/api/filebrowser/view/cancel",
...
class ResourceGuardMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self._is_essential_request(request):
            split_priority = _request_priority.begin_splittable_request(path)
            try:
                ...
                _request_priority.end_splittable_request(split_priority)
