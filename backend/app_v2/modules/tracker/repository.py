...
class TrackerIssueRepository:
    ...
    def __init__(self, path: Path):
        # Tracker rewrites the complete issue array atomically. Compact JSON
        # materially shortens the user-facing create/update transaction once
        # measurement histories have grown large.
        self.store = JsonFileStore(path, default=list, indent=None)
    ...
    def create_issue_dict(self, issue: dict) -> dict:
        ...
        def updater(current):
            rows = current if isinstance(current, list) else []
            request_id = str(issue.get("client_request_id") or "").strip()
            username = str(issue.get("username") or "").strip()
            if request_id:
                existing = next((
                    row for row in rows
                    if str(row.get("client_request_id") or "").strip() == request_id
                    and str(row.get("username") or "").strip() == username
                ), None)
