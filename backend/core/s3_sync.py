...
Config lives at `data/holweb-data/s3_sync.json`:
  {
    "bucket": "my-bucket",
    "prefix": "holweb/artifacts/",
    "region": "ap-northeast-2",
    "enabled": true
  }

Status (append-only log) at `data/holweb-data/s3_sync_status.jsonl`.
"""
...
logger = logging.getLogger("holweb.s3_sync")
...
def list_artifacts(data_root: Path, db_root: Path) -> List[Dict[str, Any]]:
    ...
    Types:
      reformatter       data/holweb-data/reformatter/*.json
      matching          data/DB/matching/*.csv
      product_config    data/holweb-data/product_config/*.yaml
    """
    out = []
    # reformatter
