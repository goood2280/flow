...
순환 구조 (Valve backend/core/alert_store.py 와 짝):
  1. Valve 파이프라인 실행 → 알람 발행 → S3 `valve-alerts/pipeline/{vehicle}.json`
     ...
     · stage_stall    : 제품별 raw/event/feature/wide 가 임계(기본 1일)를 넘게 안 늚
                        (stall|{vehicle}|{stage}|{source}) — payload 의 `health` 블록에
                        정체가 아닌 단계까지 포함된 전체 현황이 같이 온다.
                        여기서 고칠 수 있는 건이 아니라 Valve 파이프라인 점검 신호다
                        (Valve backend/core/stage_health.py, payload schema 3 이상).
...
DEFAULT_CFG = {
    ...
    # 알람 파일을 폴더에서 직접 읽는 모드의 루트 = S3 버킷 루트에 대응.
    #   · 사내: Valve 가 알람을 떨어뜨리는 공유 폴더 (보통 flow DB 루트 → `{db_root}`)
    #   · 로컬 데모: Valve fake_local 버킷 폴더
    # `{db_root}` / `{data_root}` / `{app_root}` 토큰을 쓸 수 있다 (_resolve_root).
    # 비어있으면 S3 모드.
    "local_root": "",
    ...
    # 미매칭 step 의 function step 추천 (core/valve_step_advisor.py)
    "recommend": {"enabled": True, "lookback_days": 14, "neighbors": 3,
                  "max_alerts_per_run": 10, "use_llm": True},
}
...
def load_cfg() -> dict:
    ...
    out.update({k: v for k, v in cfg.items()
                if k in DEFAULT_CFG and k not in ("s3", "recommend")})
    s3 = dict(DEFAULT_CFG["s3"])
    if isinstance(cfg.get("s3"), dict):
        s3.update({k: str(v or "") for k, v in cfg["s3"].items() if k in s3})
    out["s3"] = s3
    rec = dict(DEFAULT_CFG["recommend"])
    if isinstance(cfg.get("recommend"), dict):
        rec.update({k: v for k, v in cfg["recommend"].items() if k in rec})
    out["recommend"] = rec
    return out
...
def save_cfg(patch: dict) -> dict:
    ...
    if isinstance(patch.get("recommend"), dict):
        for k, default in DEFAULT_CFG["recommend"].items():
            if k not in patch["recommend"]:
                continue
            v = patch["recommend"][k]
            cur["recommend"][k] = bool(v) if isinstance(default, bool) else int(v or 0)
...
# ────────────────────────────────────────── 전송 계층 (local 폴더 ↔ S3)
_ROOT_TOKENS = ("db_root", "data_root", "app_root")


def resolve_root(raw: str) -> str:
    """local_root 의 `{db_root}` 류 토큰을 실제 경로로 치환한다.

    설정 파일에 드라이브 절대경로를 박아두면 사내(공유 DB)로 옮길 때마다 깨진다.
    Valve 가 알람 JSON 을 flow DB 폴더에 떨어뜨리는 구성이면 `{db_root}` 한 줄이면
    되고, 해석은 항상 FLOW_DB_ROOT 체인(core.roots)을 거친다.
    """
    s = str(raw or "").strip()
    ...
    for token in _ROOT_TOKENS:
        if "{" + token + "}" in s:
            s = s.replace("{" + token + "}", str(getattr(PATHS, token)))
    return s
...
class _Store:
    ...
    def __init__(self, cfg: dict):
        self.cfg = cfg
        self.local_root_raw = str(cfg.get("local_root") or "").strip()
        self.local_root = resolve_root(self.local_root_raw)
    ...
    def available(self) -> tuple[bool, str]:
        if self.local_root:
            ...
            shown = (f"{self.local_root_raw} → {self.local_root}"
                     if self.local_root_raw != self.local_root else self.local_root)
            return (p.is_dir(), f"local_root={shown}")
        try:
            s3cfg = self._s3_cfg()
            if not s3cfg["bucket"]:
...
def list_alerts() -> dict:
    ...
    vehicles: list[dict] = []
    alert_cols: list[str] = []   # Valve ⚙ 로 설정된 추가 전송 열 (vehicle 간 합집합)
    for key in store.list_keys(prefix):
        ...
        try:
            ...
            continue
        # health = Valve 단계별 진행 현황(raw→event→feature→wide). 정체 알람이
        # 없어도 항상 실린다 — "언제까지의 데이터를 받았는지"는 알람 유무와 별개로
        # 봐야 한다. schema 3 미만(구 Valve)이면 없으므로 화면이 알아서 감춘다.
        ...
        # 전 제품 합산 산출물(SEND_FORM prefix 분리)의 정체 알람은 제품별 파일마다
        # 같은 id 로 들어 있다 — 파일 하나만 봐도 상태를 알 수 있어야 하기 때문이다.
        # 목록에서는 한 번만 보여준다.
        for a in (payload.get("alerts") or []):
            ...
            if aid:
                seen_ids.add(aid)
            alerts.append(a)
        for c in (payload.get("alert_cols") or []):
            if isinstance(c, str) and c and c not in alert_cols:
                alert_cols.append(c)
    ...
    for a in alerts:
        ...
        if a.get("type") == "unmatched_step":
            a["recommendation"] = recs.get(
                f"{a.get('vehicle') or ''}|{a.get('step_id') or ''}")
    ...
    # 판정 대기(active)는 매칭 csv 를 고쳐야 풀리는 건만 센다. 단계 정체는 여기서
    # 고칠 수 있는 게 없다 — Valve 쪽 파이프라인 문제이므로 따로 센다.
    active = sum(1 for a in alerts
                 if a["status"] == "active" and not a.get("decision")
                 and a.get("type") != "stage_stall")
    stalled = sum(1 for a in alerts
                  if a["status"] == "active" and a.get("type") == "stage_stall")
...
def _recommendations() -> dict[str, dict]:
    """캐시된 function step 추천 (없거나 모듈이 없으면 빈 dict — 목록은 계속 뜬다)."""
    try:
        from core import valve_step_advisor
        return valve_step_advisor.load_records()
    except Exception as e:
        logger.warning("[valve_alerts] 추천 기록 로드 실패: %s", e)
        return {}
...
def poll_once() -> dict:
    ...
    if fresh and known:  # 최초 1회(콜드스타트)는 알림 생략하고 상태만 기록
        try:
            ...
            stall = sum(1 for a in fresh if a.get("type") == "stage_stall")
            ro = len(fresh) - um - stall
            # 정체 알람은 step_id/ppid 가 비어 있다 — 쓸모 있는 건 어느 단계가 왜 멈췄나다
            def _line(a: dict) -> str:
                if a.get("type") == "stage_stall":
                    return (f"{a.get('vehicle')} {a.get('stage_label') or a.get('stage')}"
                            f"{'/' + a['source'] if a.get('source') else ''}: "
                            f"{a.get('reason') or '정체'}")
                return (f"{a.get('vehicle')} {a.get('step_id')} "
                        f"{a.get('ppid') or a.get('step_desc') or ''}").strip()
            body = " | ".join(_line(a) for a in fresh[:5])
            head = f"미매칭 {um} / RO ppid {ro}" + (f" / 단계 정체 {stall}" if stall else "")
            send_to_admins(
                f"[Valve] 신규 파이프라인 알람 {len(fresh)}건 ({head})",
    ...
    # 판정 대기 미매칭 step 의 function step 추천 — 한 번 검사한 step 은
    # 기록에 남아 다시 스캔하지 않는다 (valve_step_advisor).
    recommended = 0
    try:
        from core import valve_step_advisor
        recommended = valve_step_advisor.recommend_pending(
            data["alerts"]).get("checked", 0)
    except Exception as e:
        logger.warning("[valve_alerts] function step 추천 실패: %s", e)
