...
def list_backups() -> list:
    """백업 목록 — **조회는 아무것도 지우지 않는다.**

    v8.8.3 은 여기서도 기회적으로 `_cleanup_backups` 를 돌렸는데, 관리자가
    목록을 새로고침하는 것만으로 백업이 사라졌다. 복구 후보를 고르려고 목록을
    여는 순간 후보가 줄어드는 셈이라 위험하다. 정리는 새 백업을 만든 직후
    (`run_backup`) 에만 한다 — 그때가 개수가 실제로 늘어나는 유일한 시점이다.
    """
    ...
    while not _scheduler_stop.is_set():
        try:
            ...
            while remain > 0 and not _scheduler_stop.is_set():
                ...
                if new_iv < remain:
                    remain = new_iv
            if not _scheduler_stop.is_set() and is_owner() and get_settings()["enabled"]:
