...
MATCHING_TABLES = {
    ...
    "inline_item_map": {
        ...
        "description": "INLINE (step_id, item_id) → canonical item name + coordinate map_id.",
        "applies_to": ["INLINE"],
        "keys": ["product", "step_id", "item_id"],
        "outputs": ["canonical_item", "map_id"],
        "required_cols": ["product", "step_id", "item_id", "canonical_item", "map_id"],
    },
    "inline_subitem_pos": {
...
def matching_table_path(base: "Path", name: str) -> "Path":
    """Return filesystem path for a matching table. Stored under data/DB/matching/."""
