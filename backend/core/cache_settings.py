...
키:
  ...
  lot_list_mb          제품별 root_lot_id 풀 캐시 고정 MB (미설정=32MB)
"""
...
CACHE_SPEED_MIN = 1
CACHE_SPEED_MAX = 5
# 한 단계가 단순히 숫자만 바뀌는 것보다 체감 차이가 나도록 2배씩 늘린다.
# 1단계는 lookup/pivot 모두 반드시 root 1개씩 처리한다.
CACHE_SPEED_CHUNK_ROOTS = {1: 1, 2: 2, 3: 4, 4: 8, 5: 16}
...
def cache_speed_level(is_dev: bool) -> int:
    """관리자 화면의 단일 캐싱 속도(1~5). 미설정 기본은 가장 안전한 1단계."""
    value = get_int_role("cache_speed_level", is_dev, CACHE_SPEED_MIN)
    try:
        return max(CACHE_SPEED_MIN, min(CACHE_SPEED_MAX, int(value)))
    except Exception:
        return CACHE_SPEED_MIN


def cache_speed_chunk_roots(is_dev: bool) -> int:
    """속도 단계를 lookup/pivot의 root 처리 단위로 변환한다."""
    return CACHE_SPEED_CHUNK_ROOTS[cache_speed_level(is_dev)]
