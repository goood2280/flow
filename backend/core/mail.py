...
logger = logging.getLogger("flow.mail")
...
def looks_like_email(s: str) -> bool:
    """`_looks_like_email` 의 공개 별칭 — 다른 모듈(routers/auth 등)의 진입점."""
    return _looks_like_email(s)


def _users_by_login_key() -> dict:
    """로그인 키 → users.csv 행. `hong` 과 `hong@corp.com` 을 함께 찾는다.

    가입 화면에 id 만 적은 사람과 id@도메인으로 적은 사람이 섞여 있어서, 수신자
    목록에 어느 쪽 표기가 실려 와도 같은 계정의 email 컬럼을 찾아야 한다.
    대소문자는 구분한다 (아이디는 적힌 그대로 분류).
    """
    try:
        from routers.auth import read_users
        from core.auth import canonical_username, login_domains
    except Exception:
        return {}
    domains = login_domains()                       # 설정은 한 번만 읽는다
    out: dict = {}
    for u in read_users():
        un = (u.get("username") or "").strip()
        if not un:
            continue
        out.setdefault(un, u)                       # 원본 표기 우선
...
def resolve_usernames_to_emails(usernames: Iterable[str]) -> Tuple[List[str], List[str]]:
    ...
    try:
        ...
        _domain = ""
    try:
        from core.auth import canonical_username as _canon, login_domains as _login_domains
        _domains = _login_domains()
    except Exception:
        _canon, _domains = None, set()
    for un in usernames:
        un = (un or "").strip()
        if not un:
            continue
        u = all_users.get(un) or {}
        if not u and _canon is not None and not _looks_like_email(un):
            # 사내 id 만 실려 왔는데 그 계정이 `hong@corp.com` 형태로 저장돼 있는 경우.
            # 반대로 이미 주소 형태로 실려 온 값은 호출부가 지정한 주소이므로
            # 계정 조회로 바꿔치지 않는다 (비번 재발급 메일이 그렇게 온다).
            u = all_users.get(_canon(un, _domains)) or {}
        # 우선순위: users.csv 의 email 컬럼 > 계정의 username(주소 형태) >
        #   실려온 값(주소 형태) > v8.8.19 도메인 조합 > skip.
        stored = (u.get("username") or "").strip()
        for cand in ((u.get("email") or "").strip(), stored, un, _apply_domain(un, _domain)):
            if not _looks_like_email(cand):
                continue
            key = cand.lower()
            if key not in seen:
...
def send_mail(
    ...
    *,
    ...
    # v9.0.0 (Q4 확정): 사내 API 규약 — HTTP POST 의 `data` 필드가
    #   `{"mailSendString": "<JSON string of data_obj>"}` 형태여야 함.
    #   즉 평면 data_obj 가 아니라 mailSendString 키로 한 번 감싼 단일 top-level form field.
    #   키 이름은 camelCase `mailSendString` 하나로 통일.
