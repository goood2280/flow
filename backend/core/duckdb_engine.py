...
# A browser refresh or a second SQL execution must not leave the previous
# full-partition scan consuming CPU and disk.  DuckDB's interrupt() is safe to
# call from another thread, so keep only the newest connection for a logical
# File Browser source.
...
def _activate_query(query_key: str, con) -> None:
    key = str(query_key or "").strip()
    if not key:
        return
    with _ACTIVE_QUERY_LOCK:
        previous = _ACTIVE_QUERIES.get(key)
        _ACTIVE_QUERIES[key] = con
    if previous is not None and previous is not con:
        try:
            previous.interrupt()
        except Exception:
            pass
...
def release_query(query_key: str, con) -> None:
    key = str(query_key or "").strip()
    if not key:
        return
    with _ACTIVE_QUERY_LOCK:
        if _ACTIVE_QUERIES.get(key) is con:
            _ACTIVE_QUERIES.pop(key, None)


def interrupt_query(query_key: str) -> bool:
    key = str(query_key or "").strip()
    if not key:
        return False
    with _ACTIVE_QUERY_LOCK:
        con = _ACTIVE_QUERIES.get(key)
    if con is None:
        return False
    try:
        con.interrupt()
...
def is_interrupted_error(exc: Exception) -> bool:
    text = str(exc or "").casefold()
    return "interrupt" in text or "cancelled" in text or "canceled" in text
...
def open_source(files: list[Path], *, query_key: str = ""):
