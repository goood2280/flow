...
입력 파일 (파일탐색기 Files 위치 = DB root, 상대경로는 db_root 기준):
  ...
  · Teg_location.csv: vehicle, teg, ebeam_x, ebeam_y (shot 센터 기준 TEG 좌하단, TEG 는 직사각형)
    선택 열: teg_w, teg_h — 없으면 설정의 TEG 기본 사이즈 사용
             ...
             direction — H/Horizontal(가로, 기본) | V/Vertical(세로로 세움 — w/h 스왑)
...
def default_check_targets(names: list[str]) -> list[str]:
    """기본 체크 대상 — teg 이름이 H_/V_ 로 시작하는 것 전부 (대소문자 무관, 순서 유지)."""
    out: list[str] = []
    for n in names:
        s = str(n or "").strip()
        up = s.upper()
        if s and (up.startswith("H_") or up.startswith("V_")):
            out.append(s)
    return out
...
def _teg_rows_for(vehicle: str) -> tuple[list[str], dict[str, list[str]], bool, str]:
    ...
    {teg: [top_cell, ...]}, teg 파일 존재 여부, 파일 경로).
    ...
    return order, top_cells, True, str(path)


def teg_target_options(vehicle: str) -> dict:
    """위치 조회의 '체크 대상 설정' UI 용 — vehicle 의 teg 목록 + 현재 대상 선택.

    반환: {ok, teg_ok, teg_path, tegs:[{teg, top_cell:[...]}], targets:[teg 이름],
           source:"config"|"default"}. source=config 는 관리자가 저장한 명시적 목록.
    """
    names, top_cells, teg_ok, path = _teg_rows_for(vehicle)
    cfg = load_cfg()
    ...
    return {
        ...
        "teg_ok": teg_ok,
        "teg_path": path,
        "tegs": [{"teg": t, "top_cell": top_cells.get(t, [])} for t in names],
        "targets": targets,
        "source": source,
    }
...
def target_verification(vehicle: str, module_tokens) -> dict:
    ...
    반환: {source, items:[{teg, top_cell, matched, matched_by, matched_module}],
        ...
        if teg in tokens:                       # teg 이름 완전 일치
            ...
            for tc in tcs:
                if tc in tokens:
                    by, mod = "top_cell", tc
                    break
        items.append({"teg": teg, "top_cell": tcs, "matched": by is not None,
        ...
        for g in sorted(overlays):
            ...
            for row in rows_g:
                ...
                while name in taken:
                    ...
                    i += 1
                taken.add(name)
                tegs.append({"teg": name, "ebeam_x": ox * scale, "ebeam_y": oy * scale,
                             "teg_w": dw, "teg_h": dh, "flat_zone": "h",
                             "overlay_group": g})
                n_added += 1
