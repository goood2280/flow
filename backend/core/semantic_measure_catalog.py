...
DEFAULT_MEASUREMENT_TERMS: list[dict[str, Any]] = [
    {
        ...
        "step_id": "AA100040",
        "item_id": "CD2",
        "value_column": "fab_value",
        "default_agg": "raw",
        "target": None,
        "spec_low": None,
        "spec_high": None,
        "evidence": [
            {"type": "operator_seed", "label": "CA BCD is PRODA step AA100040 item CD2", "source": "bundled_setup_seed"}
        ],
    },
    {
...
def normalize_term(raw: dict[str, Any], *, actor: str = "system", base: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if default_agg not in {"raw", "avg", "mean", "median", "min", "max"}:
...
def ensure_catalog(*, actor: str = "runtime") -> dict[str, Any]:
    ...
    for row in existing.get("terms", []):
        if not isinstance(row, dict) or not row.get("id") or str(row.get("id") or "") in deleted_ids:
            continue
        row_id = str(row.get("id") or "")
        # Upgrade only the untouched legacy example. Any operator-edited
        # mapping is preserved exactly as entered.
        if (row_id == "measure_inline_proda_ca_bcd"
                and not row.get("step_id")
                and str(row.get("item_id") or "") == "CA_BCD"
                and str(row.get("updated_by") or "") in {"default_seed", "runtime", "setup.py"}):
            replacement = next((item for item in defaults if item.get("id") == row_id), row)
            by_id[row_id] = replacement
        else:
            by_id[row_id] = normalize_term(row, actor=str(row.get("updated_by") or actor))
    added = 0
    for row in defaults:
...
def parse_natural_mapping(text: str, *, use_llm: bool = True) -> dict[str, Any]:
    """Turn one operator sentence into a reviewable semantic measurement mapping.

    Deterministic extraction is preferred. The configured LLM is only used to
    fill missing fields, and its output is validated before it can be saved.
    """
    sentence = _clean(text, 1000)
    mapping: dict[str, Any] = {
        "term": "", "aliases": [], "source_type": "INLINE", "product": "",
        "step_id": "", "item_id": "", "value_column": "fab_value",
        "default_agg": "raw",
    }
    parser = "deterministic"
    m = re.search(r"^\s*(.+?)(?:은|는)\s+([A-Za-z][A-Za-z0-9_-]*)(?:에선|에서는|에서)", sentence, flags=re.I)
    if m:
        mapping["term"] = _clean(m.group(1), 160)
        mapping["product"] = _upper(m.group(2))
    step = re.search(r"\b([A-Za-z]{1,8}\d{4,}[A-Za-z0-9_-]*)\s*(?:step\s*id|step|스텝)", sentence, flags=re.I)
    if not step:
        step = re.search(r"(?:step\s*id|step|스텝)\s*(?:는|은|이|가|:|=)?\s*([A-Za-z]{1,8}\d{4,}[A-Za-z0-9_-]*)", sentence, flags=re.I)
    item = re.search(r"(?:item\s*id|item|아이템)\s*(?:는|은|이|가|:|=)?\s*([A-Za-z0-9_.-]+)", sentence, flags=re.I)
    if step:
        mapping["step_id"] = _upper(step.group(1))
    if item:
        mapping["item_id"] = _clean(item.group(1), 160)
    source = re.search(r"\b(INLINE|ET|FAB|VM|MASK)\b", sentence, flags=re.I)
    if source:
        mapping["source_type"] = _upper(source.group(1))

    required = ("term", "product", "step_id", "item_id")
    if use_llm and sentence and any(not mapping.get(key) for key in required):
        try:
            from core import llm_adapter
            schema = {
                "type": "object",
                "properties": {
                    "term": {"type": "string"}, "source_type": {"type": "string"},
                    "product": {"type": "string"}, "step_id": {"type": "string"},
                    "item_id": {"type": "string"}, "value_column": {"type": "string"},
                },
                "required": ["term", "source_type", "product", "step_id", "item_id", "value_column"],
                "additionalProperties": False,
            }
            out = llm_adapter.complete_json(
                sentence,
                system=("Extract an operator-defined semiconductor measurement mapping. "
                        "Never invent a value: return an empty string when it is not explicit. "
                        "source_type defaults to INLINE and value_column defaults to fab_value."),
                schema=schema,
                timeout=12,
            )
            obj = out.get("obj") if isinstance(out, dict) and out.get("ok") else None
            if isinstance(obj, dict):
                for key in ("term", "source_type", "product", "step_id", "item_id", "value_column"):
                    if not mapping.get(key) and obj.get(key):
                        mapping[key] = obj[key]
                parser = "gpt_oss_structured"
        except Exception:
            pass

    mapping["term"] = _clean(mapping.get("term"), 160)
    mapping["source_type"] = _upper(mapping.get("source_type") or "INLINE")
    mapping["product"] = _upper(mapping.get("product"))
    mapping["step_id"] = _upper(mapping.get("step_id"))
    mapping["item_id"] = _clean(mapping.get("item_id"), 160)
    mapping["value_column"] = _clean(mapping.get("value_column") or "fab_value", 120)
    mapping["aliases"] = _list([mapping["term"], mapping["term"].replace(" ", "_"), mapping["term"].replace(" ", "")], 10)
    mapping["evidence"] = [{"type": "operator_statement", "label": sentence, "source": "admin_flowi_learning"}]
    missing = [key for key in required if not mapping.get(key)]
    return {
        "mapping": mapping,
        "parser": parser,
        "ready_to_save": not missing,
        "warnings": [f"필수 값이 없습니다: {', '.join(missing)}"] if missing else [],
        "sql_template": build_reproducible_sql(mapping, root_lot_id="<ROOT_LOT_ID>"),
    }
...
def build_reproducible_sql(
    term: dict[str, Any], *, root_lot_id: str = "", files: list[Path] | None = None, limit: int = 25
) -> str:
    value_col = _clean(term.get("value_column") or "fab_value", 120)
    product = _upper(term.get("product"))
    if files:
        paths = ", ".join(_sql_literal(str(path).replace("\\", "/")) for path in files)
        source = f"read_parquet([{paths}], union_by_name=true)"
    else:
        source = f"{_upper(term.get('source_type') or 'INLINE')}_DB.{product or '<PRODUCT>'}"
    where = []
    if root_lot_id:
        where.append(f"root_lot_id = {_sql_literal(root_lot_id)}")
    if term.get("step_id"):
        where.append(f"step_id = {_sql_literal(_upper(term.get('step_id')))}")
    if term.get("item_id"):
        where.append(f"item_id = {_sql_literal(term.get('item_id'))}")
    where_sql = "\n  AND ".join(where) if where else "1 = 1"
    return (f"SELECT root_lot_id, wafer_id, step_id, item_id, {value_col}\n"
...
def query_measurement(prompt: str, *, product: str = "", max_rows: int = 25) -> dict[str, Any] | None:
    ...
    if not matches:
        return None
    term = matches[0]
    # A saved semantic mapping is authoritative. Lot IDs often look like
    # products (for example AZ111), so prompt token guessing comes last.
    product_u = _upper(product) or _upper(term.get("product")) or _first_product(prompt)
    ...
    if not value_col:
        value_col = _ci_col(cols, "fab_value", "FAB_VALUE", "value", "VALUE", "measure_value", "MEASURE_VALUE", "result", "RESULT", "val", "VAL")
    ...
    if item_col and item_id:
        # Exact item_id is the reproducible semantic contract. Human aliases
        # select the term, but must never broaden the underlying data filter.
        candidates = [_norm(item_id)] if _norm(item_id) else []
        if candidates:
            item_text = pl.col(item_col).cast(_STR, strict=False).str.replace_all(r"[^0-9A-Za-z가-힣]+", "").str.to_lowercase()
            filters.append(pl.any_horizontal([item_text == c for c in candidates]))
    ...
    value_alias = value_col if agg == "raw" else ("value_avg" if agg in {"avg", "mean"} else f"value_{agg or 'median'}")
    exprs: list[pl.Expr] = [
    ...
    try:
        if agg == "raw":
            df = (scoped.select([
                    "root_lot_id", "wafer_id", "step_id", "item_id", "product",
                    pl.col("_value").alias(value_alias),
                ])
                .sort("wafer_id")
    ...
    return _measurement_result(
        ...
        sql=build_reproducible_sql(term, root_lot_id=root_lot, files=files, limit=max_rows),
    )
...
def _measurement_result(
    ...
    *,
    ...
    if agg != "raw":
        columns.insert(3, {"key": "n", "label": "n"})
    answer = (
    ...
    return {
        ...
        "answer": (
            f"'{term.get('term')}' 매핑은 찾았지만 조회를 완료하지 못했습니다: {'; '.join(warnings[:3])}"
            if warnings else
            f"{source_type} DB에서 {product or term.get('product') or '-'} / {root_lot or '-'} / {term.get('term')} 원본 값을 조회했습니다. "
            f"매핑은 step_id={term.get('step_id') or '-'}, item_id={term.get('item_id') or '-'}이며 결과는 {len(rows)}건입니다."
        ),
        "slots": {
        ...
        "sql_draft": {
            "dialect": "DuckDB SQL",
            "read_only": True,
            "sql": sql or build_reproducible_sql(term, root_lot_id=root_lot),
            "source_files": list(source_files or []),
        },
        "source_detail": {
            "catalog": f"{source_type}_DB",
            "files": list(source_files or []),
            "mapping_evidence": deepcopy(term.get("evidence") or []),
        },
        "warnings": warnings,
    }
