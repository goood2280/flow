...
PIVOT_KEY_COLS = [
    ...
    "step_id", "step_seq", "flat", "flat_zone",
    # ET sources use either shot_x/shot_y or chip_x_pos/chip_y_pos for the
    # measured shot coordinate. Both must be pivot keys; otherwise a source
    # with only chip_*_pos silently collapses to one row per wafer.
    "shot_x", "shot_y", "chip_x_pos", "chip_y_pos",
]
...
# 시간 메타는 소스마다 dtype 이 제각각이다 — 문자열(ISO), Datetime, Date, Time 이
# 섞여 들어온다. 그대로 CSV 로 내보내면 Time 이나 소수점 이하 초가 붙은 값이 엑셀
# 에서 `mm:ss.0` 서식으로 잡혀 **"42:27.0"** 처럼 보인다 (날짜와 시(hour)가 통째로
# 사라진다). 같은 파일 안에서 tkin_time 은 멀쩡하고 tkout_time 만 그렇게 나오던
# 것도 두 열의 dtype 이 달라서였다. 화면·CSV 어디서나 같은 모습이 되도록
# `YYYY-MM-DD HH:MM:SS` 한 형식으로 고정한다.
TIME_META_COLS = ("tkin_time", "tkout_time")
TIME_META_FORMAT = "%Y-%m-%d %H:%M:%S"
...
def _time_meta_expr(name: str, dtype) -> pl.Expr:
    """시간 메타 한 컬럼을 `YYYY-MM-DD HH:MM:SS` 문자열로 통일하는 식.

    - Datetime/Date: 그대로 포맷 (소수점 이하 초를 떨군다 — 엑셀 오인의 원인).
    - Time: 소스에 날짜가 없다. 초 단위까지만 남긴다.
    - Utf8: 해석되는 값만 다시 포맷하고, 못 읽는 값은 **그대로 둔다**
      (사내 소스에 어떤 표기가 들어올지 다 알 수 없으므로 값을 잃지 않는다).
    - 그 밖의 dtype: 건드리지 않는다.
    """
    col = pl.col(name)
    if dtype == pl.Date:
        return col.dt.strftime("%Y-%m-%d").alias(name)
    if dtype == pl.Time:
        return col.dt.strftime("%H:%M:%S").alias(name)
    if isinstance(dtype, pl.Datetime) or dtype == pl.Datetime:
        return col.dt.strftime(TIME_META_FORMAT).alias(name)
    if dtype == pl.Utf8:
        parsed = col.str.to_datetime(strict=False)
        return (pl.when(parsed.is_not_null())
                  .then(parsed.dt.strftime(TIME_META_FORMAT))
                  .otherwise(col)
                  .alias(name))
    return col.alias(name)


def _time_only_meta_expr(name: str) -> pl.Expr:
    """날짜 없이 `15:42:27.0` 처럼 들어온 문자열의 소수점 이하 초를 떨군다.

    이 표기가 그대로 CSV 에 실리면 엑셀이 `mm:ss.0` 서식으로 잡아 시(hour)까지
    사라진 "42:27.0" 이 된다.
    """
    col = pl.col(name)
    parsed = col.str.to_time(strict=False)
    return (pl.when(parsed.is_not_null())
              .then(parsed.dt.strftime("%H:%M:%S"))
              .otherwise(col)
              .alias(name))


def normalize_time_meta(df: "pl.DataFrame") -> "pl.DataFrame":
    """tkin_time / tkout_time 을 한 표기로 맞춘다 (컬럼별로 독립 처리).

    한 컬럼이 실패해도 나머지는 정규화한다 — 문자열 컬럼은 polars 가 형식을
    추론하지 못하면 예외를 던진다. 그때는 날짜 없는 시각 표기를 한 번 더
    시도하고, 그것도 안 되면 **원본 값을 그대로 둔다**(값을 잃지 않는다).
    """
    for name in TIME_META_COLS:
        if name not in df.columns:
            continue
        dtype = df.schema[name]
        try:
            df = df.with_columns(_time_meta_expr(name, dtype))
            continue
        except Exception as e:
            first_error = e
        if dtype == pl.Utf8:
            try:
                df = df.with_columns(_time_only_meta_expr(name))
                continue
            except Exception:
                pass
        logger.warning("시간 메타 %s 는 원본 표기 그대로 둔다 (%s)", name, first_error)
    return df
...
def load_vehicle_table(csv_path: Path) -> List[Dict[str, Any]]:
    ...
    for raw in df.to_dicts():
        ...
        if category == "ADDP" and not addp_form:
            continue
        # ALIAS 는 이 테이블의 행 식별자다 — 출력 컬럼명이자 공개/비공개·선택
        # 상태의 키. 같은 ALIAS 가 두 번 오면 한 항목이 서로 다른 상태로
        # 갈려 보이고(같은 real item 인데 공개/비공개가 섞임), pivot 결과에도
        # 같은 이름의 컬럼을 두 번 만들어 계산이 통째로 실패한다.
        # 파일 순서 그대로 첫 행만 남긴다.
        if alias in seen_aliases:
            logger.warning("vehicle CSV %s: ALIAS '%s' 가 중복이라 뒤 행을 무시합니다",
                           csv_path.name, alias)
            continue
...
def reformatize(df: pl.DataFrame, table: List[Dict[str, Any]],
    ...
    ``skip_real_scale=True`` 는 원본 값에 REAL scale factor 가 이미 곱해진
    소스용 (예: auto report 산출 et_value). REAL 단계에서 scale 을 다시 곱하지
    않는다. abs 는 멱등이라 그대로 적용하고, ADDP 는 우리가 계산하는 값이므로
    자체 scale wrapping 을 포함해 평소와 동일하게 동작한다.
    """
    errors: list[str] = []
    ...
    if metas:
        ...
        # 시간 표기는 여기서 한 번만 맞춘다 — 화면 미리보기와 CSV 다운로드가
        # 같은 wide 프레임을 쓰므로 두 곳이 자동으로 같아진다.
        meta_df = normalize_time_meta(meta_df)
    ...
    for row in work_table:
        ...
        if not skip_real_scale:
            expr = expr * float(row["scale"])
        if row["absolute"]:
            expr = expr.abs()
