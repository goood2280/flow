...
All three read CSVs from `<db_root>/matching/` by default. If a matching table is
...
# ─────────────────────────────────────────────────────────────────────
# 2. INLINE coordinate mapper — (item_id, subitem_id) → (shot_x, shot_y)
# ─────────────────────────────────────────────────────────────────────
def apply_inline_coord(df: pl.DataFrame, product: str, matching_root: Path,
    ...
    """Two-step join:
      1) inline_item_map.csv: (product, item_id) → canonical_item + item-specific map_id
    ...
    if im_tbl is not None and item_col in df.columns:
        ...
        if need.issubset(set(im_tbl.columns)):
            has_step_map = "step_id" in im_tbl.columns
            im_sel = im_tbl.select([
                pl.col("item_id").cast(pl.Utf8).alias("_item"),
                (pl.col("step_id").cast(pl.Utf8) if has_step_map else pl.lit("")).alias("_step"),
            ...
            if has_step_map and step_col in df.columns and "_step" in im_sel.columns:
                out_df = out_df.join(
                    im_sel,
