...
FILTER_DRAFT_SYSTEM_PROMPT = (
    ...
    "Treat Korean '들어간/포함' about a row value as LIKE '%value%'. "
    "Treat recent/last N days as a rolling timestamp condition on the explicitly named time column. "
    "Cast time-named columns to TIMESTAMP and wafer_id/wf_id to BIGINT before comparison. "
    "If the user asks which column names contain a token, return an empty sql filter; that is projection, not row filtering."
)
...
COLUMN_DRAFT_SYSTEM_PROMPT = (
    ...
    "For requests like 'root_lot_id, wafer_id와 eSD가 들어간 열', include the exact named columns and every provided column whose name contains eSD."
)
...
def _filter_draft(state: dict[str, Any], warnings: list[str]) -> dict[str, Any]:
    ...
    fallback = False
    projection_only = fb._ai_sql_projection_only_prompt(prompt, columns)
    deterministic_required = bool(
        fb._ai_sql_recent_days_clause(prompt, columns)
        or fb._ai_sql_contains_clause(prompt, columns)
    )
    if projection_only:
        raw_sql = ""
        warnings.append("column-name search treated as display projection, not a row filter")
    elif deterministic_required:
        deterministic_sql = fb._fallback_ai_sql(
            prompt,
            columns,
        ...
        if deterministic_sql:
            raw_sql = deterministic_sql
            fallback = True
            warnings.append("deterministic contains/recent-time semantics applied")
    try:
        if projection_only:
            sql = ""
            validate_warnings = []
...
def _column_draft(state: dict[str, Any], warnings: list[str]) -> dict[str, Any]:
    ...
    warnings.extend(llm_warnings)
    deterministic_projection = fb._ai_sql_column_name_projection(prompt, columns)
    selected = deterministic_projection or fb._filter_ai_sql_selected_columns(
        _string_list(plan.get("selected_columns"), limit=300), columns, warnings, "column_draft"
    )
    if deterministic_projection:
        warnings.append("deterministic column-name projection applied")
    if not selected:
...
def _merge(state: dict[str, Any], warnings: list[str]) -> dict[str, Any]:
    ...
    try:
        if not where_sql and fb._ai_sql_projection_only_prompt(
            _safe_text((state.get("request") or {}).get("natural_language"), 2000), columns
        ):
            raise LookupError("projection-only request")
