...
def _filebrowser_sql_draft(state: dict[str, Any], warnings: list[str]) -> dict[str, Any]:
    ...
    try:
        ...
        max_rows = 50
    sql_prompt = _safe_text(req.get("natural_language"), 1600)
    query_rules = _safe_text(req.get("query_rules"), 1200)
    if query_rules:
        sql_prompt = f"{sql_prompt}\n\n{query_rules}"
    payload = {
        "natural_language": _safe_text(sql_prompt, 2800),
