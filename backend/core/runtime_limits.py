...
def splittable_product_ram_cache_scheduler_enabled() -> bool:
    """Retired: whole-product RAM warmup is never started."""
    return False


def splittable_root_lot_ram_cache_scheduler_enabled() -> bool:
    """Retired: root-lot RAM warmup is never started (manual included)."""
    return False
...
def _cgroup_memory_snapshot_bytes() -> dict[str, float | str]:
    ...
    # Keep the Grafana/cAdvisor working set separate from the anonymous-memory
    # pressure estimate.  cAdvisor subtracts only inactive_file; the soft guard
    # intentionally subtracts all file cache (active + inactive).
    stat_path = (
        "/sys/fs/cgroup/memory.stat"
        if source == "cgroup_v2"
        else "/sys/fs/cgroup/memory/memory.stat"
    )
    inactive_field = "inactive_file" if source == "cgroup_v2" else "total_inactive_file"
    active_field = "active_file" if source == "cgroup_v2" else "total_active_file"
    inactive_file = min(used_raw, max(0, _read_cgroup_stat_field(stat_path, inactive_field)))
    active_file = min(
        max(0, used_raw - inactive_file),
        max(0, _read_cgroup_stat_field(stat_path, active_field)),
    )
    reclaimable = inactive_file + active_file
    grafana_working_set = max(0, used_raw - inactive_file)
    used_working = max(0, used_raw - reclaimable)
    return {
        "total_bytes": float(total),
        "used_bytes": float(used_working),
        "used_raw_bytes": float(used_raw),
        "working_set_bytes": float(grafana_working_set),
        "inactive_file_bytes": float(inactive_file),
        "active_file_bytes": float(active_file),
...
def _read_smaps_rollup() -> dict[str, float]:
    ...
    Anonymous counts memory not backed by a file (true Python objects),
    ignoring OS page caches (like mmap parquet files).

    Returns {"pss_kb": ..., "uss_kb": ..., "anon_kb": ...}. All 0 on non-Linux / failure.
    """
    pss_kb = 0
    uss_kb = 0
    anon_kb = 0
    try:
        ...
        m_anon = re.search(r"^Anonymous:\s+(\d+)\s+kB", text, flags=re.MULTILINE)
        if m_anon:
            anon_kb = int(m_anon.group(1))
    except Exception:
        pass
    return {"pss_kb": pss_kb, "uss_kb": uss_kb, "anon_kb": anon_kb}
...
def process_memory_snapshot() -> dict:
    ...
    anon_gb = float(smaps.get("anon_kb") or 0) / (1024 ** 2) if smaps.get("anon_kb") else 0.0

    # 대표 프로세스 메모리 = Anon (파일 캐시 제외) > PSS (가용하면) > USS > RSS 순으로 정확도가 높다.
    # Prefer the exact container working set used by Grafana/cAdvisor.  It also
    # includes cache-builder child processes in the same cgroup.  On a normal
    # host, retain the process-level fallback chain.
    cgroup = _cgroup_memory_snapshot_bytes()
    container_working_set_gb = (
        float(cgroup.get("working_set_bytes") or 0.0) / (1024 ** 3) if cgroup else 0.0
    )
    container_current_gb = (
        float(cgroup.get("used_raw_bytes") or 0.0) / (1024 ** 3) if cgroup else 0.0
    )
    if container_working_set_gb > 0:
        effective_gb, effective_kind = container_working_set_gb, "container_working_set"
    elif anon_gb > 0:
        effective_gb, effective_kind = anon_gb, "anonymous"
    elif pss_gb > 0:
    ...
    out = {
        ...
        "container_memory_current_gb": round(container_current_gb, 3),
...
def _default_polars_threads() -> str:
    # SplitTable 역할 정책을 정본으로 쓴다. 특히 개발은 환경변수 잔존 여부와
    # 무관하게 1코어 고정이어야 운영 설정이나 옛 .env 값에 끌려가지 않는다.
