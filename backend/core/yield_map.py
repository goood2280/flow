    ...
    if isinstance(table_rows, list) and table_rows:
        ...
        for key, value in ((payload or {}).get("bin_colors") or {}).items():
            ...
            if bin_name and color and len(bin_map) < 200:
                ...
                colors[bin_name] = color
                bin_map.append({"bin": bin_name, "bin_color": color})
    clean = {"source": source, "fields": fields, "bin_map": bin_map, "bin_colors": colors}
    with _LOCK:
        cfg = load_config()
...
def map_data(product: str, lot_id: str = "", wafer_id: str = "") -> dict:
    ...
    if not source_id:
        raise ValueError("이 제품의 BIN/MSR TABLE 설정이 없습니다")
    ...
    df = lf.select(exprs).limit(MAX_MAP_ROWS + 1).collect(streaming=True)
    overflow = df.height > MAX_MAP_ROWS
    if overflow:
        df = df.head(MAX_MAP_ROWS)
    rows = []
    for row in df.to_dicts():
        ...
        rows.append({
            "x": x, "y": y,
            "bin": str(row.get("bin") if row.get("bin") is not None else ""),
            "msr": row.get("msr"), "lot": row.get("lot"), "wafer": row.get("wafer"),
        })
    counts = Counter(row["bin"] for row in rows)
    net_die = len({(row["x"], row["y"]) for row in rows})
    return {
        "product": product, "source": source_id, "fields": selected,
        "bin_colors": cfg.get("bin_colors") or {}, "rows": rows,
        "bins": [{"bin": key, "count": value} for key, value in sorted(counts.items())],
        "net_die": net_die, "overflow": overflow, "file_count": len(files),
    }
