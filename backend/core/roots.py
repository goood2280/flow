...
# Where Agent B (dev-admin) writes runtime overrides. Not owned by this module —
# we read-only peek at it. If the file is absent or malformed we silently fall
# through to the next priority tier.
_ADMIN_SETTINGS = _PROJECT_ROOT / "data" / "admin_settings.json"


def _read_admin_setting(key: str) -> str | None:
    """Peek `admin_settings.json → data_roots[<key>]`. Returns None if missing."""
    try:
        if not _ADMIN_SETTINGS.is_file():
            return None
        with open(_ADMIN_SETTINGS, "r", encoding="utf-8") as f:
            cfg = json.load(f)
...
def snapshot() -> dict:
    ...
    return {
        ...
        "admin_settings": str(_ADMIN_SETTINGS),
    }
