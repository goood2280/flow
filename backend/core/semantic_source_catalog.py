...
# These descriptions are bundled into setup.py so a newly extracted server
# already knows why each DB exists before an operator teaches custom terms.
_SOURCE_PURPOSES = {
    "rulebook": ("PPID/KNOB 분류 규칙의 기준표", "rule row", ["feature_name", "function_step"]),
    "step_matching": ("제품·step_id를 기능 공정과 설명으로 연결", "product + step_id", ["product", "step_id"]),
    "split_base": ("wafer 단위 SplitTable 조회 및 raw export 기준", "root_lot_id + wafer_id", ["root_lot_id", "wafer_id"]),
    "fab_db": ("랏 진행 이력과 현재 위치의 원본", "lot/wafer + step event", ["root_lot_id", "wafer_id", "step_id"]),
    "inline_db": ("공정 중 계측 item의 wafer별 원본 값", "root_lot_id + wafer_id + step_id + item_id", ["root_lot_id", "wafer_id", "step_id", "item_id"]),
    "et_db": ("전기적 검사 item의 wafer별 원본 값", "root_lot_id + wafer_id + item_id", ["root_lot_id", "wafer_id", "item_id"]),
}
for _source_id, (_description, _grain, _join_keys) in _SOURCE_PURPOSES.items():
    SEMANTIC_SOURCE_CATALOG[_source_id].setdefault("description", _description)
    SEMANTIC_SOURCE_CATALOG[_source_id].setdefault("grain", _grain)
    SEMANTIC_SOURCE_CATALOG[_source_id].setdefault("join_keys", _join_keys)
...
def normalize_source(raw: dict[str, Any], *, actor: str = "system", base: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "description": _clean_text(raw.get("description") if "description" in raw else base.get("description"), 400),
        "grain": _clean_text(raw.get("grain") if "grain" in raw else base.get("grain"), 160),
        "join_keys": _list_text(raw.get("join_keys") if "join_keys" in raw else base.get("join_keys"), 30),
