...
def source_data_files(source_type: str = "", root: str = "", product: str = "",
    ...
    if str(root or "").strip().upper() == "SPLITTABLE" and product:
        ...
        if safe_product != str(product) or safe_product in {"", ".", ".."}:
            ...
            split_dir = DB_BASE / "cache" / "split_table" / safe_product
            # Root-lot별 main pivot만 사용한다. knob/ 사이드카까지 재귀로
            # 읽으면 같은 wafer가 두 번 들어가므로 의도적으로 직속 파일만.
            files = sorted(fp for fp in split_dir.glob("*.parquet") if fp.is_file()) if split_dir.is_dir() else []
    elif source_type == "base_file":
        fp = PATHS.base_root / file
...
def atomic_write_text(path: Path, text: str, encoding: str = "utf-8") -> None:
    """같은 폴더의 임시 파일에 쓴 뒤 os.replace 로 갈아끼운다.

    `path.write_text` 는 **기존 파일을 먼저 0바이트로 비우고** 다시 쓰기 때문에,
    그 사이에 프로세스가 죽으면(OOM·재시작·디스크 full) 원본이 통째로 날아간다.
    users.csv 나 informs.json 이 그렇게 비면 로그인 불가·글 전량 소실이 되고,
    `load_json` 이 손상을 삼키고 기본값을 돌려주므로 "데이터가 없음" 으로만 보인다.

    임시 파일명에 pid+tid 를 넣는 이유: data_root 가 운영/개발 두 서버가 함께
    쓰는 공유 경로라 고정 이름(`.tmp`)이면 서로의 임시 파일을 덮어써 섞인 내용이
...
def save_json(path: Path, data, indent: int = None):
    ...
    v9.5.70: temp+os.replace 원자적 저장 — 저장 중 크래시로 파일이 비는 것을 막는다."""
    atomic_write_text(
        path,
...
# 운영 로그 상한 — 무한 성장 방지. append 마다 파일 전체를 읽으면 비싸므로
...
_JSONL_TRIM_LOCK = threading.Lock()


def jsonl_append(path: Path, entry: dict, add_timestamp: bool = True,
                 max_lines: int | None = JSONL_MAX_LINES_DEFAULT,
    ...
    if max_bytes and max_bytes > 0:
        try:
            if path.stat().st_size > int(max_bytes):
                jsonl_trim_bytes(path, int(max_bytes))
        except Exception:
            pass
    if not max_lines or max_lines <= 0:
        return
    key = str(path)
    with _JSONL_TRIM_LOCK:
        count = _JSONL_APPEND_COUNTS.get(key, 0) + 1
        due = count >= _JSONL_TRIM_CHECK_EVERY
        _JSONL_APPEND_COUNTS[key] = 0 if due else count
    if not due:
        return
    try:
        jsonl_trim(path, int(max_lines))
    except Exception:
        pass
...
def jsonl_trim(path: Path, max_lines: int):
    """Keep only last max_lines. 교체는 원자적으로 — trim 중 크래시로 로그가
    잘린 채 확정되는 것을 막는다 (read 와 write 사이의 append 유실은 별건)."""
    if not path.exists():
        return
...
def jsonl_trim_bytes(path: Path, max_bytes: int) -> None:
    ...
    with open(path, "rb") as f:
        ...
        tail = f.read()
    # The seek can land inside a record or a UTF-8 sequence. Discard that
    # partial record and retain only complete JSONL records.
    first_newline = tail.find(b"\n")
    kept = tail[first_newline + 1:] if first_newline >= 0 else b""
    atomic_write_text(path, kept.decode("utf-8"), encoding="utf-8")
