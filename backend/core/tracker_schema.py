...
def normalize_lot_row(row: dict | None) -> dict:
    ...
    # root lot 형식은 제품/세대에 따라 길이와 suffix가 다르다. UI가
    # root_lot_id 필드로 저장한 값은 길이로 재분류하지 않고 그대로 보존한다.
    # 조회부가 root 열 우선, lot/fab_lot 열 fallback 순서로 호환 처리한다.
    out["lot_id"] = lot_id
    ...
    # ET 스캔이 실패했을 때의 사유 — "측정 없음" 과 구분해 화면에 띄운다.
    out.setdefault("last_scan_error", src.get("last_scan_error") or "")
