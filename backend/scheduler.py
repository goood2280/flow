"""Lightweight daily product-list dedup scheduler.

Shared SplitTable cache construction belongs exclusively to the unified
product cache rotation.  This scheduler only normalizes product metadata.
"""
...
def _loop() -> None:
    while not _stop.is_set():
        ...
        try:
            ...
            logger.warning("product dedup scheduler tick failed: %s", exc)
...
def start_scheduler() -> bool:
    ...
    _thread.start()
    _started = True
    logger.info("product dedup scheduler started (03:00 daily; metadata only)")
    return True
