...
class WorkerRoleReq(BaseModel):
    role: str  # api | worker | standalone
...
def worker_set_role(req: WorkerRoleReq, request: Request, _admin=Depends(require_admin)):
    """이 서버의 역할을 server_role.json 에 저장 — 재시작 없이 즉시 반영.

    app_root 읽기전용이면 data_root 폴백에 저장된다 (worker_dispatch.set_role).
    FLOW_SERVER_ROLE env 로 고정된 배포에서는 UI 변경을 거부한다 (env 우선)."""
    from core import worker_dispatch
    out = worker_dispatch.set_role(req.role)
    if not out.get("ok"):
        status = 500 if out.get("code") == "write_failed" else 400
...
BASE_CSV_SCHEMAS = {
    ...
    "inline_matching": {
        "columns": ["product", "step_id", "item_id", "item_desc"],
