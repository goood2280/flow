...
v8.4.4 change:
  - CSV 저장 위치를 DB 루트 최상단으로 통일. Dashboard 데이터 소스 드롭다운은
    호환용 source_type=base_file 로 자동 인식.
...
# v8.8.33 보안: 모든 TableMap 엔드포인트는 로그인 필수.
#   쓰기(save/delete/relations) 는 admin 또는 page_admin("tablemap") 으로 격리.
def _require_tablemap_admin(request: Request):
    me = current_user(request)
    if me.get("role") == "admin":
        return me
    if is_page_admin(me.get("username") or "", "tablemap"):
        return me
    raise HTTPException(403, "admin or tablemap page_admin only")
...
def _norm_product(v: Any) -> str:
    return str(v or "").strip()


def _hidden_product_pages(cfg: Optional[dict] = None) -> List[str]:
    cfg = cfg if isinstance(cfg, dict) else _load_config()
    hidden = cfg.get("hidden_product_pages") or []
    if not isinstance(hidden, list):
        return []
    seen = set()
    out = []
    for item in hidden:
        product = _norm_product(item)
...
def _write_db_csv(tbl_id: str, name: str, rows, columns) -> str:
    ...
    """
    try:
        table_csv_root = _table_csv_root()
...
def _delete_db_csv(name: str, tbl_id: str):
    ...
    v9.0.3: 저장 위치는 DB 루트 최상단. 하위호환 alias 중복을 안전하게 제거.
    """
    try:
        fname = _csv_filename(name, tbl_id)
        roots = []
        for root in (_table_csv_root(), _db_root()):
            try:
                if root.exists() and root.resolve() in [r.resolve() for r in roots]:
                    continue
            except Exception:
                pass
            roots.append(root)
...
def list_db_sources():
    ...
    sources = []
    db_base = _db_root()
    hidden = {p.casefold() for p in _hidden_product_pages()}
    ...
    for f in sorted(db_base.iterdir()):
        if f.is_file() and f.suffix.lower() in (".parquet", ".csv"):
            ...
            sources.append({
                ...
                "name": f.stem, "path": f.name, "file": f.name,
                "label": f"📊 {f.name} (root)",
            })
    for d in sorted(db_base.iterdir()):
        if not d.is_dir():
            continue
        # Runtime/backup folders are implementation artifacts, not product pages.
        # Without this guard `_backups/mltable_knob_gaps_*` appears as a product.
        if d.name.startswith(".") or d.name.startswith("_"):
            continue
        for prod in sorted(d.iterdir()):
            if not prod.is_dir():
                continue
            if prod.name.casefold() in hidden:
                continue
...
@router.post("/tables/save", dependencies=[Depends(_require_tablemap_admin)])
...
@router.post("/tables/delete", dependencies=[Depends(_require_tablemap_admin)])
...
@router.post("/groups/delete", dependencies=[Depends(_require_tablemap_admin)])
def delete_group(group_id: str = Query(...)):
    sid = safe_id(group_id)
    g = load_json(GROUPS_DIR / f"{sid}.json", None) or {}
    for tid in g.get("tables", []):
        tfp = TABLES_DIR / f"{safe_id(tid)}.json"
        t = load_json(tfp, None)
        if not t:
            continue
        t["group_id"] = ""
        save_json(tfp, t, indent=2)
...
@router.post("/db-ref/add", dependencies=[Depends(_require_tablemap_admin)])
...
@router.post("/db-ref/description", dependencies=[Depends(_require_tablemap_admin)])
...
@router.post("/nodes/unlink", dependencies=[Depends(_require_tablemap_admin)])
...
@router.post("/node/position", dependencies=[Depends(_require_tablemap_admin)])
...
@router.get("/tables")
def list_tables():
    """List TableMap tables. 각 테이블에 대해 DB root 단일 파일 auto-link 힌트 포함.
    - base_csv_match: {path, rows, cols, size} | null — DB root 의
...
class ProductPageReq(BaseModel):
    product: str


@router.get("/product-pages")
def product_pages():
    return {"hidden_product_pages": _hidden_product_pages()}


@router.post("/product-pages/hide", dependencies=[Depends(_require_tablemap_admin)])
def hide_product_page(req: ProductPageReq):
    product = _norm_product(req.product)
    if not product:
        raise HTTPException(400, "product required")
    cfg = _load_config()
    hidden = _hidden_product_pages(cfg)
    if product.casefold() not in {p.casefold() for p in hidden}:
        hidden.append(product)
    cfg["hidden_product_pages"] = sorted(hidden, key=lambda x: x.casefold())
    _save_config(cfg)
    return {"ok": True, "hidden_product_pages": cfg["hidden_product_pages"]}


@router.post("/product-pages/delete", dependencies=[Depends(_require_tablemap_admin)])
def delete_product_page(req: ProductPageReq):
    return hide_product_page(req)


@router.delete("/product-pages/delete", dependencies=[Depends(_require_tablemap_admin)])
def delete_product_page_query(product: str = Query("")):
    return hide_product_page(ProductPageReq(product=product))


@router.post("/product-pages/unhide", dependencies=[Depends(_require_tablemap_admin)])
def unhide_product_page(req: ProductPageReq):
    product = _norm_product(req.product)
    if not product:
        raise HTTPException(400, "product required")
    cfg = _load_config()
    hidden = [p for p in _hidden_product_pages(cfg) if p.casefold() != product.casefold()]
    cfg["hidden_product_pages"] = sorted(hidden, key=lambda x: x.casefold())
    _save_config(cfg)
    return {"ok": True, "hidden_product_pages": cfg["hidden_product_pages"]}
...
def list_product_configs():
    """Product YAMLs surfaced inside TableMap so config contracts sit next to
    matching-table contracts.  Products live in data_root/product_config/products.yaml.
    """
    items = _pc.list_products(PC_ROOT)
    for it in items:
        fp = _pc.config_path(PC_ROOT, it.get("product", ""))
        it["file"] = fp.name
        it["path"] = str(fp)
        try:
            it["updated"] = datetime.datetime.fromtimestamp(fp.stat().st_mtime).isoformat()
        except Exception:
            it["updated"] = ""
    return {"configs": items, "template": _pc.TEMPLATE, "schema": _pc.SCHEMA}


@router.get("/product-config")
def get_product_config(product: str = Query(...)):
    product = (product or "").strip()
    if not product:
        raise HTTPException(400, "product required")
    fp = _pc.config_path(PC_ROOT, product)
    text = _pc.load_raw(PC_ROOT, product)
    try:
        config = _pc.parse_text(text)
    except Exception:
...
class ProductConfigSaveReq(BaseModel):
    product: str
    text: str


@router.post("/product-config/save", dependencies=[Depends(_require_tablemap_admin)])
def save_product_config(req: ProductConfigSaveReq):
    product = (req.product or "").strip()
    if not product:
        raise HTTPException(400, "product required")
    try:
        data = _pc.save_raw(PC_ROOT, product, req.text or "")
    except Exception as e:
        raise HTTPException(400, f"YAML parse failed: {e}")
    errs = _pc.validate(data)
    fp = _pc.config_path(PC_ROOT, product)
    return {"ok": not errs, "errors": errs, "product": product,
            "file": fp.name, "path": str(fp)}


class ProductConfigDeleteReq(BaseModel):
    product: str


def _delete_product_config(product: str):
    product = (product or "").strip()
    if not product:
        raise HTTPException(400, "product required")
    products = _pc.load_all(PC_ROOT)
    target = next((k for k in products.keys() if k.casefold() == product.casefold()), "")
    if not target:
        raise HTTPException(404, "product config not found")
    products.pop(target, None)
    _pc.save_all(PC_ROOT, products)
    return {"ok": True, "deleted": target, "configs": _pc.list_products(PC_ROOT)}


@router.post("/product-config/delete", dependencies=[Depends(_require_tablemap_admin)])
def delete_product_config(req: ProductConfigDeleteReq):
    return _delete_product_config(req.product)


@router.delete("/product-config/delete", dependencies=[Depends(_require_tablemap_admin)])
def delete_product_config_query(product: str = Query("")):
    return _delete_product_config(product)


@router.get("/tables/{table_id}/auto-load")
def auto_load_table_from_base(table_id: str, limit: int = Query(500, ge=1, le=5000)):
    """v8.7.6: 같은 이름의 DB root 단일 파일을 자동 로드해 TableMap 에 주입할
...
def _version_file(table_id: str, version: str) -> Path:
    """버전 스냅샷 파일 경로 — 반드시 해당 테이블의 versions 폴더 안이어야 한다.

    `table_id` 만 정제하고 `version` 을 그대로 이어붙이면
    `version=../../../sessions/tokens` 로 data_root 하위 임의 `.json`(세션 토큰,
    계정 파일)을 읽어낼 수 있었다. 파일명은 항상 `v<숫자>` 이므로 resolve 후
    폴더 안에 있는지 확인하면 정상 사용에는 영향이 없다.
    """
    vdir = (VERSIONS_DIR / safe_id(table_id)).resolve()
    vfile = (vdir / f"{version}.json").resolve()
    try:
        vfile.relative_to(vdir)
    except ValueError:
        raise HTTPException(400, "잘못된 version 값입니다")
    return vfile


@router.get("/version-content")
def get_version_content(table_id: str = Query(...), version: str = Query(...)):
    vfile = _version_file(table_id, version)
    if not vfile.exists():
        raise HTTPException(404)
...
@router.post("/versions/rollback", dependencies=[Depends(_require_tablemap_admin)])
def rollback_version(req: RollbackReq):
    """v8.4.4 — 선택 버전으로 롤백. 현재 상태도 rollback 직전 snapshot 으로 보존."""
    sid = safe_id(req.table_id)
    vdir = VERSIONS_DIR / sid
    vfile = _version_file(req.table_id, req.version)
    if not vfile.exists():
...
@router.post("/tables/import", dependencies=[Depends(_require_tablemap_admin)])
