"""routers/catalog.py v7.3 — Unified admin catalog for:
  • Matching tables (CSV)     — data/DB/matching/
  • Reformatter rules (JSON)  — data/holweb-data/reformatter/
  • Product YAML configs       — data/holweb-data/product_config/
...
logger = logging.getLogger("holweb.catalog")
...
MATCH_DIR = PATHS.db_root / "matching"
...
def matching_save(req: MatchSave):
    ...
    try:
        ...
        df.write_csv(fp)
        return {"ok": True, "rows": df.height, "path": str(fp)}
    except Exception as e:
