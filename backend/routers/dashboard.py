    ...
    def _loop(self):
        ...
        while True:
            ...
            try:
                ...
                logger.warning("Chart scheduler ownership check failed", exc_info=True)
...
def start_chart_scheduler() -> None:
    """Start chart refresh after shared background ownership is acquired."""
    _scheduler.start()
...
def multi_db_chart(req: MultiDbChartReq, request: Request):
    ...
    # 활동 대시보드: 어떤 소스/항목을 어떤 색상 분류(knob 등)로 차트를 그렸는지.
    from core.audit import record_user as _audit_user
    _src = req.primary_source + (f"+{req.secondary_source}" if req.secondary_source else "")
    _audit_user(me.get("username") or "user", "dashboard:chart",
                detail=f"type={req.chart_type} source={_src} product={req.product} "
                       f"x={req.x_item} y={req.y_item} color_by={req.color_by} "
                       f"group_by={req.group_by or '(none)'} fit={req.fit}",
                tab="dashboard")
    return result
...
# 제품 선택의 '전체'. 이 값이 오면 제품 필터를 걸지 않고, split 축 대신
# **제품 자체**를 색 구분 축으로 쓴다 — 제품마다 split 열 이름이 달라
# 하나의 split 기준으로 전 제품을 겹쳐 보는 건 의미가 없기 때문이다.
WIP_SPLIT_ALL_PRODUCTS = "ALL"


def _wip_split_is_all(value: str) -> bool:
    return str(value or "").strip().upper() == WIP_SPLIT_ALL_PRODUCTS


def _wip_split_product_name(value: str) -> str:
    """Return the dashboard product name, folding ML_TABLE_<product> aliases."""
    from core.latest_lot_cache_format import normalize_product
    return normalize_product(value).upper()


def _wip_split_product_expr(names: list[str]):
    """물량 집계 제품은 canonical latest-lot의 `product`가 authoritative다.

    FAB는 폴더별로 제품이 구분되고 원천 행에는 product 열이 없다. latest-lot
    생성기가 이미 그 귀속을 `product`로 확정하므로 대시보드가 별도 보조 열인
    `src_product`를 우선하면 운영 캐시의 올바른 제품 구분을 무시하게 된다.
    `src_product`는 product가 없는 과도기 캐시에서만 폴백으로 사용한다.
    """
    folded = {str(name).casefold(): str(name) for name in (names or [])}
    col = folded.get("product") or folded.get("src_product") or "product"
    return (
        pl.col(col).cast(_STR, strict=False).fill_null("").str.strip_chars()
...
def _wip_split_ml_table_path(product: str) -> Path | None:
    product = _wip_split_product_name(product)
...
# 신규 캐싱(SplitTable FAB 매칭 캐시)이 제품마다 들고 있는 구분 축. 원본 ML_TABLE
# 에 KNOB_/MASK_ 열이 아직 없는 신규 제품도 이 축으로는 바로 나눠 볼 수 있다.
_WIP_SPLIT_CACHE_COLS = ("lot_type", "ppid", "eqp_id", "chamber_id", "reticle_id",
                         "process_id", "line_id")
_WIP_SPLIT_CACHE_LABEL = {
    # lot_type 이 맨 앞 — 양산/엔지니어링/모니터 구분이 물량을 볼 때 가장 먼저
    # 나누고 싶은 축이다. FAB 에 열이 없으면 옵션 목록에 아예 안 나온다.
    "lot_type": "랏 구분(lot_type)",
    "ppid": "PPID", "eqp_id": "설비", "chamber_id": "챔버",
    "reticle_id": "레티클", "process_id": "공정", "line_id": "라인",
}


def _wip_split_cache_path(product: str) -> Path | None:
    """이 제품의 FAB 매칭 캐시(신규 캐싱 산출물).

    경로는 SplitTable 이 소유한다 — 여기서 다시 조립하면 캐시 위치가 바뀔 때
    대시보드만 조용히 옛 경로를 보게 된다."""
    product = _wip_split_product_name(product)
    if not product:
        return None
    try:
        from routers.splittable import _match_cache_path
        fp = Path(_match_cache_path(f"ML_TABLE_{product}"))
    except Exception:
        fp = Path(PATHS.data_root) / "splittable" / "match_cache" / f"ML_TABLE_{product}.parquet"
    return fp if fp.is_file() else None


def _wip_split_column_group(col: str) -> str:
    upper = str(col or "").upper()
    for prefix in _WIP_SPLIT_PREFIXES:
        if upper.startswith(prefix):
            return prefix.rstrip("_")
    return "CACHE"


def _wip_split_column_options(product: str) -> tuple[list[dict], Path | None, Path | None]:
    """이 제품에서 고를 수 있는 split 축 목록.

    원본 ML_TABLE 의 KNOB_/MASK_/FAB_ 열과 신규 캐시의 FAB 축을 **함께** 돌려준다.
    신규 제품은 latest cache 에는 떠도 원본 ML_TABLE 에 split 열이 아직 없어서
    '나눌 축이 하나도 없음' 이 되던 문제 때문이다 — 최소한 캐시 축으로는 바로
    나눠 볼 수 있어야 한다. 각 항목의 group 으로 화면에서 구분(KNOB/MASK/FAB/캐시)을
    고른다."""
    options: list[dict] = []
    ml_fp = _wip_split_ml_table_path(product)
    if ml_fp is not None:
        try:
            for col in pl.scan_parquet(ml_fp).collect_schema().names():
                if str(col).upper().startswith(_WIP_SPLIT_PREFIXES):
                    options.append({
                        "col": col,
                        "group": _wip_split_column_group(col),
                        "label": col,
                        "source": "ml_table",
                    })
        except Exception:
            logger.warning("wip-split ML_TABLE schema read failed", exc_info=True)
    cache_fp = _wip_split_cache_path(product)
    if cache_fp is not None:
        try:
            names = {str(c).lower() for c in pl.scan_parquet(cache_fp).collect_schema().names()}
            for col in _WIP_SPLIT_CACHE_COLS:
                if col in names:
                    options.append({
                        "col": col,
                        "group": "CACHE",
                        "label": f"{_WIP_SPLIT_CACHE_LABEL.get(col, col)} ({col})",
                        "source": "match_cache",
                    })
        except Exception:
            logger.warning("wip-split match cache schema read failed", exc_info=True)
    return options, ml_fp, cache_fp
...
def _wip_split_nearest_step_desc(step_id: str, candidates: list[tuple[str, int, str]]) -> str:
    ...
    - 번호부 000000(AA000000/A000000 등)은 F/O(fab out) — 공정의 끝이므로
      999999 로 간주해 마지막 main step 그룹에 귀속한다 (맨 앞 매칭 금지)."""
...
def _wip_split_exclude_prefixes(raw: str) -> list[str]:
    """"Z" / "Z,Y" 처럼 들어온 제외 문자열을 대문자 prefix 목록으로.
...
def _wip_split_latest_cache(product: str = "") -> tuple[pl.DataFrame, str, list[str], Path]:
    ...
    # Only the SplitTable-owned canonical cache is authoritative. Legacy and
    # scanner-internal files stay on disk for rollback, but are not combined.
    ...
    candidates = [fp]
    infos: list[tuple[Path, list[str]]] = []
    errors: list[str] = []
    ...
    if not infos:
        raise HTTPException(500, "latest cache 읽기 실패: " + "; ".join(errors[:3]))

    # ML_TABLE_<product> is a source/cache alias, not a separate dashboard
    # product.  Keeping it as a separate choice made the WIP rows visible but
    # attempted to join ML_TABLE_ML_TABLE_<product>.parquet, so every split was
    # shown as unassigned.
    product_map: dict[str, str] = {}
    for _fp, values in infos:
        for value in values:
            canonical = _wip_split_product_name(value)
            if canonical:
                product_map.setdefault(canonical.upper(), canonical)
    products = sorted(product_map.values())
    if _wip_split_is_all(product):
        # '전체'는 제품 필터 없이 캐시 전체를 그대로 쓴다.
        try:
            frame = pl.read_parquet(fp)
        except Exception as exc:
            raise HTTPException(500, f"latest cache 읽기 실패: {exc}")
        return frame, WIP_SPLIT_ALL_PRODUCTS, products, fp
    query = _wip_split_product_name(product).upper()
    if not query:
        selected = products[0] if products else ""
        query = selected.upper()
    elif query in product_map:
        selected = product_map[query]
    else:
        raise HTTPException(400, f"product '{product}' 가 latest cache에 없습니다. 선택 가능: {products}")

    chosen = next((
        fp for fp, values in infos
        if any(_wip_split_product_name(v).upper() == query for v in values)
    ), None)
    if chosen is None:
...
def _wip_split_prepare(product: str, bin_size: int, split_col: str, axis: str,
    ...
    exclude_root_prefix 는 UI 톱니바퀴 설정값이다. 기본값은 서버가 아니라
    화면이 들고 있다 — 여기서 기본 제외를 걸면 파라미터를 안 보내는 다른
    호출자까지 조용히 필터되기 때문이다."""
    from core import lot_progress_cache
    ...
    canonical_product = raw_product.str.replace(r"(?i)^ML_TABLE_", "").str.to_uppercase()
    all_products = _wip_split_is_all(product)
    if all_products:
        # 전체 보기 — 제품 필터는 걸지 않되, 제품이 곧 색 구분 축이 되므로
        # 어느 제품에 귀속되는지는 여기서 한 번에 정해 둔다.
        cur = cur.with_columns(canonical_product.alias("_product_key"))
        cur = cur.filter(pl.col("_product_key") != "")
    else:
    ...
    dedup_keys = [c for c in ("root_lot_id", "wafer_id") if c in cur.columns]
    if len(dedup_keys) == 2 and cur.height:
        if all_products:
            # 전체 보기의 중복 제거 키에는 제품이 들어가야 한다 — 제품별로
            # 세는 화면에서 (root, wafer) 만으로 접으면 한 제품의 물량이
            # 다른 제품 행에 먹힌다.
            dedup_keys = ["_product_key", *dedup_keys]
            alias_rank = pl.lit(1, dtype=pl.Int8)
        else:
            alias_rank = (
                (raw_product.str.to_uppercase() == str(product).upper()).cast(pl.Int8)
            )
        cur = cur.with_columns(alias_rank.alias("_product_alias_rank"))
        order_cols = [c for c in ("tkout_time", "update_time") if c in cur.columns]
        cur = (
            cur.sort([*order_cols, "_product_alias_rank"])
            .unique(subset=dedup_keys, keep="last", maintain_order=True)
            .drop("_product_alias_rank")
        )

    # lot_type 은 FAB 원본 값 그대로라 앞뒤 공백과 대소문자 표기가 섞여 들어온다.
    # 정규화하지 않으면 "BUILD" / "BUILD " / "build" 가 서로 다른 값이 되어 필터
    # 목록에 **눈으로 똑같은 항목이 여러 번** 뜨고(반복 표시), 그중 하나를 골라도
    # 다른 철자의 wafer 는 통째로 걸러진다. 표시는 처음 만난 철자를 쓰고 비교는
    # 정규화 키로 한다. 목록은 필터를 걸기 **전에** 만들어 선택 후에도 유지된다.
    lot_types: list[str] = []
    lot_type_col = next((c for c in cur.columns if str(c).casefold() == "lot_type"), "")
    if lot_type_col:
        ...
        # polars 의 unique() 는 순서를 보장하지 않는다 — 먼저 정렬해서 어느 철자를
        # 대표로 쓸지 고정한다. 안 그러면 새로고침할 때마다 라벨이 BUILD/build 로
        # 널뛴다.
        distinct = {str(v or "").strip() for v in cur.get_column("lot_type").unique().to_list()}
        for text in sorted(t for t in distinct if t):
            spellings.setdefault(text.casefold(), text)
        lot_types = sorted(spellings.values(), key=lambda v: v.casefold())
        wanted = str(lot_type or "").strip()
        if wanted and wanted.upper() != "ALL":
            cur = cur.filter(pl.col("lot_type").str.to_lowercase() == wanted.casefold())

    # root_lot_id prefix 제외 — 엔지니어링/더미 lot 처럼 물량 통계에서 빼고 보는
    # 랏을 화면 설정으로 끊는다. 대소문자 무관, 여러 문자는 콤마로.
    # 값이 없는 root 는 어떤 prefix 로도 시작하지 않으므로 남긴다(null → "").
    exclude_prefixes = _wip_split_exclude_prefixes(exclude_root_prefix)
    excluded_wafers = 0
    if exclude_prefixes and "root_lot_id" in cur.columns:
        before = int(cur.height)
        root_u = (pl.col("root_lot_id").cast(_STR, strict=False)
                    .fill_null("").str.strip_chars().str.to_uppercase())
        hit = None
        for p in exclude_prefixes:
            e = root_u.str.starts_with(p)
            hit = e if hit is None else (hit | e)
        cur = cur.filter(~hit)
        excluded_wafers = before - int(cur.height)
    ...
    UNASSIGNED = "(미지정)"
    if all_products:
        # 전체 보기는 split 조인을 하지 않는다 — 색 구분 축이 제품이다.
        cur = cur.with_columns(
            pl.when(pl.col("_product_key") == "")
            .then(pl.lit(UNASSIGNED))
            .otherwise(pl.col("_product_key"))
            .alias("_split")
        )
        return {
            "cur": cur, "product": WIP_SPLIT_ALL_PRODUCTS, "products": products,
            "bin_size": bin_size, "axis": axis,
            "split_col": "", "split_cols": [], "split_options": [],
            "split_source": "product", "has_ml_table": False, "has_match_cache": False,
            "split_join_ok": True, "unassigned": UNASSIGNED,
            "exclude_root_prefix": ",".join(exclude_prefixes),
            "excluded_wafers": excluded_wafers,
            "lot_types": lot_types,
        }

    # ML_TABLE split 조인 — (root_lot_id, wafer_id) 키. lot_id 는 child lot 재편성
    # 때문에 조인 키로 쓰지 않는다.
    split_options, ml_fp, cache_fp = _wip_split_column_options(product)
    split_cols = [opt["col"] for opt in split_options]
    split_join_ok = False
    if split_col and split_col not in split_cols:
        split_col = ""
    if not split_col:
        # 기본은 지금까지처럼 KNOB — 없으면(신규 제품) 첫 축.
        split_col = next((c for c in split_cols if c.upper().startswith("KNOB_")),
                         split_cols[0] if split_cols else "")
    chosen = next((opt for opt in split_options if opt["col"] == split_col), None)

    if chosen and chosen["source"] == "match_cache" and cache_fp is not None:
        # 신규 캐시는 root/wafer 를 __cache_* 열로 들고 있고, 한 wafer 가 여러 step
        # 행을 가진다 — 최신 행(ts) 하나만 남겨 wafer 당 하나의 split 값을 만든다.
        try:
            lf = pl.scan_parquet(cache_fp)
            names = lf.collect_schema().names()
            root_col = next((c for c in names if c.lower() == "__cache_root_lot_id"), "")
            wafer_col = next((c for c in names if c.lower() == "__cache_wafer_id"), "")
            value_col = next((c for c in names if c.lower() == split_col.lower()), "")
            ts_col = next((c for c in names if c.lower() == "__cache_ts"), "")
            if root_col and wafer_col and value_col:
                sel = [
                    pl.col(root_col).cast(_STR, strict=False).str.to_uppercase().alias("_root"),
                    pl.col(wafer_col).cast(_STR, strict=False).str.strip_chars().alias("_wf"),
                    pl.col(value_col).cast(_STR, strict=False).alias("_split"),
                ]
                if ts_col:
                ...
                # 지킬 의무가 없어 "가장 최신 __cache_ts 행" 대신 아무 행의 값이
                # 뽑힐 수 있다. lot_type 처럼 step 마다 값이 다를 수 있는 축은
                # 그때 화면에 엉뚱한 값이 뜬다.
    ...
    # the chart kept whitespace/empty ML_TABLE values while the drill endpoint
    # stripped the query value, so those visible segments filtered to zero rows.
    split_text = pl.col("_split").cast(_STR, strict=False).fill_null("").str.strip_chars()
    cur = cur.with_columns(
        pl.when(split_text == "")
        .then(pl.lit(UNASSIGNED))
        .otherwise(split_text)
        .alias("_split")
    )
    return {
        "cur": cur, "product": product, "products": products, "bin_size": bin_size,
        "axis": axis, "split_col": split_col, "split_cols": split_cols,
        # 화면의 '구분' 선택용 — 각 축이 어느 그룹(KNOB/MASK/FAB/CACHE)이고 어느
        # 소스(원본 ML_TABLE / 신규 캐시)에서 왔는지.
        "split_options": split_options,
        "split_source": (chosen or {}).get("source") or "",
        "has_ml_table": ml_fp is not None,
        "has_match_cache": cache_fp is not None,
        "split_join_ok": split_join_ok, "unassigned": UNASSIGNED,
        "exclude_root_prefix": ",".join(exclude_prefixes),
        "excluded_wafers": excluded_wafers,
        "lot_types": lot_types,
    }
...
def wip_split_summary(
    ...
    axis:
      ...
      - step_desc (기본): latest cache 의 step_id 를 Vehicle_matching.csv 로 매핑한
        ...
        번호부가 000000 인 step(AA000000/A000000 등)은 F/O(fab out) —
        999999 로 간주해 마지막 main step 그룹에 귀속한다 (맨 앞 매칭 금지).

    exclude_root_prefix: 해당 문자로 시작하는 root_lot_id 를 집계에서 제외
    ...
    split_col, split_cols = ctx["split_col"], ctx["split_cols"]
    split_join_ok = ctx["split_join_ok"]
    ...
    return {
        ...
        "split_options": ctx["split_options"],
        "split_source": ctx["split_source"],
        "has_ml_table": ctx["has_ml_table"],
        "has_match_cache": ctx["has_match_cache"],
        "split_values": split_values,
        "total_wafers": int(cur.height),
        "matched_wafers": matched,
        "unassigned_label": UNASSIGNED,
        "exclude_root_prefix": ctx["exclude_root_prefix"],
        "excluded_wafers": ctx["excluded_wafers"],
        "bins": bins,
...
def wip_split_lots(
    ...
    """특정 구간(+split 값)의 root_lot_id/wafer_id 목록 — 차트 세그먼트 드릴다운.

    wip-split 과 같은 슬라이스에서 요청한 bin(과 split 값)에 속한 wafer 만 골라
    반환한다. summary 와 같은 전처리를 거치고 필터만 더하므로 비용도 같다.

    bin: step_id 축은 구간 시작값(미해석은 -1), step_desc 축은 그룹 라벨
    (예: "1.0", 미해석은 "미해석"). split 을 비우면 구간 전체.

    exclude_root_prefix 는 summary 와 같은 값을 받아야 목록 합계가 차트와 맞는다."""
    _require_dashboard_section(request, "charts")
    ctx = _wip_split_prepare(product, bin_size, split_col, axis, exclude_root_prefix, lot_type)
    cur = ctx["cur"]
    axis = ctx["axis"]
    ...
    if axis == "step_id":
        if b in ("-1", UNPARSED):
            cur = cur.filter(pl.col("_bin").is_null())
        else:
            try:
                cur = cur.filter(pl.col("_bin") == int(float(b)))
            except Exception:
                raise HTTPException(400, f"bin '{bin}' 을 해석할 수 없습니다.")
    else:
        if not b or b == UNPARSED:
            cur = cur.filter(
                pl.col("_bin_label").is_null()
                | (pl.col("_bin_label").str.strip_chars() == ""))
        else:
            cur = cur.filter(pl.col("_bin_label") == b)
    sv = str(split or "").strip()
    if sv:
        cur = cur.filter(pl.col("_split") == sv)

    try:
        limit = max(1, min(20000, int(limit or 3000)))
    except Exception:
        limit = 3000
    cols = [c for c in ("root_lot_id", "lot_id", "wafer_id", "step_id") if c in cur.columns]
    if not cols:
        raise HTTPException(500, "latest cache에 lot/wafer 식별 열이 없습니다.")
    uniq = cur.select(cols).unique().sort(cols)
    total = int(uniq.height)
    rows = uniq.head(limit).to_dicts()
    # 활동 대시보드: 어떤 구간(+split 값)을 드릴다운했는지.
    from core.audit import record as _audit_record
    _audit_record(request, "dashboard:wip_split_drill",
                  detail=f"product={ctx['product']} split_col={ctx['split_col'] or '(none)'} "
                         f"bin={b} split={sv or '(all)'} wafers={total}",
                  tab="dashboard")
    return {
        "ok": True,
        "product": ctx["product"],
