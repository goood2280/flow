...
def _chart_builder_reformatter_frame(
    ...
    *,
    ...
    warnings = [str(item) for item in (reformatted.get("rule_errors") or []) if str(item).strip()]
...
def _chart_assistant_deterministic_operations(
    ...
    if join_context:
        how = next((value for value in _CHART_ASSISTANT_JOIN_HOWS if re.search(rf"\b{value}\b", folded)), "")
...
def chart_builder_run(req: ChartBuilderRunReq, request: Request):
    """Run multiple read-only FileBrowser SQL queries and join their bounded results."""
    me = current_user(request)
    ...
    for idx, source in enumerate(sources, start=1):
        ...
        if source.apply_reformatter:
            ...
            try:
                ...
                where = _combine_where(
                    _normalize_view_sql_filter(normalized, all_columns, schema),
                    _duckdb_valid_wafer_where(all_columns),
                )
    ...
    _fb_audit(
        request,
        "chartbuilder:run",
        detail=f"user={me.get('username') or ''} sources={','.join(source_order)} joins={len(join_evidence)} rows={joined.height}",
        tab="chartbuilder",
    )
    result = {
        "ok": True,
