"""routers/monitor.py v9.2.1 — 시스템 모니터 라우터.
...
class LoadScheduleReq(BaseModel):
    enabled: bool = True
    time: str = "11:00"
...
@mon_router.get("/load/schedule")
def load_schedule(_admin=Depends(require_admin)):
    return sysmon.get_schedule()


@mon_router.put("/load/schedule")
def update_load_schedule(req: LoadScheduleReq, _admin=Depends(require_admin)):
    try:
        return sysmon.save_schedule(enabled=req.enabled, at=req.time)
