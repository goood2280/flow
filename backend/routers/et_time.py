...
PGM(pt) 라벨 규칙 — auto report Main.py 와 동일:
  ...
  GET /api/et-time/lots?product=&col=&prefix=  lot 후보 (col 기본 root_lot_id)
...
def et_time_lots(request: Request, product: str = Query(""), prefix: str = Query(""),
                 col: str = Query("root_lot_id"), limit: int = Query(200)):
    """제품의 lot 후보. col 기본값이 root_lot_id 라 SplitTable 의 ROOT LOT ID
    드롭다운과 같은 목록이 나온다 (예전엔 fab_lot_id/lot_id 가 섞여 나왔다).
    col="" 이면 root/fab/lot 3종 전부.
    """
    from core.lot_step import ET_ROOT, lot_id_candidates
    current_user(request)
    if not str(product or "").strip():
        return {"requires_product": True, "candidates": []}
    cols = [c.strip() for c in str(col or "").split(",") if c.strip()]
    if len(cols) == 1 and cols[0].casefold() == "root_lot_id":
        # ET 원천을 다시 unique scan하지 않고 SplitTable/Inform이 공유하는
        # 제품별 RAM + 디스크 Root Lot 후보 풀을 그대로 사용한다. 캐시가 준비
        # 중이면 complete/provisional/lookup_cache 메타도 유지해 프런트가 직접
        # 입력을 막지 않은 채 완성 목록을 재확인할 수 있게 한다.
        from routers import splittable as splittable_router

        return splittable_router.get_lot_candidates(
            product=str(product or "").strip(),
            col="root_lot_id",
            prefix=str(prefix or ""),
            limit=max(1, min(50000, int(limit or 200))),
            source="auto",
            root_lot_id="",
        )
    ...
    return {"col": col, "candidates": candidates or []}
...
def et_time_trend(request: Request, product: str = Query(""), months: int = Query(0)):
    ...
    # 활동 대시보드: 어떤 제품의 추이를 어떤 기간(months)으로 봤는지.
    from core.audit import record_user as _audit_user
    _audit_user(me.get("username", ""), "et-time:trend",
                detail=f"product={product_text} months={months or 'all'} steps={len(steps)}",
                tab="ettime")
...
def et_time_measure(request: Request, product: str = Query(""),
    ...
    # 활동 대시보드: 어떤 제품/랏의 측정시간을 조회했는지.
    from core.audit import record_user as _audit_user
    _audit_user(me.get("username", ""), "et-time:measure",
                detail=f"product={product_text} root_lot_id={root_text or lot_text} "
                       f"steps={len(step_totals)} pgm={len(rows)}",
                tab="ettime")
    return {
