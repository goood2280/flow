...
# 신규 인폼 등록 마법사는 한 랏의 스냅샷을 여러 번 다시 그린다(미리보기 → 항목
# 수정 → 표시형식 변경 → 제출). 20초는 그 리듬보다 짧아서 거의 매번 SplitTable
# 파이프라인을 통째로 다시 돌렸다 — 사내 랏에서 이게 "스냅샷이 오래 걸린다"의
# 대부분이다. 키에 제품·랏·항목·표시형식이 모두 들어가므로 늘려도 다른 조건의
# 결과와 섞이지 않는다. 1분이면 방금 저장한 plan 도 곧 반영된다.
SPLITTABLE_SNAPSHOT_CACHE_TTL_SEC = 60
...
# 매칭 룰북(KNOB/INLINE/VM) 메타 — 제품별로 CSV 를 다시 읽어 조립하는 비용이라
# 스냅샷마다 반복할 이유가 없다. 룰북이 바뀌면 최대 이 시간만큼 늦게 반영된다.
_STEP_METAS_CACHE_TTL_SEC = 60.0
_STEP_METAS_CACHE: dict[str, tuple[float, dict]] = {}
_STEP_METAS_LOCK = threading.Lock()
...
_INFORMS_MUTATION_LOCK = threading.RLock()
_INFORM_POSTPROCESS_STARTED = False
_INFORM_POSTPROCESS_THREAD: threading.Thread | None = None
...
_cache_sweeper.register_ttl_dict("informs._STEP_METAS_CACHE", _STEP_METAS_CACHE,
                                 _STEP_METAS_CACHE_TTL_SEC, lock=_STEP_METAS_LOCK)
...
# 메일 하단 링크 — 인폼 톱니바퀴에서 관리자가 관리한다. [{"label","url"}]
MAIL_LINKS_MAX = 12
DEFAULT_MAIL_MAX_MB = 2.0
MAIL_MAX_MB_CEILING = 20.0


def _clean_mail_links(raw: Any) -> list[dict]:
    out: list[dict] = []
    ...
    for item in raw:
        ...
        if not url:
            continue
        # javascript:/data: 같은 스킴은 메일 본문에 넣지 않는다.
        if not re.match(r"^(https?://|mailto:|/)", url, flags=re.I):
            url = "http://" + url.lstrip("/")
        label = str(item.get("label") or "").strip()[:120] or url
        out.append({"label": label, "url": url})
        if len(out) >= MAIL_LINKS_MAX:
            break
    return out


def _clean_mail_max_mb(raw: Any) -> float:
    try:
        value = float(raw)
    except Exception:
        return DEFAULT_MAIL_MAX_MB
    if value <= 0:
        return DEFAULT_MAIL_MAX_MB
    return round(min(value, MAIL_MAX_MB_CEILING), 2)


def _mail_content_max_bytes() -> int:
    """메일 본문+첨부 한도(byte). 톱니바퀴 설정값이 우선."""
    try:
        return int(_clean_mail_max_mb(_load_config().get("mail_max_mb")) * 1024 * 1024)
    except Exception:
        return int(DEFAULT_MAIL_MAX_MB * 1024 * 1024)
...
def _ml_table_catalog_products() -> list[str]:
    """Return products that are selectable even before latest-lot is ready.

    신규 제품의 latest cache가 아직 빌드 중이거나 한 제품만 실패하면 LOT progress
    cache만 보는 Inform product 목록도 같이 사라졌다. ML_TABLE 원본 파일은 제품의
    존재를 확정하는 독립 원천이므로, 행을 읽지 않는 파일 discovery 결과를 함께 쓴다.
    """
    try:
        from core.ml_table_lookup import discover_ml_table_files
        files = discover_ml_table_files()
    except Exception:
        logger.warning("inform ML_TABLE product discovery failed", exc_info=True)
        return []
    out: list[str] = []
    seen: set[str] = set()
    for fp in files:
        name = _canonical_product(Path(fp).stem)
        key = name.casefold()
...
def _merged_catalog_products(extra: list[str] | None = None) -> list[str]:
    """Return cache-backed products plus ML_TABLE products awaiting latest cache."""
    out: dict[str, str] = {}
    for raw in [*_lot_progress_cache_products(), *_ml_table_catalog_products()]:
        name = _canonical_product(raw)
        if name:
            out.setdefault(name.casefold(), name)
    return sorted(out.values(), key=lambda s: s.casefold())
...
def _save(items: list) -> None:
    global _INFORMS_CACHE_SIG, _INFORMS_CACHE_ITEMS
    # Registration rewrites the canonical array. Compact JSON substantially
    # reduces that durable transaction once embeds and histories have grown.
    save_json(INFORMS_FILE, items, indent=None)
    try:
        st = INFORMS_FILE.stat()
...
def _provisional_lot_identity_snapshot(
    lot_id: str,
    requested_wafer_id: str,
    embed: Optional[dict],
    explicit_fab_lot_id: str = "",
) -> dict[str, Any]:
    """Build the create-response identity without opening ML_TABLE data.

    The full FAB/root/wafer resolution is performed by a post-response task.
    Values already supplied by the browser or embedded SplitTable preview are
    safe to expose immediately and cover the normal single-LOT path.
    """
    lot = str(lot_id or "").strip()
    requested_wafers = _split_saved_ids(requested_wafer_id)
    explicit_fab = _first_saved_id(explicit_fab_lot_id)
    fab = explicit_fab or (lot if _looks_like_fab_lot(lot) else "")
    embed_meta = _embed_lot_identity(embed, fab)
    root = str(embed_meta.get("root_lot_id") or "").strip()
    if not root:
        root = _root_lot_from_values("" if fab else lot, embed)
    if not root and lot:
        root = _root_lot_from_values(lot)
    wafers = list(dict.fromkeys(requested_wafers or (embed_meta.get("wafer_ids") or [])))
    return {
        "root_lot_id": root,
        "root_lot_ids": [root] if root else [],
        "wafer_id": ", ".join(wafers),
        "wafer_ids": wafers,
        "fab_lot_id": str(explicit_fab_lot_id or fab).strip(),
        "source": "request_or_embed",
    }
...
class InformCreate(BaseModel):
    ...
    # Multi-LOT registration may defer the expensive SplitTable snapshot until
    # after the durable Inform row has been returned to the browser.
    snapshot_request: Optional[dict] = None
...
class ConfigReq(BaseModel):
    ...
    # 메일 하단 링크 [{"label","url"}] / 메일 총 용량 한도(MB).
    mail_links: Optional[List[dict]] = None
    mail_max_mb: Optional[float] = None
...
class SplitTableSnapshotReq(BaseModel):
    ...
    # SplitTable 의 "적용 공정 정보" 와 같은 표기 — 항목명을 연결 step_id 로 바꾼다.
    show_step_ids: bool = False
...
def _sanitize_embed_table(embed_table: Optional[EmbedTable | dict]) -> Optional[dict]:
    if not embed_table:
        return None
    if isinstance(embed_table, dict):
        raw = embed_table
    else:
        raw = {
            "source": embed_table.source,
            "columns": embed_table.columns,
            "rows": embed_table.rows,
            "note": embed_table.note,
            "st_view": embed_table.st_view,
            "st_scope": embed_table.st_scope,
            "attached_sets": embed_table.attached_sets,
        }
    attached_sets = _sanitize_attached_sets(raw.get("attached_sets") or [])
    if not (raw.get("columns") or raw.get("rows") or raw.get("st_view") or raw.get("st_scope") or attached_sets):
        return None
    cols = [str(c) for c in (raw.get("columns") or [])][:80]
    rows = []
    for r in (raw.get("rows") or [])[:200]:
        rows.append([str(x) if x is not None else "" for x in (r or [])[:80]])
    out = {
        "source": str(raw.get("source") or "").strip()[:160],
        "columns": cols,
        "rows": rows,
        "note": str(raw.get("note") or "").strip()[:500],
        "st_view": raw.get("st_view") or None,
        "st_scope": raw.get("st_scope") or None,
        "attached_sets": attached_sets,
    }
    # 서버가 생성한 스냅샷의 표시 계약도 함께 보존한다.
    if raw.get("display_mode"):
        out["display_mode"] = _splittable_snapshot_display_mode(raw.get("display_mode"))
    if raw.get("step_labels"):
        out["step_labels"] = True
    if isinstance(raw.get("applied_processes"), list):
        out["applied_processes"] = [dict(item) for item in raw["applied_processes"] if isinstance(item, dict)][:200]
    return out
...
def save_config_endpoint(req: ConfigReq, _admin=Depends(require_page_manager("inform"))):
    ...
    if req.mail_links is not None:
        cfg["mail_links"] = _clean_mail_links(req.mail_links)
    if req.mail_max_mb is not None:
        cfg["mail_max_mb"] = _clean_mail_max_mb(req.mail_max_mb)
    # de-dup 유지 순서
...
def _splittable_snapshot_cache_key(req: SplitTableSnapshotReq) -> str:
    payload = {
        ...
        "show_step_ids": bool(req.show_step_ids),
    }
...
def _splittable_snapshot_display_mode(value: Any) -> str:
    """스냅샷 표시 형식 — SplitTable 화면의 3종(cell/split/merged)과 같은 이름 규약."""
    mode = str(value or "matrix").strip().lower()
    if mode in {"split_check", "merged"}:
        return mode
    return "matrix"


# ── 적용 공정 표기 (SplitTable "적용 공정 정보" 와 같은 규약) ───────────────
#   KNOB        → 한 rule_order 안의 조건 step_id 를 `A & B` 로 묶어 줄마다 하나
#   INLINE / VM → `step_id | item_id`
# step_id 가 없는 매칭 행은 표시 대상이 아니라 스냅샷에서 뺀다 (화면과 동일).
def _step_match_kind(param: str) -> str:
...
def _step_meta_lookup(meta_map: dict, param: str, prefix: str) -> dict:
    if not param or not isinstance(meta_map, dict):
        return {}
    full = str(param or "").strip()
    tail = re.sub(rf"^{prefix}_", "", full, flags=re.I).strip()
    ...
    for k, v in meta_map.items():
        ...
        if key in {full.casefold(), tail.casefold()} and isinstance(v, dict):
            return v
    return {}


def _group_step_ids(group: dict) -> list[str]:
...
def _knob_step_lines(groups: list) -> list[str]:
    ...
    for order in order_seq:
        ...
        for g in by_order[order]:
            operator = re.sub(r"[\s-]+", "_", str(g.get("operator") or "").strip().lower())
            if operator == "is_null":
                continue
            ...
            bucket = by_desc.setdefault(desc_key, {"step_desc": desc, "step_ids": [], "seen": set()})
            for sid in _group_step_ids(g):
                sid_key = sid.casefold()
                if sid_key not in bucket["seen"]:
                    bucket["seen"].add(sid_key)
                    bucket["step_ids"].append(sid)
        desc_groups = [item for item in by_desc.values() if item["step_ids"]]
        if not desc_groups:
            continue
        # 같은 step_desc 의 복수 step은 대체 후보라 줄바꿈, 서로 다른
        # step_desc 조건만 AND 관계라 `&`를 별도 줄에 둔다 (SplitTable과 동일).
...
def _step_item_lines(meta: dict) -> list[str]:
    out: list[str] = []
    seen: set[str] = set()
    fallback_item = str((meta or {}).get("item_id") or "").strip()

    def push(sid: Any, item_id: Any) -> None:
        s = str(sid or "").strip()
        if not s:
            return
...
def _step_lines_for_param(param: str, metas: dict) -> tuple[str, list[str]]:
    kind = _step_match_kind(param)
    if not kind:
        return "", []
    if kind == "knob_ppid":
        meta = _step_meta_lookup(metas.get("knob") or {}, param, "KNOB")
        return kind, _knob_step_lines((meta or {}).get("groups") or [])
    if kind == "inline_matching":
        meta = _step_meta_lookup(metas.get("inline") or {}, param, "INLINE")
        return kind, _step_item_lines(meta)
    meta = _step_meta_lookup(metas.get("vm") or {}, param, "VM")
    return kind, _step_item_lines(meta)


def _load_step_metas(product: str) -> dict:
    """제품의 KNOB/INLINE/VM 매칭 메타. 제품별로 짧게 캐시한다 (읽기 전용으로만 쓸 것)."""
    prod = str(product or "").strip()
    out = {"knob": {}, "inline": {}, "vm": {}}
    if not prod:
        return out
    now = time.time()
    with _STEP_METAS_LOCK:
        hit = _STEP_METAS_CACHE.get(prod)
        if hit and now - hit[0] <= _STEP_METAS_CACHE_TTL_SEC:
            return hit[1]
    try:
    ...
    for key, fn in (("knob", _build_knob_meta), ("inline", _build_inline_meta), ("vm", _build_vm_meta)):
        try:
            ...
            out[key] = {}
    with _STEP_METAS_LOCK:
        _STEP_METAS_CACHE[prod] = (time.time(), out)
    return out


def _shallow_embed_copy(embed: dict) -> dict:
    """embed 를 **얕게** 복제한다 — 겉 dict 와 st_view / st_scope 만 새로 만든다.

    왜 deepcopy 가 아닌가: 스냅샷 변환기들은 행 dict 에 키를 더하거나 rows 를
    통째로 갈아끼울 뿐 **셀은 건드리지 않는다.** 그런데 deepcopy 는 (항목 × wafer)
    셀 dict 을 전부 새로 만든다 — 신규 인폼 등록에서 스냅샷이 오래 걸리던 몫의
    상당 부분이 이 복사였다.

    왜 원본을 제자리에서 고치면 안 되는가: 원본 st_view 의 행/셀은 SplitTable
    view 캐시가 들고 있는 객체일 수 있다. 그래서 **행 dict 은 변환기에서 새로
    만들고**(`{**row, ...}`), 셀은 읽기만 하며 공유한다.
    """
    out = dict(embed)
    st_view = embed.get("st_view")
    out["st_view"] = dict(st_view) if isinstance(st_view, dict) else {}
    scope = embed.get("st_scope")
    out["st_scope"] = dict(scope) if isinstance(scope, dict) else {}
    return out
...
def _apply_step_labels_to_embed(embed: dict, product: str) -> dict:
    ...
    못하는 환경에서도 적용 공정 내용이 사라지지 않게 한다.
    """
    if not isinstance(embed, dict):
        return embed
    st_view = embed.get("st_view")
    if not isinstance(st_view, dict) or not isinstance(st_view.get("rows"), list):
        return embed
    ...
    # 메타를 재조회하지 않고 당시 선택 내용을 확인할 수 있게 한다.
    if applied_processes:
        st["applied_processes"] = applied_processes
    ...
    for row in rows_out:
        ...
        for idx, _header in enumerate(headers):
            ...
            plan = cell.get("plan") if isinstance(cell, dict) else None
            actual_text = "" if actual is None else str(actual)
            plan_text = "" if plan is None else str(plan)
            if plan_text and actual_text and plan_text != actual_text:
                legacy_row.append(f"{actual_text} → {plan_text}")
            elif plan_text and actual_text and plan_text == actual_text:
                legacy_row.append(f"✓ {plan_text} (plan 적용)")
            else:
...
def _merge_view_allowed_param(param: str) -> bool:
    u = str(param or "").strip().upper()
    return any(u == p or u.startswith(f"{p}_") for p in _MERGE_VIEW_PREFIXES)


def _convert_splittable_embed_to_merged(embed: dict) -> dict:
    """행에서 옆칸과 값이 같으면 하나로 묶어 표시(읽기 전용) — SplitTable '병합' 형식."""
    if not isinstance(embed, dict):
        return embed
    out = _shallow_embed_copy(embed)
    st_view = out["st_view"]
    ...
    if not st_view or not headers:
        return out
    # 화면과 같이 미진행 여부가 다르면 같은 값이어도 묶지 않는다 (회색 구간이 뭉개지지 않게).
    ...
    for row in st_view.get("rows") or []:
        ...
        for ci in range(len(headers)):
            ...
            gray = not_reached.cell(param, ci)
            if runs and runs[-1]["value"] == value and runs[-1].get("not_reached") == gray:
                runs[-1]["span"] += 1
            else:
                runs.append({"start": ci, "span": 1, "value": value, "not_reached": gray})
        # 원본 행은 SplitTable view 캐시의 객체일 수 있어 제자리에서 고치지 않는다.
        merged_rows.append({**row, "_merged_runs": runs})
    st_view["rows"] = merged_rows
    st_view["display_mode"] = "merged"
    out["st_view"] = st_view
    out["display_mode"] = "merged"
    scope = out.get("st_scope") if isinstance(out.get("st_scope"), dict) else {}
    scope["display_mode"] = "merged"
    out["st_scope"] = scope
    return out
...
def _image_upload_ext(filename: str, content_type: str = "") -> str:
    ...
    if mime in clipboard_mimes:
        return clipboard_mimes[mime]
...
def _build_inform_entry(
    ...
    *,
    ...
    if defer_enrichment:
        identity_snapshot = _provisional_lot_identity_snapshot(
            inherit_lot,
            requested_wafer_id,
            embed,
    ...
    fab_snapshot = (
        ...
        or ("" if defer_enrichment else _resolve_fab_lot_snapshot(inherit_product, root_lot, requested_wafer_id))
    ...
    entry = {
        ...
        "text": (
            sanitize_rich_html(req.text)
            if str(req.text_format or "").lower() == "html"
            else (req.text or "").strip()
        ),
        "text_format": "html" if str(req.text_format or "").lower() == "html" else "plain",
        "author": me["username"],
        "created_at": now,
        "checked": False,
        "checked_by": "",
    ...
    if defer_enrichment:
        entry["postprocess_pending"] = True
        if isinstance(req.snapshot_request, dict) and req.snapshot_request:
            entry["snapshot_request"] = {
                "custom_cols": [str(v).strip() for v in (req.snapshot_request.get("custom_cols") or []) if str(v or "").strip()],
                "is_fab_lot": bool(req.snapshot_request.get("is_fab_lot")),
                "display_mode": _splittable_snapshot_display_mode(req.snapshot_request.get("display_mode")),
                "show_step_ids": bool(req.snapshot_request.get("show_step_ids")),
                "attached_sets": list(req.attached_sets or []),
            }
    audit = {
...
def _finalize_inform_registrations(
    inform_ids: list[str],
    audits: list[dict],
    actor: str,
    *,
    emit_wiki: bool = False,
    emit_semantic: bool = False,
) -> None:
    """Complete lookup, snapshot, audit, and learning work after registration."""
    wanted = {str(v or "").strip() for v in inform_ids if str(v or "").strip()}
    finalized: list[dict] = []
    try:
        with _INFORMS_MUTATION_LOCK:
            items = _load_upgraded()
            dirty = False
            for entry in items:
                if str(entry.get("id") or "") not in wanted:
                    continue
                snapshot_req = entry.get("snapshot_request") if isinstance(entry.get("snapshot_request"), dict) else None
                snapshot_ok = True
                if snapshot_req and not entry.get("embed_table"):
                    try:
                        # 각 Inform의 lot_id로 새 요청을 만들어 root 전체 preview가
                        # 첫 LOT에 재사용되지 않게 한다. 표시 형식/적용 공정 옵션도 유지한다.
                        embed = _build_splittable_snapshot_embed(SplitTableSnapshotReq(
                            product=str(entry.get("product") or ""),
                            lot_id=str(entry.get("lot_id") or entry.get("fab_lot_id_at_save") or ""),
                            custom_cols=snapshot_req.get("custom_cols") or [],
                            is_fab_lot=bool(snapshot_req.get("is_fab_lot")),
                            display_mode=snapshot_req.get("display_mode") or "matrix",
                            show_step_ids=bool(snapshot_req.get("show_step_ids")),
                        ))
                        entry["embed_table"] = _embed_with_attached_sets(
                            _sanitize_embed_table(embed), snapshot_req.get("attached_sets") or [],
                        )
                        entry["attachments"] = _attachment_view(entry)
                    except Exception:
                        snapshot_ok = False
                        logger.warning("deferred Inform snapshot failed inform_id=%s", entry.get("id"), exc_info=True)

                identity = _resolve_lot_identity_snapshot(
                    str(entry.get("product") or ""),
                    str(entry.get("lot_id") or ""),
                    str(entry.get("wafer_id") or ""),
                    entry.get("embed_table"),
                    str(entry.get("fab_lot_id_at_save") or ""),
                )
                root = str(identity.get("root_lot_id") or entry.get("root_lot_id") or "").strip()
                wafer = str(identity.get("wafer_id") or entry.get("wafer_id") or "").strip()
                fab = str(identity.get("fab_lot_id") or entry.get("fab_lot_id_at_save") or "").strip()
                entry["root_lot_id"] = root
                entry["root_lot_ids_at_save"] = list(identity.get("root_lot_ids") or ([root] if root else []))
                entry["wafer_id"] = wafer
                entry["wafer_ids_at_save"] = list(identity.get("wafer_ids") or _split_saved_ids(wafer))
                entry["fab_lot_id_at_save"] = fab
                entry["lot_identity_snapshot"] = {
                    "lot_id": str(entry.get("lot_id") or ""), "fab_lot_id": fab,
                    "root_lot_id": root,
                    "root_lot_ids": list(identity.get("root_lot_ids") or ([root] if root else [])),
                    "wafer_id": wafer,
                    "wafer_ids": list(identity.get("wafer_ids") or _split_saved_ids(wafer)),
                    "source": identity.get("source") or "",
                    "created_at": (entry.get("lot_identity_snapshot") or {}).get("created_at") or entry.get("created_at") or _now(),
                }
                if snapshot_ok:
                    entry.pop("snapshot_request", None)
                    entry.pop("postprocess_pending", None)
                dirty = True
                finalized.append(dict(entry))
            if dirty:
                _save(items)
    except Exception:
        logger.warning("Inform post-registration enrichment failed ids=%s", sorted(wanted), exc_info=True)

    by_id = {str(row.get("id") or ""): row for row in finalized}
    for audit in audits:
        target = by_id.get(str(audit.get("inform_id") or "")) or audit.get("entry") or {}
        _audit_record(actor, audit["type"], target, audit["payload"], audit["summary"], at=audit["at"])
    if emit_wiki:
        try:
            from core.wiki_event_hooks import emit_inform_event
            for entry in finalized:
                emit_inform_event(
                    inform_id=str(entry.get("id") or ""), parent_id=entry.get("parent_id"),
                    title=str(entry.get("title") or entry.get("subject") or ""),
                    text=str(entry.get("text") or entry.get("body") or ""),
                    module=str(entry.get("module") or ""), actor=actor,
                    product=str(entry.get("product") or ""),
                    root_lot_id=str(entry.get("root_lot_id") or entry.get("lot_id") or ""),
                    wafer_id=str(entry.get("wafer_id") or ""),
                )
        except Exception:
            logger.warning("deferred Inform wiki event failed ids=%s", sorted(wanted), exc_info=True)
    if emit_semantic:
        try:
            ...
            logger.warning("deferred Inform semantic submission failed ids=%s", sorted(wanted), exc_info=True)


def _deferred_audit_payload(entry: dict, audit: dict) -> dict:
    return {**audit, "inform_id": str(entry.get("id") or ""), "entry": dict(entry)}


def _resume_pending_inform_registrations(limit: int = 20) -> int:
    """Resume durable post-processing markers left by a restart or task error."""
    try:
        rows = _load_upgraded()
        ids = [
            str(row.get("id") or "")
            for row in rows
            if isinstance(row, dict) and row.get("postprocess_pending") and row.get("id")
        ][:max(1, int(limit or 20))]
        if ids:
            _finalize_inform_registrations(ids, [], "system")
        return len(ids)
    except Exception:
        logger.warning("pending Inform registration resume failed", exc_info=True)
        return 0


def start_inform_postprocess_worker(interval_sec: float = 60.0) -> bool:
    global _INFORM_POSTPROCESS_STARTED, _INFORM_POSTPROCESS_THREAD
    if _INFORM_POSTPROCESS_STARTED:
        return False
    ...
    def _loop() -> None:
        while True:
            try:
                ...
                if is_owner():
                    _resume_pending_inform_registrations()
            except Exception:
                logger.warning("Inform postprocessor ownership check failed", exc_info=True)
            time.sleep(max(10.0, float(interval_sec or 60.0)))

    _INFORM_POSTPROCESS_THREAD = threading.Thread(
        target=_loop, name="inform-registration-postprocess", daemon=True,
    )
    _INFORM_POSTPROCESS_THREAD.start()
    _INFORM_POSTPROCESS_STARTED = True
    return True


@router.post("")
def create_inform(req: InformCreate, request: Request, background_tasks: BackgroundTasks = None):
    create_started = time.perf_counter()
    me = current_user(request)
    with _INFORMS_MUTATION_LOCK:
        items = _load_upgraded()
        entry, audit = _build_inform_entry(req, request, me, items, defer_enrichment=True)
        items.append(entry)
        _save(items)
    args = ([entry["id"]], [_deferred_audit_payload(entry, audit)], me.get("username") or "")
    if background_tasks is not None:
        background_tasks.add_task(_finalize_inform_registrations, *args, emit_wiki=True)
    else:
        _finalize_inform_registrations(*args, emit_wiki=True)
    durable_ms = round((time.perf_counter() - create_started) * 1000, 1)
    logger.info("Inform registration durable id=%s ms=%.1f", entry.get("id"), durable_ms)
    return {"ok": True, "inform": entry, "postprocess_pending": True, "registration_ms": durable_ms}


@router.post("/bulk-create")
def bulk_create_informs(req: InformBulkCreateReq, request: Request, background_tasks: BackgroundTasks = None):
    create_started = time.perf_counter()
    me = current_user(request)
    ...
    with _INFORMS_MUTATION_LOCK:
        ...
        for payload in payloads:
            entry, audit = _build_inform_entry(payload, request, me, items, defer_enrichment=True)
            items.append(entry)
            created.append(entry)
            audits.append(_deferred_audit_payload(entry, audit))
        _save(items)
    args = ([entry["id"] for entry in created], audits, me.get("username") or "")
    if background_tasks is not None:
        background_tasks.add_task(_finalize_inform_registrations, *args, emit_semantic=True)
    else:
        _finalize_inform_registrations(*args, emit_semantic=True)
    durable_ms = round((time.perf_counter() - create_started) * 1000, 1)
    logger.info("Inform bulk registration durable count=%d ms=%.1f", len(created), durable_ms)
    return {"ok": True, "informs": created, "postprocess_pending": True, "registration_ms": durable_ms}
...
def _edit_inform_by_id(id: str, req: InformEditReq, request: Request):
    ...
    if req.text is not None and (req.text or "").strip() != (target.get("text") or ""):
        ...
        use_html = str(req.text_format or target.get("text_format") or "").lower() == "html"
        target["text"] = sanitize_rich_html(req.text) if use_html else (req.text or "").strip()
        target["text_format"] = "html" if use_html else "plain"
...
def _thread_html(items: list, root_id: str) -> str:
    ...
    def render(node: dict, depth: int):
        ...
        if node.get("text_format") == "html":
            body_html = _render_prose_html(body) or "<i style='color:#999'>(본문 없음)</i>"
        else:
...
_GO_FLOW_URL = "http://go/flow_process-inform"
...
def _mail_scroll_col_styles(first_px: int = SPLIT_FIRST_COL_PX,
                            data_px: int = SPLIT_DATA_COL_PX) -> tuple[str, str]:
    """SplitTable 그리드와 같은 고정 폭(항목 288px / wafer 115px). 넘치면 가로 스크롤."""
    return _mail_px_col_style(first_px), _mail_px_col_style(data_px)


def _mail_scroll_table_style(data_columns: int, prefix_px: int = SPLIT_FIRST_COL_PX,
                             data_px: int = SPLIT_DATA_COL_PX) -> str:
    n = max(1, int(data_columns or 0))
    total_px = int(prefix_px) + int(data_px) * n
    return (
        f"border-collapse:collapse;font-size:{_MAIL_SNAPSHOT_FONT};width:{total_px}px;"
...
def _mail_colgroup_from_widths(widths: list[int]) -> str:
    """Split 체크처럼 앞쪽 열 폭이 서로 다른 표용 colgroup."""
    cols = [f"<col style='width:{int(w)}px;max-width:{int(w)}px;'/>" for w in widths]
    return "<colgroup>" + "".join(cols) + "</colgroup>" if cols else ""


def _mail_px_col_style(width_px: int) -> str:
    break_style = "white-space:normal;word-break:break-word;overflow-wrap:anywhere;vertical-align:top;box-sizing:border-box;"
    return f"width:{int(width_px)}px;max-width:{int(width_px)}px;{break_style}"


# SplitTable 그리드의 plan / 미진행 표기를 메일에서도 같은 모습으로.
#   왼쪽 파란선 = plan 이 있다 · 칸 빨강 = plan 과 다르게 진행 · 회색 = 미진행 공정
_SPLIT_PLAN_LINE = "border-left:3px solid #3b82f6;"
_SPLIT_MISMATCH_STYLE = "border-left:3px solid #3b82f6;background:#ef4444;color:#ffffff;"
# rgba(107,114,128,0.45) on white — 일부 메일 클라이언트가 rgba 를 못 읽어 hex 로 굳힌다.
_SPLIT_NOT_REACHED_BG = "background:#bcc0c6;"
...
def _fallback_mail_subject(root: dict) -> str:
    """기본 제목 = `[인폼] 제품 lot 모듈 split table` (FE defaultInformMailSubject 와 같은 규약)."""
    product, lot_label, module, _reason = _mail_subject_parts(root)
    parts = [x for x in [product, lot_label, module] if x]
    return ("[인폼] " + " ".join(parts) + " split table").replace("  ", " ").strip()
...
def _render_embed_table_html(embed: Optional[dict], max_rows: int = 60, module: str = "") -> str:
    ...
    def _wrap(body_rows_html: str, head_cells: list[str], truncated: bool,
        ...
        return (
            ...
            f"</table></div>{trunc_html}{_attached_sets_html()}"
        )
    ...
    snapshot_mode = _splittable_snapshot_display_mode(
        st.get("display_mode") or embed.get("display_mode") or st_scope.get("display_mode")
    )
    split_check_mode = snapshot_mode == "split_check" and len(prefix_columns) >= 3
    merged_mode = snapshot_mode == "merged"
    # SplitTable 화면과 같은 표시 규약 — 소수 자리수 / 미진행 회색 / 항목명 라벨.
    precision = st.get("precision") if isinstance(st.get("precision"), dict) else {}
    not_reached = NotReachedLookup(st)
    step_labels = bool(st.get("step_labels") or embed.get("step_labels"))
    ...
    if split_check_mode and rows_st and headers:
        ...
        data_col_style = _mail_px_col_style(SPLIT_DATA_COL_PX)
        colgroup = _mail_colgroup_from_widths([*prefix_widths, *([SPLIT_DATA_COL_PX] * len(headers))])
        # 화면과 같이 같은 항목이 연달아 나오면 항목 칸을 rowspan 으로 합친다.
        param_spans: list[int] = []
        for idx, row in enumerate(shown):
            param = str(row.get("_param") or "").strip()
            if not param:
                param_spans.append(1)
                continue
            prev = str(shown[idx - 1].get("_param") or "").strip() if idx > 0 else ""
            if prev == param:
                param_spans.append(0)
                continue
            span = 1
            for j in range(idx + 1, len(shown)):
                if str(shown[j].get("_param") or "").strip() != param:
                    break
                span += 1
        ...
        for ri, r in enumerate(shown):
            ...
            for i in range(len(headers)):
                ...
                row_cells.append(
                    f"<td style='{td_check}{data_col_style}"
                    f"{split_color if mark else ''}{cell_gray}'>{esc(mark)}</td>"
                )
            body_parts.append("<tr>" + "".join(row_cells) + "</tr>")

        thead_parts = [
            "<tr>"
            f"<th colspan='{len(prefix_columns)}' style='{th_prefix}{_mail_px_col_style(prefix_total_px)}'>{esc(root_row_label)}</th>"
            f"<th colspan='{max(1, len(headers))}' style='{th_root}{_mail_data_col_style(data_col_style, len(headers))}'>{esc(root_lot_id or '—')}</th>"
            "</tr>"
        ]
        if header_groups:
            cells = [f"<th colspan='{len(prefix_columns)}' style='{th_prefix}{_mail_px_col_style(prefix_total_px)}'>{esc(lot_row_label)}</th>"]
            ...
            thead_parts.append(
                "<tr>"
                f"<th colspan='{len(prefix_columns)}' style='{th_prefix}{_mail_px_col_style(prefix_total_px)}'>{esc(lot_row_label)}</th>"
        ...
        thead_parts.append("<tr>" + "".join(
            f"<th style='{th_style}{prefix_col_styles[i] if i < len(prefix_col_styles) else data_col_style}'>{esc(c)}</th>"
            for i, c in enumerate(head_cells)
        ) + "</tr>")
        table_style = _mail_scroll_table_style(len(headers), prefix_px=prefix_total_px)
        table_html = (
    ...
    if rows_st and headers:
        ...
        for r in shown:
            ...
            if merged_mode and isinstance(r.get("_merged_runs"), list):
                ...
                runs = r["_merged_runs"]
                for run in runs:
                    span = max(1, int(run.get("span") or 1))
                    value = str(run.get("value") or "")
                    cell_bg = _st_cell_bg(value, uniq_map, param_raw)
                    start = int(run.get("start") or 0)
                    run_gray = _SPLIT_NOT_REACHED_BG if (
                        run.get("not_reached") is True or not_reached.cell(param_raw, start)
                    ) else ""
                    run_bold = "font-weight:700;" if span > 1 else ""
                    tds.append(
                        f"<td colspan='{span}' style='{td_cell_base}"
                        f"{_mail_data_col_style(data_col_style, span)}{cell_bg}{run_bold}{hilite}{run_gray}'>"
                        f"{esc(format_split_cell_value(value, param_raw, precision))}</td>"
                    )
            ...
            for i in range(len(headers)):
                ...
                disp = format_split_cell_value(actual, param_raw, precision) if has_actual else ""
                plan_diff = has_plan and has_actual and str(plan) != str(actual)
                plan_only = has_plan and not has_actual
                # SplitTable 그리드와 같은 규약 — 파란 왼쪽선 = plan 있음,
                # 칸 빨강(흰 글씨) = plan 과 다르게 진행, 나머지는 값만.
                cell_plan_style = ""
                if plan_diff:
                    cell_plan_style = _SPLIT_MISMATCH_STYLE
                    disp_html = (
                        f"<span style='color:#ffffff;font-weight:800'>✗ {esc(disp)}"
                        f"<span style='font-size:{_MAIL_SNAPSHOT_FONT};color:rgba(255,255,255,0.85)'>"
                        f" (≠{esc(str(plan))})</span></span>"
                    )
                elif plan_only:
                    cell_plan_style = _SPLIT_PLAN_LINE + "font-style:italic;font-weight:700;"
                    disp_html = f"<span style='color:{_MAIL_GRID_TEXT};font-style:italic;font-weight:700'>📌 {esc(str(plan))}</span>"
                else:
                    if has_plan:
                        cell_plan_style = _SPLIT_PLAN_LINE
                    disp_html = esc(disp)
                ...
                # 미진행 회색은 화면과 같이 mismatch 빨강까지 덮는다.
                cell_gray = _SPLIT_NOT_REACHED_BG if not_reached.cell(param_raw, i) else ""
                tds.append(f"<td style='{td_cell_base}{data_col_style}{cell_bg}{hilite}{cell_plan_style}{cell_gray}'>{disp_html}</td>")
            body_parts.append("<tr>" + "".join(tds) + "</tr>")
        ...
        return (
            f"{hdr}{note_html}"
            f"{table_html}{trunc_html}{_attached_sets_html()}"
        )
...
def _build_html_body(root: dict, thread_html: str, extra_prose: str,
    ...
    if prose_text.strip():
        prose_block = (
            ...
            f"{_render_prose_html(prose_text)}</div>"
        )
...
# 본문에 붙은 사진 1장당 인라인(base64) 허용 크기. 넘으면 링크로만 남긴다.
_MAIL_INLINE_IMG_MAX = 1024 * 1024


def _render_prose_html(prose_text: str) -> str:
    """메일 본문 prose 렌더 — 마크다운 이미지(`![alt](url)`)는 인라인 이미지로 바꾼다.

    본문 편집에서 붙인 사진이 수신자에게도 보이도록, 업로드된 인폼 이미지는
    data URI 로 임베드한다(사내 메일에서 상대경로 src 는 열리지 않는다).
    한도를 넘는 파일은 링크만 남겨 본문 용량이 터지지 않게 한다.
    """
    # 새 게시판형 note는 이미 서버 allowlist로 정리된 HTML이다. 표는 그대로
    # 유지하고, 상대경로 이미지만 메일용 data URI로 치환한다.
    if re.search(r"<(?:p|div|br|img|table|ul|ol|blockquote|h[1-4])\b", prose_text or "", re.I):
        rich = sanitize_rich_html(prose_text)

        def inline_rich_image(match):
            before, url, after = match.group(1), match.group(2), match.group(3)
            fp = _resolve_inform_attachment(url)
            if fp is None:
                return f"{before}{_html.escape(url, quote=True)}{after}"
            try:
                raw = fp.read_bytes()
                if len(raw) <= _MAIL_INLINE_IMG_MAX:
                    mime = mimetypes.guess_type(fp.name)[0] or "image/png"
                    data = base64.b64encode(raw).decode("ascii")
                    return f"{before}data:{mime};base64,{data}{after}"
            except Exception:
                pass
            return "<span style='color:#6b7280'>[이미지]</span>"

        return re.sub(
            r'(<img\b[^>]*\bsrc=["\'])(/api/informs/files/[^"\']+)(["\'][^>]*>)',
            inline_rich_image,
            rich,
            flags=re.I,
        )
    ...
    out: list[str] = []
    pos = 0
    for m in re.finditer(r"!\[([^\]]*)\]\(([^)\s]+)\)", prose_text or ""):
        out.append(esc(prose_text[pos:m.start()]).replace("\n", "<br/>"))
        pos = m.end()
        ...
        if fp is not None:
            try:
                ...
                if len(raw) <= _MAIL_INLINE_IMG_MAX:
                    ...
                    img_html = (
                        f"<div style='margin:6px 0;'><img src='data:{mime};base64,{b64}' "
                        f"alt='{esc(alt)}' style='max-width:100%;height:auto;border:1px solid #e5e7eb;'/></div>"
                    )
            except Exception:
                img_html = ""
        if not img_html:
            img_html = (
                f"<div style='margin:6px 0;font-size:{_MAIL_MIN_FONT};color:#6b7280;'>"
                f"[이미지] {esc(alt or url)}</div>"
            )
        out.append(img_html)
    out.append(esc(prose_text[pos:]).replace("\n", "<br/>"))
    return "".join(out)


def _mail_links_html() -> str:
    """톱니바퀴(인폼 설정)에서 관리하는 메일 하단 링크 목록."""
    esc = _html.escape
    try:
        links = _clean_mail_links(_load_config().get("mail_links"))
    except Exception:
        links = []
    if not links:
        return ""
    items = " · ".join(
        f"<a href='{esc(x['url'])}' target='_blank' rel='noopener noreferrer' "
        f"style='color:#374151;text-decoration:underline;font-weight:700;'>{esc(x['label'])}</a>"
        for x in links
    )
    return (
        f"<div style='font-size:{_MAIL_MIN_FONT};color:#6b7280;margin-top:2px;'>{items}</div>"
    )
...
# 기본 한도 — 실제 판정은 인폼 톱니바퀴 설정(mail_max_mb)을 따르는 _mail_content_max_bytes().
MAIL_CONTENT_MAX = 2 * 1024 * 1024          # 2 MB HTML body (fallback)
...
class DraftMailPreviewReq(BaseModel):
    """아직 저장되지 않은 신규 인폼(위저드)용 메일 미리보기 입력."""
    product: str = ""
    lot_id: str = ""
    root_lot_id: str = ""
    module: str = ""
    ...
    body: str = ""
    author: str = ""
    embed_table: Optional[dict] = None


@router.post("/mail-preview-draft")
def mail_preview_draft(req: DraftMailPreviewReq, request: Request):
    """위저드(등록 전) 메일 미리보기 — 저장 후 발송되는 것과 **같은** HTML 을 만든다.

    상세 화면의 `/{id}/mail-preview` 와 렌더러(_build_html_body)를 공유하므로
    신규 인폼 미리보기와 인폼 상세의 메일 양식이 어긋나지 않는다.
    """
    me = current_user(request)
    target = {
        "product": req.product,
        "lot_id": req.lot_id or req.root_lot_id,
        "root_lot_id": req.root_lot_id or req.lot_id,
        "module": req.module,
        "reason": req.reason or "PEMS",
        "text": req.text,
        "author": req.author or me.get("username", ""),
        "created_at": datetime.datetime.now().isoformat(timespec="seconds"),
        "embed_table": req.embed_table or None,
    }
    pc_data = _load_product_contacts()
    pc_list = (pc_data.get("products") or {}).get(req.product, []) or []
    html = _build_html_body(target, "", req.body or "", sender_username=me.get("username", ""),
                            product_contacts=pc_list, embed_table=target["embed_table"])
    size_bytes = len(html.encode("utf-8"))
    snap = _build_inform_snapshot_xlsx(target)
    attachment_bytes = len(snap[1]) if snap else 0
    limit_bytes = _mail_content_max_bytes()
    total_bytes = size_bytes + attachment_bytes
    return {
        ...
        "auto_attachments": [{"name": snap[0], "bytes": attachment_bytes}] if snap else [],
        "attachment_total_bytes": attachment_bytes,
        "attachment_total_kb": round(attachment_bytes / 1024, 1),
        "mail_total_bytes": total_bytes,
        "mail_total_kb": round(total_bytes / 1024, 1),
        "mail_limit_bytes": limit_bytes,
        "mail_limit_mb": round(limit_bytes / 1024 / 1024, 2),
        "mail_over_limit": total_bytes > limit_bytes,
...
def mail_preview(inform_id: str, request: Request, body: str = Query(""),
                 ...
                 attachments: Optional[List[str]] = Query(None)):
    ...
    q_attachments = list(attachments) if isinstance(attachments, list) else []
    ...
    snap = _build_inform_snapshot_xlsx(target)
    limit_bytes = _mail_content_max_bytes()
    auto_attachment_bytes = len(snap[1]) if snap else 0
    selected_attachment_bytes = 0
    selected_attachment_rows = []
    for url in q_attachments:
        fp = _resolve_inform_attachment(url)
        if fp is None:
            continue
        try:
            ...
            continue
        selected_attachment_bytes += file_bytes
        selected_attachment_rows.append({"name": fp.name, "bytes": file_bytes})
    attachment_bytes = auto_attachment_bytes + selected_attachment_bytes
    total_bytes = size_bytes + attachment_bytes
    ...
    return {
        ...
        "html_size_bytes": size_bytes,
        "html_size_kb": round(size_bytes / 1024, 1),
        "html_size_limit_bytes": limit_bytes,
        "html_over_limit": size_bytes > limit_bytes,
        "auto_attachments": ([{"name": snap[0], "bytes": auto_attachment_bytes}] if snap else []) + selected_attachment_rows,
        "attachment_total_bytes": attachment_bytes,
        "attachment_total_kb": round(attachment_bytes / 1024, 1),
        # 본문 + 자동첨부 합계 기준 한도 판정 — FE 는 이 값으로 발송/등록을 막는다.
        "mail_total_bytes": total_bytes,
        "mail_total_kb": round(total_bytes / 1024, 1),
        "mail_limit_bytes": limit_bytes,
        "mail_limit_mb": round(limit_bytes / 1024 / 1024, 2),
...
def _send_inform_mail_core(inform_id: str, req: SendMailReq, me: dict, request: Request = None):
    ...
    html_body = _build_html_body(target, thread_html, (req.body or ""),
                                 ...
                                 product_contacts=pc_list,
                                 embed_table=target.get("embed_table"))
    # 본문 + 첨부 합계가 톱니바퀴에서 설정한 한도를 넘으면 발송하지 않는다.
    mail_limit = _mail_content_max_bytes()
    limit_mb = round(mail_limit / 1024 / 1024, 2)
    content_bytes_len = len(html_body.encode("utf-8"))
    if content_bytes_len > mail_limit:
        raise HTTPException(400, f"메일 본문이 {limit_mb}MB 한도를 초과했습니다 ({content_bytes_len // 1024}KB). 사진/스냅샷을 줄이거나 본문을 단축해 주세요.")
    ...
    if content_bytes_len + attach_total > mail_limit:
        total_kb = (content_bytes_len + attach_total) // 1024
        raise HTTPException(400, f"메일 본문+첨부 총 용량이 {limit_mb}MB 한도를 초과했습니다 ({total_kb}KB). 사진 첨부를 줄여주세요.")
