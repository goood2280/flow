...
def test_minute_budget_slides_and_fails_closed_if_corrupt(monkeypatch):
    ...
    monkeypatch.setattr(llm_usage.time, "time", lambda: 1059.0)
    assert llm_usage.snapshot()["minute_calls_used"] == 1
    monkeypatch.setattr(llm_usage.time, "time", lambda: 1060.0)
    assert llm_usage.snapshot()["minute_calls_used"] == 0
    assert llm_usage.reserve_attempt() == ""
    assert llm_usage.snapshot()["minute_calls_used"] == 1
...
def test_exhausted_budget_blocks_admin_network_call(monkeypatch):
    ...
    monkeypatch.setenv("FLOW_LLM_MINUTE_CALL_LIMIT", "0")
    ...
    assert "minute call limit" in result["error"]
    assert llm_usage.snapshot()["minute_calls_used"] == 0
...
def test_auth_middleware_binds_admin_scope_for_sync_endpoints(monkeypatch):
    ...
    async def run():
        for role in ("admin", "user", ""):
            request = Request({"type": "http", "method": "GET", "path": "/api/filebrowser/chart-builder/assistant",
